const APP_VERSION = 'v1.282.07'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
function addSearchSourcesButton(msgElement, results) {
    console.log("🔘 [addSearchSourcesButton] Called with", results?.length, "results");
    console.log("🔘 [addSearchSourcesButton] msgElement:", msgElement?.className);
    
    let actionsDiv = msgElement.querySelector('.msg-actions');
    if (!actionsDiv) {
        console.log("❌ [addSearchSourcesButton] No .msg-actions found!");
        return;
    }

    // منع التكرار
    if (actionsDiv.querySelector('.sources-toggle-btn')) {
        console.log("⚠️ [addSearchSourcesButton] Button already exists");
        return;
    }

    const sourcesBtn = document.createElement('button');
    sourcesBtn.className = 'action-btn sources-toggle-btn';
    sourcesBtn.textContent = `📚 Sources (${results.length})`;
    sourcesBtn.title = 'View search sources';
    ...
    dropdown.innerHTML = results.map(r => 
        ...
        ">[${r.index}] ${escapeHtml(r.title)}</a>`
    ).join('');
    
    sourcesBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        e.preventDefault();
        console.log("🔘 [Sources] Button clicked, toggling dropdown");
    ...
    // إضافة الزر قبل model-label
    const modelLabel = actionsDiv.querySelector('.model-label-tag');
    if (modelLabel) {
        actionsDiv.insertBefore(sourcesBtn, modelLabel);
    } else {
        actionsDiv.appendChild(sourcesBtn);
    }
    
    // إضافة القائمة المنسدلة بعد الزر
    actionsDiv.appendChild(dropdown);
    
    console.log("✅ [addSearchSourcesButton] Button added successfully");
}
...
function initSSE() {
    ...
    if (payload.status === 'searching') {
        ...
        updateStatusText("Thinking");
                    ...
                    break;
               case 'search_results':
                 currentSearchResults = payload.results; // [{ index, title, url }]
                    break;
