const APP_VERSION = 'v1.282.18'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
document.getElementById('registerBtn')?.addEventListener('click', async () => {
    ...
    // حالة 1: تم إدخال بريد إلكتروني وكلمة مرور (تسجيل مع تحقق)
    if (email && password) {
        ...
        // ✅ تسجيل مع إرسال بريد تحقق تلقائي
        const { data, error } = await supabaseClient.auth.signUp({
            email: email,
            password: password,
            options: { 
                data: { nickname: nickname },
                emailRedirectTo: window.location.origin // ✅ بعد تأكيد البريد، يعود للتطبيق
            }
        });
        ...
        // ♂️ عرض رسالة "تحقق من بريدك" وعدم إكمال تلقائياً
        showEmailVerificationMessage(email);
    }
    // حالة 2: تم إدخال بريد فقط بدون كلمة مرور
    else if (email && !password) {
        errorDiv.textContent = 'Password is required when email is provided';
        errorDiv.style.display = 'block';
        return;
    }
    // حالة 3: تسجيل بـ nickname فقط (بدون بريد)
    else {
        errorDiv.style.display = 'none';
        registerBtn.disabled = true;
        btnText.style.display = 'none';
        ...
        // تسجيل بدون بريد (استخدم الطريقة الحالية)
...
allowMicWhenEmpty = false; // منع المايك أثناء الإرسال
    checkInputState(inputEl); // إعادة تفعيل زر الإرسال
