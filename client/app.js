...
async function loadSharedMessage(shareCode) {
    try {
        // استخدام fetch عادي بدون إرسال توكن (للمستخدمين غير المسجلين)
        ...
        // إنشاء محادثة مؤقتة بنفس هيكل convs
        const tempConv = {
            ...
            files: data.files_snapshot || []
        };
        
        // استبدال قائمة المحادثات مؤقتاً
        convs = [tempConv];
        activeId = tempConv.id;
        
        // استعادة الملفات إذا وجدت
        if (data.files_snapshot && data.files_snapshot.length) {
            projectFiles = data.files_snapshot;
            activeFileIndex = 0;
            renderTabs();
            updateView();
        }
        ...
        // إخفاء عناصر التفاعل (لأنها مجرد عرض)
        ...
        document.getElementById('btn-retry-icon').style.display = 'none';
        document.getElementById('btn-edit-icon').style.display = 'none';
        document.getElementById('restore-checkpoint-btn').style.display = 'none';
        
        // إضافة footer للمشاركة
        ...
        `;
        document.getElementById('messages').appendChild(sharedFooter);
...
// ==========================================
// إنشاء غلاف bottomSection لتوحيد حركة الرفع
// ==========================================
...
// أضف هذه الدالة في بداية الملف أو داخل DOMContentLoaded
// أضف هذه الدوال في بداية الملف أو داخل DOMContentLoaded
// أضف هذه الدوال
// app3.js - ضمن المتغيرات العامة في بداية الملف (قبل DOMContentLoaded)
let avatarVideo = null;
let videoInitialized = false;
let videoPlayed = false; // ✅ متغير جديد لتتبع ما إذا كان الفيديو قد تم تشغيله مرة واحدة
...
loadUserNickname();
    ...
    let convs = [];
     // هذه الجديدة: السماح بالمايك بعد الإرسال
    let activeId = null;
    ...
    let waveData;
    let waveRAF;
    // --- متغيرات نظام الأخطاء وإعادة المحاولة ---
let lastErrorMessageElement = null; // عنصر رسالة الخطأ الحالية
let pendingRetryMessage = null; // الرسالة التي تنتظر إعادة المحاولة
...
let isStageActive = false;
    ...
    let isStageUpdate = false;  // ✅ أضف هذا السطر
...
// بعد تعريف المتغيرات العامة، أضف:
initTaskIcons();
...
// في app4.js، استبدل مستمع زر registerBtn بهذا
// في app4.js، داخل DOMContentLoaded
// ========== استبدال كتلة مستمع زر registerBtn بالكامل ==========
...
// داخل DOMContentLoaded، بعد const registerBtn = ... 

// إذا كان وضع التصحيح مفعّلاً، أظهر شاشة التسجيل فوراً
...
// داخل DOMContentLoaded، استبدل هذا الجزء:
(async function init() {
...
// ===== إصلاح نظام التنقل - ضع هذا في نهاية DOMContentLoaded =====

// ===== نظام الأمثلة التفاعلية =====
const examplesData = {
    flappy: {
        title: 'Flappy Bird',
...
// انتظر قليلاً حتى يتم إنشاء جميع العناصر
 // انتظر نصف ثانية لضمان تحميل DOM
// ==========================================
// نظام الباقات والاشتراكات (Plans & Subscriptions)
// ==========================================
