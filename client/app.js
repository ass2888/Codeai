const APP_VERSION = 'v1.281.88'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
video.load();
    ...
    // استبدل دالة handleVideoEnd داخل initSplashScreen بهذه النسخة
function handleVideoEnd() {
    ...
    // إضافة كلاس fade-out لبدء التلاشي
    if (videoContainer) {
        videoContainer.classList.add('fade-out');
        
        // بعد انتهاء تأثير التلاشي (500ms)، نخفي الحاوية تماماً
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    // جلب النصوص الترحيبية بعد إخفاء السبلاش
                    if (typeof fetchWelcomeMessages === 'function') {
        ...
        }, 500); // مدة التلاشي 500ms
    } else {
        // Fallback إذا لم يوجد container
        splash.classList.add('hidden');
        document.body.style.overflow = '';
        document.body.style.touchAction = '';
        if (typeof fetchWelcomeMessages === 'function') {
...
async function fetchWelcomeMessages() {
    ...
    // بدء تأثير الكتابة بعد الحصول على النصوص
...
function typeNextWelcomeMessage() {
    ...
    welcomeTypingInterval = setInterval(() => {
        if (charIndex < message.length) {
            currentWelcomeDisplay += message[charIndex];
