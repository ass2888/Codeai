const APP_VERSION = 'v1.281.49'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
async function fetchConversationFromServer(convId) {
    ...
    try {
        ...
        // تحويل الرسائل إلى نفس هيكل conv.messages القديم
        ...
        // تحويل الملفات إلى نفس هيكل projectFiles القديم
        const files = data.files.map(file => ({
            name: file.name,
            content: file.content,
            url: file.url,
            previewUrl: file.preview_url,
            previewUrls: file.preview_urls,
            type: file.type,
            isBinary: !!file.url
        }));
...
function updatePreviewButtonVisibility() {
    ...
    const btnRunOptions = document.getElementById('btnRunOptions'); // زر الثلاث نقاط
    const currentFile = projectFiles[activeFileIndex];
    if (!currentFile) return;
    if (isImageFile(currentFile.name)) {
        // إخفاء زر المعاينة للصور
        runFab.style.display = 'none';
        
        // إخفاء زر الثلاث نقاط أيضاً
        if (btnRunOptions) {
            btnRunOptions.style.display = 'none';
        }
        
        // إظهار رسالة أن هذا ملف صورة
    ...
    runFab._pdfHandler && runFab.removeEventListener('click', runFab._pdfHandler);
runFab._pdfHandler = () => window.open(currentFile.url || '/generated/' + currentFile.name, '_blank');
runFab.addEventListener('click', runFab._pdfHandler);
    } else {
      if (runFab._pdfHandler) {
    runFab.removeEventListener('click', runFab._pdfHandler);
    runFab._pdfHandler = null;
}
        // إظهار زر المعاينة للملفات الأخرى
        runFab.style.display = '';
        
        // إظهار زر الثلاث نقاط
        if (btnRunOptions) {
            btnRunOptions.style.display = '';
        }
        
        // إزالة رسالة الحالة إذا موجودة
        const statusMsg = document.getElementById('imageStatusMsg');
        if (statusMsg) statusMsg.remove();
    }
}
...
function displayDocumentInEditor(file) {
    ...
    // إنشاء أو تحديث حاوية عرض المستند
    ...
    if (!docContainer) {
        ...
        const editorContainer =
            document.querySelector('.code-editor-container') || codeArea.parentNode;
...
document.getElementById('nicknameInput')?.addEventListener('keypress', (e) => {
    ...
    // حفظ على السيرفر
    await saveConversationToServer(newConv);
    
    // تحديث الواجهة
    codeArea.value = '// Start coding...';
    updateView();
    renderMessages();
    renderConversations();
...
if (previewUrls && previewUrls.length > 0) {
    ...
    // مسار قديم للتوافق مع الملفات القديمة
...
const editorContainer = codeArea.parentElement;
editorContainer.innerHTML = `<textarea id="codeArea"></textarea>`;
...
async function loadConversations() {
    ...
    // ✅ فقط من السيرفر، لا دمج مع localStorage
    const serverConvs = await fetchConversationsFromServer();
    
    convs = serverConvs;
    
    // ❌ لا نحفظ في localStorage
    // localStorage.setItem('codeai_convs', JSON.stringify(convs));
...
if (isStageUpdate && !isStage) {
    ...
    if (!isBackground && !isStage) {
        // حاول العثور على JSON في النص المتراكم
        const jsonMatch = safeBuffer.match(/\{[\s\S]*\}/);
        if (jsonMatch) {
            try {
                ...
                if (parsed.type === 'document' && parsed.document) {
                    ...
                    if (parsed.user_message) {
                        // مسح النص الحالي وعرض الرسالة فقط
                        if (currentAiMsgElement) {
                            const contentEl = currentAiMsgElement.querySelector('.ai-content');
                            if (contentEl) {
                    ...
                    // تنظيف safeBuffer ومنع المزيد من المعالجة
                    safeBuffer = "";
                    streamCursor = 0;
                    typeQueue = [];
                    ...
                    return; // نخرج من الدالة لأننا عالجنا كل شيء
                }
            } catch (e) {
                // ليس JSON صالح، نكمل بشكل طبيعي
                console.log("Not a valid JSON document");
            }
        }
    }
