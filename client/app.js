const APP_VERSION = 'v1.282.30'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
async function fetchConversationFromServer(convId) {
    ...
    // تجميع الرسائل في هيكل المستخدم + الردود
    const userMessages = [];
    const messageMap = new Map();
    
    // أولاً: جمع رسائل المستخدم
    data.messages.forEach(msg => {
        if (msg.role === 'user') {
            ...
            messageMap.set(msg.id, userMsg);
        }
    });
    
    // ثانياً: إضافة الردود إلى رسائل المستخدم المناسبة
    data.messages.forEach(msg => {
        if (msg.role === 'ai' && msg.parent_id && messageMap.has(msg.parent_id)) {
    ...
    // ترتيب الردود زمنياً وتعيين currentReplyIndex
    userMessages.forEach(userMsg => {
        userMsg.replies.sort((a,b) => a.timestamp - b.timestamp);
        userMsg.currentReplyIndex = userMsg.replies.length - 1; // آخر رد
    });
