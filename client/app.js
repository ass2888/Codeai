const APP_VERSION = 'v1.280.60'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
function injectThoughtButton(msgElement, thoughtText) {
    ...
    let thoughtBox = msgElement.querySelector('.thought-content-box');
    if (!thoughtBox) {
        thoughtBox = document.createElement('div');
        thoughtBox.className = 'thought-content-box';
    ...
    if (header && !header.querySelector('.thought-toggle-btn')) {
        ...
        btn.className = 'thought-toggle-btn';
        btn.textContent = 'Analysis'; // أو "التحليل"
...
if (savedThought) {
    ...
    const langu = localStorage.getItem('codeai_lang') || 'en';
thoughtBtn.textContent = translations[langu].thinking;

    const thoughtContent = document.createElement('div');
    thoughtContent.className = 'thought-content hidden';
    thoughtContent.textContent = savedThought;

    thoughtBtn.onclick = () => {
    const opened = !thoughtContent.classList.contains('hidden');
    thoughtContent.classList.toggle('hidden');

    const lang = localStorage.getItem('codeai_lang') || 'en';
    thoughtBtn.textContent = opened
        ? translations[lang].thinking
        : translations[lang].hideThinking;
};

    thoughtContainer.appendChild(thoughtBtn);
    thoughtContainer.appendChild(thoughtContent);

    // 🔥 الإدراج الصحيح: فوق رد النموذج مباشرة
    d.insertBefore(thoughtContainer, contentEl);
}
