const APP_VERSION = 'v1.281.76'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
async function fetchConversationFromServer(convId) {
    ...
    try {
        ...
        // ✅ تحويل الرسائل مع إضافة tasks فقط
        const messages = data.messages.map(msg => ({
            role: msg.role,
            text: msg.content,
            model: msg.model,
            thought: msg.thought,
            tasks: msg.tasks,  // ✅ كائن JSON فقط
...
async function fetchConversationFromServer(convId) {
    ...
    try {
        ...
        // ✅ تحويل الرسائل مع إضافة filesSnapshot
        const messages = data.messages.map(msg => ({
            role: msg.role,
            text: msg.content,
            model: msg.model,
            thought: msg.thought,
            tasks: msg.tasks,
            filesSnapshot: msg.files_snapshot,  // ✅ لقطة الملفات
            created_at: msg.created_at
        }));
...
function addRestoreButtonToExistingMessage(contentEl, msgElement, filesSnapshot) {
    ...
    conv.messages.forEach((m, index) => {
        ...
        if (m.role === 'user') {
            ...
            // ✅ استخدام المهام المحفوظة (كائن JSON) لإنشاء HTML
            if (m.tasks && (m.tasks.built?.length || m.tasks.modified?.length || m.tasks.deleted?.length)) {
                tasksHTML = generateTasksHTML(m.tasks);  // ✅ إنشاء HTML من الكائن
            }
...
function generateTasksHTML(tasks) {
    ...
    if (tasks && typeof tasks === 'object' && !Array.isArray(tasks)) {
        ...
        return _generateTasksHTMLFromObject(tasks);
    }
...
function _generateTasksHTMLFromObject(tasks) {
    if (!tasks.built?.length && !tasks.modified?.length && !tasks.deleted?.length) {
        return '';
    }
...
function addRestoreButton(contentEl, msgElement) {
    ...
    // الحصول على لقطة الملفات من الرسالة
    const conv = convs.find(c => c.id === activeId);
    const msgIndex = conv.messages.findIndex(m => m === conv.messages[conv.messages.length - 1]);
    const filesSnapshot = conv.messages[msgIndex]?.filesSnapshot;
