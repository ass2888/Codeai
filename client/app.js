const APP_VERSION = 'v1.280.64'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
function initSplashScreen() {
    ...
    if (!splash || !video) {
        console.log('❌ عناصر السبلاش غير موجودة');
        return;
    }
    
    console.log('✅ بدء شاشة السبلاش');
    ...
    setTimeout(() => {
        ...
        console.log('✅ بعد 1 ثانية: التطبيق ظاهر خلف الفيديو');
    }, 2500);
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        if (splash && splash.parentNode) {
                            ...
                            console.log('✅ السبلاش أزيلت من DOM');
                        }
                    }, 500);
                }, 2000);
            }, 500);
    ...
    setTimeout(() => {
        ...
        if (splash && !splash.classList.contains('hidden') && video.duration > 0) {
            console.log('⏰ تفعيل انتهاء الفيديو يدوياً');
            handleVideoEnd();
        }
    }, 5500);
    ...
    setTimeout(() => {
        if (splash && !splash.classList.contains('hidden')) {
            console.log('⚠️ تجاوز الحد الزمني، إخفاء السبلاش');
            splash.classList.add('hidden');
...
function injectThoughtButton(msgElement, thoughtText) {
    ...
    if (!thoughtBox) {
        ...
        thoughtBox.style.display = 'none'; // ⚡ مخفي افتراضياً
        
        // نضيفه قبل محتوى الرسالة (ai-content)
...
if (savedThought) {
    // 1. الحصول على header أو إنشاءه
    let header = d.querySelector('.msg-header');
    if (!header) {
        header = document.createElement('div');
        header.className = 'msg-header';
        d.insertBefore(header, contentEl);
    }

    // 2. إنشاء الزر
    const thoughtBtn = document.createElement('button');
    thoughtBtn.className = 'thought-toggle-btn thought-btn';
    
    const lang = localStorage.getItem('codeai_lang') || 'en';
    thoughtBtn.textContent = translations[lang].thinking || 'Analysis';
    thoughtBtn.setAttribute('data-show-text', translations[lang].thinking || 'Analysis');
    thoughtBtn.setAttribute('data-hide-text', translations[lang].hideThinking || 'Hide');
    thoughtBtn.style.opacity = '0.5'; // شفاف افتراضياً

    // 3. إنشاء صندوق المحتوى
    const thoughtBox = document.createElement('div');
    thoughtBox.className = 'thought-content';
    thoughtBox.textContent = savedThought;
    thoughtBox.style.display = 'none'; // مخفي افتراضياً

    // 4. وظيفة التبديل
    thoughtBtn.onclick = function() {
        if (thoughtBox.style.display === 'none' || !thoughtBox.style.display) {
            thoughtBox.style.display = 'block';
            thoughtBtn.style.opacity = '1';
            thoughtBtn.textContent = thoughtBtn.getAttribute('data-hide-text');
        } else {
            thoughtBox.style.display = 'none';
            thoughtBtn.style.opacity = '0.5';
            thoughtBtn.textContent = thoughtBtn.getAttribute('data-show-text');
        }
    };

    // 5. إضافة الزر للهيدر
    header.appendChild(thoughtBtn);

    // 6. إضافة الصندوق قبل المحتوى
    d.insertBefore(thoughtBox, contentEl);
}
