const APP_VERSION = 'v1.280.71'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
function stopRecording() {
  ...
  // إيقاف المؤقتات وإخفاء العناصر البصرية
  ...
  // دالة مساعدة لإغلاق جميع المسارات
  ...
  // إذا كان mediaRecorder موجوداً ولم يتوقف بعد
  if (mediaRecorder && mediaRecorder.state !== 'inactive') {
    // إعداد معالج onstop قبل الإيقاف
    mediaRecorder.onstop = () => {
      console.log("✅ MediaRecorder stopped");
      const blob = new Blob(audioChunks, { type: mediaRecorder.mimeType });
      audioChunks = [];
      
      // إرسال الصوت للخادم (مع معالجة الأخطاء)
      sendAudio(blob).catch(err => {
        console.error("❌ STT send failed:", err);
      });

      // إغلاق المسارات (في حال لم تُغلق بعد)
      if (mediaRecorder.stream) {
        stopAllTracks(mediaRecorder.stream);
      }
    };
    ...
    try {
      ...
      // إذا فشل الإيقاف، أغلق المسارات مباشرة
      if (mediaRecorder.stream) {
        stopAllTracks(mediaRecorder.stream);
      }
    }
  } else {
    // إذا لم يكن mediaRecorder موجوداً، نحاول إغلاق أي دفق عالق
    console.warn("⚠️ mediaRecorder not found or already inactive");
    // قد يكون هناك دفق قديم ما زال نشطاً (مثلاً من محاولة سابقة)
    // يمكنك محاولة الوصول إلى آخر دفق معروف عبر متغير عام
    if (window.lastStream) stopAllTracks(window.lastStream);
  }
