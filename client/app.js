const APP_VERSION = 'v1.281.96'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
function initSplashScreen() {
    ...
    // 1. إخفاء اللوقو والنص في البداية
    ...
    // تعيين مصدر الفيديو حسب الثيم
    if (isLight) {
        video.src = 'assets/splash-light.mp4';
    } else {
        video.src = 'assets/splash-dark.mp4';
    }
    
    // ✅ إعادة تعيين أي مستمعين سابقين
    video.onended = null;
    video.onloadedmetadata = null;
    
    // ✅ متغير للتأكد من عدم تكرار الإخفاء
    let hasEnded = false;
    let videoDuration = 0;
    
    // ✅ انتظار تحميل معلومات الفيديو
    video.onloadedmetadata = function() {
        videoDuration = video.duration;
        console.log("🎬 Video loaded, duration:", videoDuration);
        
        // بدء تشغيل الفيديو بعد تحميل البيانات
        ...
        if (playPromise !== undefined) {
            playPromise.then(() => {
                ...
                // إذا فشل الفيديو، ننتظر قليلاً ثم نخفي السبلاش
                if (!hasEnded) {
                    hasEnded = true;
                    setTimeout(() => hideSplashAndShowWelcome(), 500);
                }
            });
        }
    };
    ...
    // 3. عند انتهاء الفيديو - ✅ فقط هذا الحدث
    video.onended = function() {
        if (!hasEnded) {
            hasEnded = true;
            console.log("🎬 Video ended event triggered at", new Date().toLocaleTimeString());
            hideSplashAndShowWelcome();
        }
    };
    ...
    // 5. بديل آمن: إذا لم ينته الفيديو بعد 8 ثوان (أطول من مدة الفيديو)
    setTimeout(() => {
        if (!welcomeMessagesReady && splash && !splash.classList.contains('hidden')) {
            console.log("⏰ Timeout: forcing splash hide with fallback messages");
    ...
    // أضف هذا داخل initSplashScreen، قبل إغلاق الدالة
// هذا للتأكد من أن welcome-sub سيتم إخفاؤه بعد الفيديو
window.hideWelcomeSubAfterVideo = function() {
    const welcomeSub = document.querySelector('.welcome-sub');
    if (welcomeSub) {
        console.log("Hiding welcome-sub now");
        welcomeSub.style.transition = 'opacity 0.5s ease';
...
async function fetchWelcomeMessages() {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/welcome-messages`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function hideSplashAfterMessagesReady() {
    // ✅ التحقق مرة أخرى من عدم وجود محادثة نشطة
    const hasActiveConversation = activeId && convs.find(c => c.id === activeId);
    
    if (hasActiveConversation) {
        console.log("📂 Active conversation detected, skipping welcome display");
    ...
    // لا توجد محادثة نشطة - أكمل بشكل طبيعي
    splashHiddenDueToMessages = true;
    ...
    // إخفاء الفيديو مع أنميشن مطابق
    if (videoContainer) {
        videoContainer.classList.add('fade-out');
    }
    if (welcomeSub) {
        welcomeSub.classList.add('fade-out');
    }
    
    // إخفاء اللوقو والنسخة إذا كانا ظاهرين
    if (logo) {
        logo.classList.add('fade-out');
    }
    if (version) {
        version.classList.add('fade-out');
    }
    ...
    setTimeout(() => {
        ...
        // ✅ التحقق مرة أخرى قبل إظهار عناصر الترحيب
        const stillNoActiveConversation = !activeId || !convs.find(c => c.id === activeId);
        
        if (stillNoActiveConversation) {
            // إظهار الحاوية
            const container = document.getElementById('dynamicWelcomeContainer');
            if (container) {
            ...
            // بدء تشغيل الفيديو بعد 3 ثوان
            setTimeout(() => {
                console.log("🎬 Starting avatar video after 3 seconds...");
                if (!videoInitialized) {
                    initAvatarVideo();
                }
                
                if (avatarVideo) {
                    if (avatarVideo.ended || videoPlayed) {
                        setTimeout(() => {
                            if (typeof startWelcomeTypingEffect === 'function') {
                                startWelcomeTypingEffect();
                            }
                        }, 100);
                    } else {
                        avatarVideo.addEventListener('ended', () => {
                            console.log("🎬 Avatar video ended, starting typing effect...");
                            setTimeout(() => {
                                if (typeof startWelcomeTypingEffect === 'function') {
                                    startWelcomeTypingEffect();
                                }
            ...
            console.log("📂 Conversation opened during transition, skipping welcome display");
        }
        
        setTimeout(() => {
            if (splash && splash.parentNode) splash.style.display = 'none';
            window._splashHiding = false;
        }, 500);
