...
// تحديث المعاينة (محدثة: كشف الأخطاء + إرسالها للـ Output)
function updatePreview() {
    ...
    if (htmlFile) htmlContent = htmlFile.content;
    ...
    if (!previewFrame.srcdoc) {
        previewFrame.srcdoc = `<html><body style="background:#080808;color:#fff;display:flex;justify-content:center;align-items:center;height:100vh;">Running...</body></html>`;
    }

    // كود الجافاسكربت الذي سيحقن داخل المعاينة لالتقاط الأخطاء وإرسالها للتطبيق
    ...
    (function(){
        function sendToParent(type, args) {
            try {
                ...
                // إرسال الرسالة إلى التطبيق الرئيسي
        ...
        window.addEventListener('unhandledrejection', function(event) {
            sendToParent('error', ["⚠️ Async Error: " + event.reason]);
        });
    })();
    <\/script>
    `;
...
if (typeof(EventSource) !== 'undefined') {
    ...
    if (safeBuffer.length === 0 && payload.text && payload.text !== '\n[STREAM COMPLETE]') {
        ...
        if (!currentAiMsgElement) {
            const d = document.createElement('div');
            // نفترض الاتجاه LTR مبدئياً للكود، وسيتم تعديله لاحقاً
            d.className = 'msg ai ltr'; 
            
            // --- التصحيح هنا: إضافة الهيكل الداخلي (الأيقونة والمحتوى) ---
            d.innerHTML = `
