const APP_VERSION = 'v1.282.13'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
function initAvatarVideo() {
    ...
    avatarVideo.addEventListener('ended', () => {
        avatarVideo.currentTime = avatarVideo.duration - 0.1;
        avatarVideo.pause();
        console.log("🎬 Avatar video ended event triggered");
    }, { once: true });
...
function initsecondVideoLoop() {
    const welcomeVideo = document.getElementById('welcomeVideo');
    if (welcomeVideo) {
        console.log("✅ Welcome video found, setting up loop to 1.7s");
        ...
        function welcomeVideoLoopHandler() {
            console.log("🎬 Welcome video ended, resetting to 1.7s");
        ...
        function welcomeVideoTimeHandler() {
            if (welcomeVideo.duration - welcomeVideo.currentTime <= 0.1) {
                console.log("🎬 Welcome video near end, resetting to 1.7s");
                welcomeVideo.currentTime = 1.7;
                welcomeVideo.play().catch(e => console.log("Replay error:", e));
            }
        }
        ...
        console.warn("⚠️ Welcome video not found, retrying in 500ms...");
...
function hideSplashAndShowWelcome() {
    console.log("🎬 Video ended - checking if messages are ready...");
...
async function fetchUserLimits() {
    ...
    try {
        ...
        bonusQuota = data.bonus_quota || 0;
        lastActiveDate = data.last_active_date || null;
        currentDailyLimit = FREE_DAILY_LIMIT + bonusQuota;
        
        localStorage.setItem('codeai_daily_used', dailyUsed);
        localStorage.setItem('codeai_last_active', lastActiveDate);
        
        updateLimitProgressBar();
        updateAccountLimitsDisplay();
        
    } catch (error) {
        dailyUsed = parseInt(localStorage.getItem('codeai_daily_used') || '0');
        lastActiveDate = localStorage.getItem('codeai_last_active');
        currentDailyLimit = FREE_DAILY_LIMIT + bonusQuota;
    }
}
...
function updateLimitProgressBar() {
    ...
    const percentage = Math.min(100, (dailyUsed / currentDailyLimit) * 100);
    const remainingPercentage = Math.max(0, 100 - percentage);
    
    fill.style.width = `${remainingPercentage}%`;
    
    if (remaining <= 3) {
        fill.style.background = 'linear-gradient(90deg, #ff6b6b, #ff4444)';
        text.textContent = `⚠️ ${remaining} left today`;
    } else if (remaining <= 5) {
        fill.style.background = 'linear-gradient(90deg, #ffa726, #ff9800)';
        text.textContent = `${remaining} messages left`;
    } else {
        fill.style.background = 'linear-gradient(90deg, #64B5F6, #42A5F5)';
        text.textContent = `${remaining} messages left today`;
    }
...
setTimeout(() => {
    ...
    currentStreak = 3;
    updateAccountLimitsDisplay();
    console.log("✅ Account limits section:", document.getElementById('accountLimitsSection')?.style.display);
}, 2000);
