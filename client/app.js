...
// متغيرات الكتابة (التي سببت المشكلة)
let safeBuffer = "";
let typeQueue = [];
let typeTimeout = null;      // تأكد أن هذا المتغير معرف هنا فقط
let serverFinished = false;  // تأكد أن هذا المتغير معرف هنا فقط
let fullMarkdownBuffer = "";
let streamCursor = 0;
let currentAiMsgElement = null;
let incomingPacketBuffer = [];
...
// ============================================================
// 1. دالة استقبال البيانات من السيرفر (مصححة)
// ============================================================
window.handleServerPacket = (payload) => {
    // تجاهل أي رسالة لا تحتوي على نص أو نوع
    if (!payload) return;

    // أ) التحقق من أمر الإنهاء الصريح (من core.js أو السيرفر)
    if (payload.type === 'done') {
        serverFinished = true;
        checkAndFinish(); // فحص فوري للإنهاء
        return;
    }
    ...
    // فحص علامة النهاية النصية القادمة من السيرفر
    const endMarker = "[STREAM COMPLETE]";
    if (chunk.includes(endMarker)) {
        serverFinished = true;
        // حذف العلامة لتنظيف النص
        chunk = chunk.replace(endMarker, "").replace(endMarker, ""); 
    }

    // إذا تبقى نص حقيقي، نرسله للمعالجة
    if (chunk.length > 0) {
        processTextChunk(chunk);
    }

    // ج) نقطة الإصلاح الحاسمة:
    // إذا انتهى السيرفر، يجب التأكد من استدعاء الإنهاء حتى لو لم يكن هناك نص جديد
    if (serverFinished) {
        checkAndFinish();
    }
};

// دالة مساعدة لفحص هل يمكننا الإنهاء الآن؟
function checkAndFinish() {
    // ننهي فقط إذا كان طابور الكتابة فارغاً، ولا يوجد مؤقت كتابة يعمل حالياً
    if (typeQueue.length === 0 && !typeTimeout) {
        finishMessageProcessing();
    } 
    // ملاحظة: إذا كان الطابور ممتلئاً، فإن typeLoop سيتولى الإنهاء تلقائياً عند نفاذ الحروف
}
...
// ============================================================
// 2. حلقة الكتابة (مصححة لضمان عدم التوقف المفاجئ)
// ============================================================
function startTyping() {
    // إذا كانت الحلقة تعمل بالفعل، لا تشغلها مرة أخرى
    if (typeTimeout) return; 
    ...
    function typeLoop() {
        if (typeQueue.length > 0) {
            ...
            // 3. تحديد السرعة (سريع جداً إذا تراكم النص)
            const queueLen = typeQueue.length;
            let delay = queueLen > 50 ? 1 : (queueLen > 20 ? 3 : 10);
            ...
            // هل وصلتنا إشارة انتهاء من السيرفر؟
            if (serverFinished) {
                finishMessageProcessing();
            }
        }
...
// ============================================================
// 3. دالة إنهاء المعالجة وتفعيل الأزرار
// ============================================================
function finishMessageProcessing() {
    console.log("Processing Finished. Updating UI..."); // للتأكد في الكونسول

    // تنظيف
    if (typeTimeout) clearTimeout(typeTimeout);
    typeTimeout = null;
    serverFinished = false; // إعادة تعيين للمرة القادمة
    ...
    if (activeId && currentAiMsgElement) {
        ...
        // 1. حفظ النص النهائي في سجل المحادثة
        if (conv && conv.messages.length > 0) {
            // نتأكد أننا نحدث آخر رسالة للـ AI
        ...
        // 2. تحويل الماركداون إلى HTML
        const contentEl = currentAiMsgElement.querySelector('.ai-content');
        if (contentEl) {
            let html = typeof marked !== 'undefined' ? marked.parse(fullMarkdownBuffer) : fullMarkdownBuffer;
            contentEl.innerHTML = html;
            
            // 3. إضافة الأزرار (Copy, Retry, Edit)
            // نتأكد من عدم إضافتها مرتين
            if (!contentEl.querySelector('.msg-actions')) {
                addMessageActions(contentEl, fullMarkdownBuffer);
            }
        }
        
        // 4. معالجة نهائية للملفات (لضمان التقاط أي تغييرات معلقة)
        if (safeBuffer) {
...
// --- Version Display Logic ---
    const APP_VERSION = 'v1.275.1'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
