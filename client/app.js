const APP_VERSION = 'v1.282.42';
const versionEl = document.getElementById('appVersion');
if(versionEl) {

    const userId = localStorage.getItem('userId');
    versionEl.innerHTML = `${APP_VERSION}`
    //<br><span style="font-size:9px;opacity:0.6;">user-${userId}</span>`;
}


// ==========================================
// DEBUG MODE - إظهار صفحة التسجيل كل مرة
// ==========================================
const DEBUG_FORCE_REGISTER = false; // ✅ غيّر إلى false لتعطيل وضع التصحيح

// تعديل دالة checkUserRegistration لتعمل مع وضع التصحيح


// تعديل دالة loadConversations لمنع تحميل المحادثات في وضع التصحيح
...
const translations = {
    en: {
        ...
        betaDiscount: "Beta Launch Offer",
        commands: "Commands",
        cmdCreatePDF: "Create a PDF",
        cmdCreatePDFDesc: "Ask me to \"Create a PDF about...\" and I will generate a formatted PDF file for you.",
        cmdCreateDOCX: "Create a Word Document",
cmdCreateDOCXDesc: "Request a DOCX document by describing the content, and I'll craft a professional Word file.",
cmdCreatePPTX: "Create a PowerPoint",
cmdCreatePPTXDesc: "Turn your ideas into slides: \"Make a presentation about...\" generates a PPTX.",
cmdSearch: "Web Search",
cmdSearchDesc: "I can search the internet for real-time information when you ask about recent events or facts.",
cmdQuiz: "Quiz Generator",
cmdQuizDesc: "Ask me to \"Create a quiz on...\" and I'll build an interactive multiple-choice quiz.",
commandsAuto: "These commands are automatic — just ask the model and they'll activate instantly.",
    },
    ar: {
        settings: "الإعدادات",
        theme: "المظهر",
        ...
        betaDiscount: "عرض إصدار Beta",
        commands: "الأوامر",
cmdCreatePDF: "إنشاء PDF",
cmdCreatePDFDesc: "اطلب مني \"أنشئ PDF عن...\" وسأقوم بإنشاء ملف PDF مُنسق لك.",
cmdCreateDOCX: "إنشاء مستند Word",
cmdCreateDOCXDesc: "اطلب مستند DOCX بوصف المحتوى، وسأصنع ملف Word احترافي.",
cmdCreatePPTX: "إنشاء عرض تقديمي",
cmdCreatePPTXDesc: "حوّل أفكارك إلى شرائح: \"اصنع عرضاً عن...\" يولد ملف PPTX.",
cmdSearch: "البحث في الإنترنت",
cmdSearchDesc: "أستطيع البحث عن معلومات حديثة عندما تسأل عن أحداث أو حقائق جديدة.",
cmdQuiz: "إنشاء اختبار",
cmdQuizDesc: "اطلب مني \"أنشئ اختباراً في...\" وسأبني لك اختباراً تفاعلياً متعدد الخيارات.",
    commandsAuto: "هذه الأوامر تلقائية — فقط اطلب من النموذج وستُفعّل مباشرة.",
    }
};
...
const commandsData = [
    {
        id: 'pdf',
        titleEn: 'Create a PDF...',
        titleAr: 'إنشاء PDF',
        descEn: 'Generates a formatted PDF file from your description.',
        descAr: 'إنشاء ملف PDF منسق من وصفك.',
        icon: '📄'
    },
    {
        id: 'docx',
        titleEn: 'Create a Word Document...',
        titleAr: 'إنشاء مستند Word',
        descEn: 'Generates a professional Word document (.docx) from your input.',
        descAr: 'إنشاء مستند Word احترافي (.docx) من مدخلاتك.',
        icon: '📝'
    },
    {
        id: 'pptx',
        titleEn: 'Create a PowerPoint...',
        titleAr: 'إنشاء عرض تقديمي',
        descEn: 'Generates a PowerPoint presentation (PPTX) from your ideas.',
        descAr: 'إنشاء عرض تقديمي PowerPoint (PPTX) من أفكارك.',
        icon: '📊'
    },
    {
        id: 'search',
        ...
        descEn: 'Searches the web for real-time information on recent events or facts.',
        descAr: 'البحث في الإنترنت عن معلومات حديثة حول الأحداث أو الحقائق.',
        icon: '🔍'
    },
    {
        id: 'quiz',
        titleEn: 'Create a Quiz...',
        titleAr: 'إنشاء اختبار تفاعلي',
        descEn: 'Builds an interactive multiple-choice quiz from any topic.',
        descAr: 'بناء اختبار تفاعلي متعدد الخيارات من أي موضوع.',
        icon: '❓'
    }
];
...
function initTaskIcons() {
    // إنشاء template للأيقونات إذا لم يكن موجوداً
    if (!document.getElementById('taskIconsTemplate')) {
        const template = document.createElement('template');
        template.id = 'taskIconsTemplate';
        template.innerHTML = `
            <svg class="task-icon-svg built" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <svg class="task-icon-svg modified" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M17 3L21 7L7 21H3V17L17 3Z" stroke="currentColor" stroke-width="1.5" fill="none" stroke-linejoin="round"/>
                <path d="M15 5L19 9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
            </svg>
            <svg class="task-icon-svg deleted" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path d="M6 7L8 21H16L18 7" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
                <path d="M9 4H15" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
            </svg>
        `;
        document.body.appendChild(template);
        taskIconsTemplate = template;
    }
}

// دالة مساعدة للحصول على نسخة من أيقونة SVG
function getTaskIcon(type) {
    if (!taskIconsTemplate) initTaskIcons();
    
    const template = document.getElementById('taskIconsTemplate');
    if (!template) return '';
    
    const clone = template.content.cloneNode(true);
    const icon = clone.querySelector(`.task-icon-svg.${type}`);
...
// مراجع للمستمعات العامة التي قد تمنع التمرير
let globalTouchMoveHandler1 = null;
let globalTouchMoveHandler2 = null;
let globalTouchMoveHandler3 = null;

function pauseGlobalTouchHandlers() {
    // ابحث عن المستمعات العامة وأزلهم (سيتم تخزينهم لاحقًا)
    // لكن بما أننا لا نعرف مراجعهم الأصلية، سنستخدم طريقة بديلة:
    // نضيف مستمعًا جديدًا على document يوقف الانتشار قبل وصوله للمستمعات القديمة.
    // الطريقة الأفضل: إعادة تعريف المتغيرات العامة التي تتحكم بالسحب.
    if (window._globalTouchMove1) {
        document.removeEventListener('touchmove', window._globalTouchMove1);
    }
    if (window._globalTouchMove2) {
        document.removeEventListener('touchmove', window._globalTouchMove2);
    }
    // سنقوم بتخزينهم عند إضافتهم أول مرة.
}

function resumeGlobalTouchHandlers() {
    if (window._globalTouchMove1) {
        document.addEventListener('touchmove', window._globalTouchMove1, { passive: false });
    }
    if (window._globalTouchMove2) {
        document.addEventListener('touchmove', window._globalTouchMove2, { passive: false });
    }
}
...
// أضف داخل updateUIText
const welcomeText = document.getElementById('dynamicWelcomeText');
if (welcomeText) {
    if (lang === 'ar') {
...
if (cancelBtn) cancelBtn.textContent = t.cancel;
    ...
    if (document.getElementById('commandsPage')?.classList.contains('open')) {
    renderCommandsPage();
}
}
...
const SUPABASE_URL = 'https://slchljnlapixdtxqucny.supabase.co'; // عنوان مشروعك
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNsY2hsam5sYXBpeGR0eHF1Y255Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzM1MDg1NTEsImV4cCI6MjA4OTA4NDU1MX0.WWaZRV-DDPQ_z-UPTQQ16rCoqMul2tgjow0DB9jcASY'; // استبدل بـ anon key الحقيقي
...
async function initAuth() {
  ...
  if (session) {
    userId = session.user.id;
    // ✅ تأكد من أن localStorage يحتوي على المعرف الصحيح
...
async function signInWithGoogle() {
  const { error } = await supabaseClient.auth.signInWithOAuth({
    provider: 'google',
    options: { redirectTo: window.location.origin } // بعد العودة للموقع
  });
...
async function fetchConversationsFromServer() {
    const userId = localStorage.getItem('userId');
    if (!userId) return [];
    const nickname = localStorage.getItem('codeai_nickname');
    if (!nickname) {
        console.log("ℹ️ Not registered, skipping conversations fetch");
        return [];
    }
    try {
        const response = await fetchWithAuth(`${API_BASE_URL}/api/user?userId=${userId}`);
        ...
        return data.conversations.map(conv => ({
            ...
            checkpoints: data.conversation?.checkpoints || []
        }));
...
/*async function fetchConversationFromServer(convId) {
    ...
    try {
        const response = await fetchWithAuth(`${API_BASE_URL}/api/conversation?convId=${convId}&userId=${userId}`);
        if (!response.ok) throw new Error('Failed to fetch conversation');
        
        const data = await response.json();
        
        // ✅ تحويل الرسائل مع تصفية الفارغة وإزالة التكرار
        const rawMessages = (data.messages || [])
            .filter(msg => msg.content && msg.content.trim().length > 0);
        
        // ✅ إزالة الرسائل المكررة (نفس المحتوى ونفس الدور)
        const uniqueMessages = [];
        const seen = new Set();
        
        for (const msg of rawMessages) {
            // إنشاء مفتاح فريد للرسالة
            const key = `${msg.role}_${msg.content.substring(0, 100)}_${msg.created_at}`;
            if (!seen.has(key)) {
                seen.add(key);
                uniqueMessages.push(msg);
            } else {
                console.warn(`⚠️ Duplicate message detected and removed:`, msg.role, msg.content.substring(0, 50));
            }
        }
        
        const messages = uniqueMessages.map(msg => ({
            role: msg.role,
            text: msg.content || '',
            model: msg.model || null,
            thought: msg.thought || null,
            tasks: msg.tasks || null,
            filesSnapshot: msg.files_snapshot || null,
...
async function fetchConversationFromServer(convId) {
    ...
    try {
        ...
        // 1. تحويل جميع الرسائل إلى كائنات موحدة مع timestamp
        const allMessages = (data.messages || []).map(msg => ({
            id: Number(msg.id),                     // تأكد من أنه رقم
            role: msg.role,
            content: msg.content || '',
            model: msg.model,
            thought: msg.thought,
            tasks: msg.tasks,
            files_snapshot: msg.files_snapshot,
            parent_id: msg.parent_id ? Number(msg.parent_id) : null,  // تحويل إلى رقم
            reply_index: msg.reply_index || 0,
            is_latest: msg.is_latest === true,
            timestamp: new Date(msg.created_at).getTime()
        })).sort((a, b) => a.timestamp - b.timestamp);
        
        // 2. بناء قائمة رسائل المستخدم وخريطة للمعرفات
        const userMessages = [];
        const userMsgMap = new Map();  // id -> user message object
        ...
        for (const msg of allMessages) {
            if (msg.role === 'user') {
                ...
                userMsgMap.set(msg.id, userMsg);
            }
        }
        
        // 3. ربط الردود (AI) بآخر رسالة مستخدم (بغض النظر عن parent_id، كحل مضمون)
        // هذه هي الإستراتيجية القوية: ربط كل رد بأحدث رسالة مستخدم قبله زمنياً
        let lastUserMsg = null;
        for (const msg of allMessages) {
            if (msg.role === 'user') {
                lastUserMsg = userMsgMap.get(msg.id);
            } else if (msg.role === 'ai' && lastUserMsg) {
                // إضافة الرد إلى آخر رسالة مستخدم
                lastUserMsg.replies.push({
                    id: msg.id,
                    text: msg.content,
                    model: msg.model,
                    thought: msg.thought,
                    tasks: msg.tasks,
                    filesSnapshot: msg.files_snapshot,
                    timestamp: msg.timestamp,
                    reply_index: msg.reply_index,
                    is_latest: msg.is_latest
                });
            }
        }
        
        // 4. فرز الردود لكل مستخدم وتحديد currentReplyIndex
        for (const userMsg of userMessages) {
            userMsg.replies.sort((a, b) => {
                if (a.reply_index !== undefined && b.reply_index !== undefined)
                    return a.reply_index - b.reply_index;
                return a.timestamp - b.timestamp;
            });
            // تعيين آخر رد كـ current
            if (userMsg.replies.length > 0) {
                userMsg.currentReplyIndex = userMsg.replies.length - 1;
                // التأكد من أن آخر رد هو الأحدث
                userMsg.replies[userMsg.replies.length - 1].is_latest = true;
            } else {
                userMsg.currentReplyIndex = -1;
            }
        }
        
        // 5. تحويل الملفات (نفس الكود القديم)
        const files = (data.files || []).map(file => ({
            name: file.name,
            content: file.content || '',
            url: file.url || null,
...
/*async function saveConversationToServer(conversation) {
  const lastMsg = conversation.messages[conversation.messages.length - 1];
    ...
    try {
        const response = await fetchWithAuth(`${API_BASE_URL}/api/conversation`, {
            method: 'POST',
...
async function saveConversationToServer(conversation) {
  if (conversation.id && conversation.id.startsWith('shared_')) {
    console.log("⏭️ Skipping save for shared conversation");
    return false;
}
    const userId = localStorage.getItem('userId');
    if (!userId || !conversation) return false;

    // تحويل الهيكل المتفرع إلى رسائل مسطحة
    const flatMessages = [];
    for (const userMsg of conversation.messages) {
        flatMessages.push({
            role: 'user',
        ...
        for (let i = 0; i < userMsg.replies.length; i++) {
            const reply = userMsg.replies[i];
            flatMessages.push({
                role: 'ai',
                content: reply.text,
                model: reply.model,
                ...
                tasks: reply.tasks,
                files_snapshot: reply.filesSnapshot,
                parent_id: userMsg.id,
                reply_index: i,
                is_latest: (i === userMsg.replies.length - 1),
                id: reply.id,
    ...
    try {
        const res = await fetchWithAuth(`${API_BASE_URL}/api/conversation`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                userId,
        ...
        if (!res.ok) {
            ...
            console.error("❌ Save conversation response:", res.status, data);
            return false;
        }
        return true;
    } catch (error) {
...
// متغيرات عامة للسحب والقوائم
let startX = 0;
let menuStartY = 0;
let isDragging = false;
let isVerticalScroll = false;
let isGestureDetermined = false;
let sideMenuWidth = 320;

// متغيرات سحب صفحة الكود
let codeStartX = 0;
let codeStartY = 0;
let isDraggingCodezone = false;
let hasMoved = false;
let codeDragX = 0;
let isCodeGestureDetermined = false;
let isCodeVerticalScroll = false;
const CODEZONE_WIDTH = window.innerWidth;

// عناصر واجهة مهمة
let sendBtn = null;
let micIconTpl = null;
let sendIconTpl = null;
let stopIconTpl = null;
let isStreamingActive = false;
let inputEl = null;
let isSending = false;
let allowMicWhenEmpty = true;

function checkInputState(inputElement) {
    if (!sendBtn) sendBtn = document.getElementById('sendBtn');
    if (!micIconTpl) micIconTpl = document.getElementById('micIcon');
    if (!sendIconTpl) sendIconTpl = document.getElementById('sendIcon');
    if (!stopIconTpl) stopIconTpl = document.getElementById('stopIcon');
    
    const hasText = inputEl?.value?.trim() !== '';
    
    // ✅ حالة الـ streaming (النموذج يرد) - الأولوية القصوى
    if (isStreamingActive) {
        sendBtn.disabled = false;
        sendBtn.classList.remove('disabled');
        sendBtn.dataset.mode = 'stop';
        if (stopIconTpl) {
            sendBtn.innerHTML = stopIconTpl.innerHTML;
        } else {
            sendBtn.innerHTML = '⬛'; // fallback
        }
        return;
    }
    
    // حالة الإرسال العادي (قبل بدء الـ streaming)
    if (isSending) {
    ...
    if (!hasText && allowMicWhenEmpty) {
        ...
        sendBtn.innerHTML = micIconTpl?.innerHTML || '🎤';
    } else if (hasText) {
        sendBtn.disabled = false;
        sendBtn.classList.remove('disabled');
        sendBtn.dataset.mode = 'send';
        sendBtn.innerHTML = sendIconTpl?.innerHTML || '📤';
    } else {
        sendBtn.disabled = true;
        sendBtn.classList.add('disabled');
    }
...
// تخزين المستمعات الأصلية
let originalTouchMoveListeners = [];

function disableAllTouchMoveListeners() {
    // نسخ المستمعات الحالية (لا يمكن الوصول إليها مباشرة، لذا نستخدم طريقة مختلفة)
    // سنقوم بإعادة تعريف addEventListener لمنع إضافة مستمعات جديدة،
    // واستبدال document.addEventListener بـ "وهمي" للمستمعات الموجودة
    
    // حفظ الدالة الأصلية
    const originalAddEventListener = Document.prototype.addEventListener;
    
    // تخزين المستمعات التي سنضيفها يدويًا لاحقًا
    Document.prototype.addEventListener = function(type, listener, options) {
        if (type === 'touchmove') {
            console.log('🚫 منع إضافة مستمع touchmove جديد');
            return;
        }
        return originalAddEventListener.call(this, type, listener, options);
    };
    
    // إزالة المستمعات الموجودة (نحتاج لتخزين مراجعها أولاً، وهذا صعب)
    // لذا نستخدم طريقة أبسط: إضافة مستمع ذو أولوية عالية يمنع التنفيذ
    const blocker = function(e) {
        const authScreen = document.getElementById('welcomeAuthScreen');
        if (authScreen && authScreen.classList.contains('open')) {
            e.stopImmediatePropagation(); // يمنع أي مستمع آخر من العمل
            console.log('🛡️ تم حظر مستمع touchmove');
        }
    };
    
    document.addEventListener('touchmove', blocker, { capture: true, passive: false });
    window._touchMoveBlocker = blocker;
    
    console.log('✅ تم تعطيل جميع مستمعات touchmove');
}

function enableAllTouchMoveListeners() {
    // استعادة addEventListener الأصلي
    // هذا صعب لأننا لا نستطيع استعادة prototype بسهولة، لكن يمكننا إعادة تحميل الصفحة أو تجاهل
    if (window._touchMoveBlocker) {
        document.removeEventListener('touchmove', window._touchMoveBlocker, { capture: true });
        window._touchMoveBlocker = null;
    }
    console.log('✅ تم استعادة مستمعات touchmove');
}
...
function showSigninScreen() {
    ...
    // إخفاء شاشة التسجيل نهائياً
    if (authScreen) {
        authScreen.classList.remove('open');
        authScreen.style.display = 'none';
    }

    // إخفاء أي عناصر متطفلة
    if (modelSelector) modelSelector.style.display = 'none';
    if (codezone) codezone.classList.remove('open');
    if (menuPanel) menuPanel.classList.remove('open');

    // إظهار صفحة تسجيل الدخول وفرض قيم visibility
    if (signinScreen) {
        ...
        // ✅ إجبار إظهار جميع العناصر الداخلية دفعة واحدة
        const allInside = signinScreen.querySelectorAll('*');
        ...
        // إعادة التأكيد على الحقول والأزرار المهمة
        const importantSelectors = [
            '.register-card.transparent-card',
            '.welcome-auth-form',
            '.input-field',
            '.register-title',
            '.register-subtitle',
        ...
        importantSelectors.forEach(sel => {
            const el = signinScreen.querySelector(sel);
            if (el) {
                el.style.setProperty('display', '', 'important');
                el.style.setProperty('visibility', 'visible', 'important');
                el.style.setProperty('opacity', '1', 'important');
    ...
    // في دالة showSigninScreen، أضف بعد setTimeout
setTimeout(() => {
    document.getElementById('changeEmailBtn')?.addEventListener('click', openChangeEmail);
...
function showRegisterScreen() {
    ...
    if (signinScreen) {
        ...
        signinScreen.style.visibility = 'hidden';
        signinScreen.style.opacity = '0';
        signinScreen.style.zIndex = '';
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ✅ استعادة الجلسة بعد العودة من رابط تأكيد البريد
(async function restoreSessionAfterEmailConfirmation() {
    await initAuth(); // تأكد من وجود هذه الدالة (تستمع لتغييرات Auth)
    
    const { data: { session } } = await supabaseClient.auth.getSession();
    
    if (session) {
        // ✅ المستخدم لديه جلسة صالحة (بعد تأكيد البريد)
        const isConfirmed = session.user.email_confirmed_at || session.user.confirmed_at;
        
        if (isConfirmed) {
            // البريد مؤكد - إخفاء شاشة التسجيل تماماً
            const authScreen = document.getElementById('welcomeAuthScreen');
            const signinScreen = document.getElementById('signinScreen');
            if (authScreen) authScreen.style.display = 'none';
            if (signinScreen) signinScreen.style.display = 'none';
            
            // تحميل المحادثات مباشرة
            await loadConversations();
            await loadUserNickname();
            updateAccountDisplay();
            
            console.log("✅ Email confirmed, app loaded directly.");
        }
    }
})();

async function loadSharedMessage(shareCode) {
    try {
        const response = await fetch(`${API_BASE_URL}/api/share/${shareCode}`);
        const data = await response.json();

        if (!response.ok || data.error) {
            showToast(data.error || 'Message not found or expired', 'error');
            return;
        }

        // ✅ تعيين علامة المشاركة
        isSharedView = true;

        // ✅ تجهيز الملفات
// داخل loadSharedMessage، بعد const data = await response.json();

// ✅ تجهيز الملفات
if (data.files_snapshot && data.files_snapshot.length) {
    projectFiles = data.files_snapshot.map(file => ({
        name: file.name,
        content: file.content || '',
        url: file.url || null,
        previewUrl: file.preview_url || null,
    ...
    projectFiles = [{ name: 'index.html', content: '// No shared files' }];
}

// ✅ تحديث واجهة التبويبات والمحرر
renderTabs();

// ✅ عرض الملف النشط بشكل صحيح (يدعم الصور والمستندات)
const currentFile = projectFiles[activeFileIndex];
if (currentFile) {
    if (isImageFile(currentFile.name)) {
            ...
            messages: [
                {
                    ...
                    replies: [{
                        id: 'shared_reply_' + shareCode,
                        text: data.message_content,
                        model: data.model_name,
                        filesSnapshot: data.files_snapshot || [],
                        timestamp: new Date(data.created_at).getTime()
                    }],
                    currentReplyIndex: 0
                }
        ...
        convs = [tempConv];
        activeId = tempConv.id;
        renderMessages();

        // ✅ إخفاء العناصر غير الضرورية (باستثناء زر الكود وزر القائمة الجانبية سنقوم بتحويله)
        ...
        elementsToHide.forEach(id => {
            const el = typeof id === 'string' ? document.getElementById(id) : document.querySelector(id);
            if (el) el.style.display = 'none';
        });

        // ✅ إخفاء شريط الإدخال (class)
        const inputBar = document.querySelector('.input-bar');
        if (inputBar) inputBar.style.display = 'none';

        // ✅ تحويل زر القائمة الجانبية إلى زر "جرب كوداي"
        const menuBtn = document.getElementById('menuBtn');
        if (menuBtn) {
            // تغيير النص والأيقونة
            menuBtn.innerHTML = '<span>Try Codeai!</span>';
            menuBtn.style.background = '#089cff';
            menuBtn.style.color = '#fff';
            menuBtn.style.borderRadius = '30px';
            menuBtn.style.padding = '8px 16px';
            menuBtn.style.width = 'auto';
            menuBtn.style.gap = '6px';
            // إزالة وظيفة فتح القائمة الجانبية
            const newMenuBtn = menuBtn.cloneNode(true);
            menuBtn.parentNode.replaceChild(newMenuBtn, menuBtn);
            newMenuBtn.onclick = () => {
                // العودة إلى الصفحة الرئيسية (إزالة رابط المشاركة)
                window.location.href = window.location.origin;
            };
        }

        // ✅ التأكد من أن زر فتح صفحة الكود يعمل (لم نخفيه)
        const codeToggleBtn = document.getElementById('codeToggleBtn');
        if (codeToggleBtn) codeToggleBtn.style.display = 'flex';

        // ✅ إخفاء لوحة القائمة الجانبية بالكامل ومنع السحب عليها
        const menuPanel = document.getElementById('menuPanel');
        if (menuPanel) menuPanel.style.display = 'none';

        // ✅ إضافة كلاس CSS لإخفاء الأزرار الديناميكية في الرسائل
        document.body.classList.add('shared-view');

        // ✅ إضافة footer
        const sharedFooter = document.createElement('div');
        sharedFooter.className = 'shared-message-footer';
        sharedFooter.innerHTML = `
            <div style="text-align: center; padding: 16px; border-top: 1px solid var(--border-color); margin-top: 20px; font-size: 12px; opacity: 0.6;">
                🔗 Shared from Codeai · ${new Date(data.created_at).toLocaleString()}
            </div>
        `;
        const messagesContainer = document.getElementById('messages');
        if (messagesContainer) messagesContainer.appendChild(sharedFooter);

    } catch (err) {
        console.error('❌ loadSharedMessage error:', err);
        showToast('Failed to load shared message', 'error');
    }
}
  // --- التحقق من الأصول (Assets Check) ---
// ملاحظة: نستخدم مسارات نسبية الآن لأن index.html و assets في نفس المستوى داخل client

// إنشاء حاوية bottomSection الموحدة لحركة الرفع


// نقل شريط الاقتراحات إلى الحاوية الجديدة
const suggestionBar = document.getElementById('suggestionBar');


// نقل صندوق الإدخال إلى الحاوية الجديدة
const inputBar = document.querySelector('.input-bar');


// التأكد من أن لوحة التحكم موجودة (إنشاؤها في HTML سابقاً)
const controlPanel = document.getElementById('controlPanel');


// تعديل padding-bottom للرسائل لإفساح المجال للحاوية
const messagesEl = document.getElementById('messages');

// إخفاء شريط الأخطاء المنفصل إذا كان موجوداً
const inputAttachmentsEl = document.getElementById('inputAttachments');
if (inputAttachmentsEl) {
    inputAttachmentsEl.style.display = 'none';
}
...
function initAvatarVideo() {
    ...
    const theme = localStorage.getItem('codeai_theme') || 'dark';
    const videoSrc = theme === 'light' ? 'assets/avatar-vid-light.mp4' : 'assets/avatar-vid-dark.mp4';
    
    avatarVideo.src = videoSrc;
    avatarVideo.load();
    
    avatarVideo.onended = null;
    avatarVideo.onloadedmetadata = null;
    
    avatarVideo.onloadedmetadata = function() {
        if (videoPlayed) {
            avatarVideo.currentTime = avatarVideo.duration - 0.1;
            avatarVideo.pause();
        }
        avatarVideo.onloadedmetadata = null;
    };
    
    // تشغيل الفيديو مباشرة بدون تأخير
    if (!videoPlayed) {
...
// تحديث موضع الفيديو والنص حسب اللغة (بدون إخفاء)
// استبدال الدالة الحالية بـ:
function updateWelcomeContainerDirection() {
    const container = document.getElementById('dynamicWelcomeContainer');
    const lang = localStorage.getItem('codeai_lang') || 'en';
    
    if (container) {
        // استخدام flex-direction للتحكم في الترتيب
        // في اللغة العربية: الفيديو على اليمين (يظهر أولاً) والنص على اليسار
        // في اللغة الإنجليزية: الفيديو على اليسار والنص على اليمين
        container.style.flexDirection = (lang === 'ar') ? 'row-reverse' : 'row';
        
        const welcomeText = document.getElementById('dynamicWelcomeText');
        if (welcomeText) {
            welcomeText.style.textAlign = (lang === 'ar') ? 'right' : 'left';
            welcomeText.style.direction = (lang === 'ar') ? 'rtl' : 'ltr';
        }
    }
}

// تحديث دالة hideSplashAndShowWelcome
// داخل hideSplashAndShowWelcome، بعد إخفاء السبلاش

// في بداية app3.js، أضف هذه الأنماط للجداول والخرائط
function injectTableStyles() {
    if (document.getElementById('markdown-table-styles')) return;
    
    const style = document.createElement('style');
    style.id = 'markdown-table-styles';
    style.textContent = `
        /* أنماط الجداول في المحادثة */
        .ai-content table {
        ...
        .ai-content thead {
            ...
            border-bottom: 2px solid var(--border-color, #555) !important;
        }
        ...
        .ai-content th {
            ...
            border-right: 1px solid var(--border-color, #444) !important;
            border-bottom: 2px solid var(--border-color, #555) !important;
        ...
        .ai-content td {
            ...
            color: var(--text-secondary, #ddd) !important;
        }
        ...
        /* أنماط خاصة للمخططات والخرائط (ASCII art / code blocks) */
        .ai-content pre {
        ...
        /* تحسين عرض الخرائط النصية (ASCII diagrams) */
        .ai-content pre:has(code.language-text),
        .ai-content pre:has(code:not([class])) {
            font-family: 'Consolas', 'Monaco', 'Courier New', monospace !important;
            line-height: 1.2 !important;
        }
        
        /* دعم الوضع الفاتح */
        [data-theme="light"] .ai-content table {
            border-color: #ddd !important;
        }
        
        [data-theme="light"] .ai-content thead {
            background: #f5f5f5 !important;
            border-bottom-color: #ccc !important;
        }
        
        [data-theme="light"] .ai-content th {
            color: #222 !important;
            border-right-color: #ddd !important;
            border-bottom-color: #ccc !important;
        }
        
        [data-theme="light"] .ai-content td {
            border-right-color: #ddd !important;
            border-bottom-color: #eee !important;
            color: #444 !important;
        }
        
        [data-theme="light"] .ai-content tbody tr:hover {
            background: rgba(0,0,0,0.03) !important;
        }
        
        [data-theme="light"] .ai-content pre {
            background: #f8f8f8 !important;
            border-color: #ddd !important;
            color: #333 !important;
        }
        
        /* دعم RTL للجداول العربية */
        .ai-content[dir="rtl"] th,
        .ai-content[dir="rtl"] td {
            text-align: right !important;
        }
        
        .ai-content[dir="rtl"] th {
            border-right: none !important;
            border-left: 1px solid var(--border-color, #444) !important;
        }
        
        .ai-content[dir="rtl"] th:last-child {
        ...
        .ai-content[dir="rtl"] td:last-child {
            border-left: none !important;
        }
        
        /* مزامنة الثيم مع المتغيرات */
        :root {
            --border-color: #444;
            --bg-secondary: #2A2A2A;
...
// أنماط الروابط في المحادثة
function injectLinkStyles() {
    if (document.getElementById('markdown-link-styles')) return;
    
    const style = document.createElement('style');
    style.id = 'markdown-link-styles';
    style.textContent = `
        /* ✅ تنسيق الروابط في المحادثة - خط Fredoka ولون أزرق فاتح */
        .ai-content a,
        .msg.ai a {
            font-family: 'Fredoka', 'Segoe UI', 'Tajawal', sans-serif !important;
        ...
        .ai-content a:hover,
        .msg.ai a:hover {
            color: #90CAF9 !important;  /* أزرق أفتح عند التحويم */
            border-bottom-color: #90CAF9 !important;
        }
        
        .ai-content a:visited,
        .msg.ai a:visited {
            color: #7986CB !important;  /* بنفسجي فاتح للروابط المزارة */
        }
        ...
        /* ✅ تنسيق الروابط داخل الكود (لا نغيرها) */
        .ai-content pre a,
        .ai-content code a,
        .msg.ai pre a,
        .msg.ai code a {
        ...
        /* ✅ دعم الوضع الفاتح */
        [data-theme="light"] .ai-content a,
        [data-theme="light"] .msg.ai a {
            color: #1976D2 !important;  /* أزرق أغمق للوضع الفاتح */
            border-bottom-color: rgba(25, 118, 210, 0.3) !important;
        }
        
        [data-theme="light"] .ai-content a:hover,
        [data-theme="light"] .msg.ai a:hover {
            color: #1565C0 !important;
            border-bottom-color: #1565C0 !important;
        }
        
        [data-theme="light"] .ai-content a:visited,
        [data-theme="light"] .msg.ai a:visited {
            color: #5C6BC0 !important;
        }
        
        /* ✅ دعم RTL للروابط العربية */
        .ai-content[dir="rtl"] a,
        .msg.ai[dir="rtl"] a {
            font-family: 'Fredoka', 'Tajawal', sans-serif !important;
        }
        
        /* ✅ أيقونة رابط خارجي (اختياري) */
        .ai-content a[href^="http"]:not([href*="codeai"]):after,
        .msg.ai a[href^="http"]:not([href*="codeai"]):after {
            content: " ↗";
            font-size: 0.8em;
            opacity: 0.7;
        ...
        /* ✅ تنسيق الروابط داخل رسائل المستخدم */
        .msg.user a {
        ...
        [data-theme="light"] .msg.user a {
            color: #1976D2 !important;
        }
        
        /* ✅ تحميل خط Fredoka إذا لم يكن محملاً */
    ...
    if (document.getElementById('citation-link-styles')) return;
    
    const style = document.createElement('style');
    style.id = 'citation-link-styles';
    style.textContent = `
        /* الاستشهادات الرقمية [1]، [2]، [3] */
        .ai-content sup {
            font-size: 0.75em;
...
.ai-content a[href^="#ref-"],
.ai-content a.citation-link,
.ai-content .citation-ref {
    ...
    transition: all 0.2s;
        ...
        .ai-content a.citation-link:hover,
        .ai-content .citation-ref:hover {
            background: #2a2a2a
            border-color: none;
            color: #fff;
            text-decoration: none;
        ...
        /* دعم الوضع الفاتح */
        [data-theme="light"] .ai-content a.citation-link,
        [data-theme="light"] .ai-content .citation-ref {
            background: #2a2a2a
            color: #fff;
          
        }
        
        [data-theme="light"] .ai-content a.citation-link:hover,
        [data-theme="light"] .ai-content .citation-ref:hover {
            background: #2a2a2a
...
function injectSharedViewStyles() {
    if (document.getElementById('shared-view-styles')) return;
    const style = document.createElement('style');
    style.id = 'shared-view-styles';
    style.textContent = `
        body.shared-view .msg-actions,
        body.shared-view .action-btn,
        body.shared-view .more-actions-btn,
        body.shared-view .share-btn,
        body.shared-view .model-label-tag,
        body.shared-view .restore-checkpoint-btn,
        body.shared-view .thought-toggle-btn {
            display: none !important;
        }
        body.shared-view .msg.ai {
            pointer-events: none;
        }
    `;
    document.head.appendChild(style);
...
injectSharedViewStyles();
// تحديث اتجاه الفيديو والنص حسب اللغة
...
window.setLanguage = function(lang) {
    ...
    updateWelcomeContainerDirection();
};
...
function processCitations(htmlContent, searchResults) {
    if (!searchResults || !searchResults.length) return htmlContent;
    
    // استبدال [1]، [2]... بروابط تشير إلى المصادر
    return htmlContent.replace(/\[(\d+)\]/g, (match, num) => {
        const result = searchResults.find(r => r.index == num);
        if (result) {
            return `<a href="${result.url}" target="_blank" class="citation-link" title="${result.title}">${num}</a>`;
        }
        return match; // يبقى كما هو إذا لم يجد مصدر
    });
}


// ==========================================
// دوال تحسين شاشة السبلاش والتسجيل
// ==========================================

// 1. أنميشن تلاشي خلفية السبلاش
// ==========================================
// دوال تحسين شاشة السبلاش والتسجيل - النسخة النهائية
// ==========================================
// ==========================================
// دوال تحسين شاشة السبلاش والتسجيل - النسخة المصححة
// ==========================================
// ==========================================
// دوال تحسين شاشة السبلاش والتسجيل - النسخة النهائية المصححة
// ==========================================
// ==========================================
// دوال تحسين شاشة السبلاش والتسجيل - النسخة المصححة
// ==========================================
...
// ==========================================
// ==========================================
// SplashManager - مدير موحد لشاشة السبلاش
// ==========================================
const SplashManager = {
    video: null,
    splashScreen: null,
    authScreen: null,
    transferredContainer: null,
    transitionInProgress: false,

    init() {
        this.video = document.getElementById('splashVideoContainer');
        this.splashScreen = document.getElementById('splashScreen');
        this.authScreen = document.getElementById('welcomeAuthScreen');
        this.transferredContainer = document.getElementById('transferredVideoContainer');
    },

    start() {
        this.init();
        const splash = this.splashScreen;
        const videoEl = document.getElementById('splashVideo');
        if (!splash || !videoEl) return;

        splash.classList.remove('hidden');
        document.body.style.overflow = 'hidden';
        splash.style.visibility = 'visible';
        const isLight = document.documentElement.getAttribute('data-theme') === 'light';
        videoEl.src = isLight ? 'assets/splash-light.mp4' : 'assets/splash-dark.mp4';
        videoEl.load();
        ...
        videoEl.onended = () => this.onVideoEnded();

        // بديل آمن بعد 8 ثوانٍ
        setTimeout(() => {
            if (!welcomeMessagesReady && splash && !splash.classList.contains('hidden')) {
                this.forceFallbackMessages();
                this.onVideoEnded();
            }
        }, 8000);
    },

    // تعديل دالة onVideoEnded في SplashManager
onVideoEnded() {
    if (this.transitionInProgress) return;
    

    // ✅ التحقق من حالة التسجيل الحقيقية
    (async () => {
        const isUserRegistered = await checkUserRegistration(); // دالة checkUserRegistration الحقيقية
        const hasActiveConv = activeId && convs.find(c => c.id === activeId);
        
        if (hasActiveConv || isUserRegistered) {
            // إذا كان المستخدم مسجلاً أو لديه محادثة نشطة، نخفي السبلاش مباشرة
            this.hideSplash();
        } else {
            // غير مسجل وليس لديه محادثة → نعرض شاشة التسجيل
            this.transitionToAuthScreen();
        }

        // جلب الرسائل الترحيبية في الخلفية (لا تؤثر على القرار)
        if (!welcomeMessagesReady) {
            fetchWelcomeMessages().catch(e => console.warn("Welcome messages fetch failed:", e));
        }
    })();
},
    ...
    transitionToAuthScreen() {
      if (this.splashScreen.classList.contains('hidden')) {
            console.warn("⚠️ Splash already hidden, skipping duplicate call.");
            return;
        }
    if (this.transitionInProgress || !this.video || !this.authScreen || !this.transferredContainer) return;
    this.transitionInProgress = true;
    console.log("🎬 بدء الانتقال...");
    disableAllTouchMoveListeners();
    console.log("🟢 transitionToAuthScreen STARTED at", performance.now());
    console.log("🟢 splashScreen classes:", this.splashScreen.className);
    console.log("🟢 splashScreen opacity:", getComputedStyle(this.splashScreen).opacity);
    console.log("🟢 splashScreen visibility:", getComputedStyle(this.splashScreen).visibility);
    console.log("🟢 video opacity:", getComputedStyle(this.video).opacity);

    // 1. استخدم requestAnimationFrame لضمان رسم الإطار الحالي
    requestAnimationFrame(() => {
        requestAnimationFrame(() => {
            // الآن نضيف hidden لبدء transition
            this.splashScreen.classList.add('hidden');
        });
    });

    // 2. تجهيز شاشة التسجيل
    this.authScreen.style.display = 'block';
    this.authScreen.style.opacity = '0';
    this.authScreen.style.overflowY = 'auto';
    this.authScreen.style.webkitOverflowScrolling = 'touch';
    this.transferredContainer.style.display = 'flex';

    // 3. بعد مدة transition (500ms)
    setTimeout(() => {
        // إضافة visibility: hidden يدوياً بعد انتهاء transition
        this.splashScreen.style.visibility = 'hidden';
        
        // نقل الفيديو
        this.transferredContainer.appendChild(this.video);
        
        // إظهار شاشة التسجيل
        this.authScreen.style.opacity = '1';
        this.authScreen.classList.add('open');
        // تهيئة شرائح التسجيل بعد أن تصبح الشاشة مفتوحة
setTimeout(() => {
    if (!initOnboardingSlides()) {
        setTimeout(initOnboardingSlides, 200);
    }
    // ✅ استدعاء goToSlide(0) بعد التأكد من تهيئة الشرائح
    setTimeout(() => {
        if (window.goToSlide) {
            window.goToSlide(0);
        }
        ...
        // إظهار الفيديو في المكان الجديد
        setTimeout(() => {
            this.video.classList.add('visible');
            this.transitionInProgress = false;
            console.log("✅ تم الانتقال.");
        }, 50);
        
        const content = this.authScreen.querySelector('.welcome-auth-content');
        if (content) {
            content.style.pointerEvents = 'auto';
            content.style.touchAction = 'pan-y';
        }
        initScrollTrigger();
        console.log("🔴 setTimeout 500ms EXECUTED at", performance.now());
    console.log("🔴 splashScreen opacity:", getComputedStyle(this.splashScreen).opacity);
    console.log("🔴 splashScreen visibility:", getComputedStyle(this.splashScreen).visibility);
    }, 500);
    setTimeout(() => {
    ...
    forceFallbackMessages() {
        const lang = localStorage.getItem('codeai_lang') || 'en';
        welcomeMessages = lang === 'ar'
            ? ["✨ كيف يمكنني مساعدتك اليوم؟", "🚀 جاهز لبناء شيء رائع!"]
            : ["✨ How can I help you today?", "🚀 Ready to build something awesome!"];
        welcomeMessagesReady = true;
    }
};
...
// 1. إخفاء الخلفية فقط (الفيديو يبقى)


// 2. نقل الفيديو إلى صفحة التسجيل
// 2. نقل الفيديو إلى صفحة التسجيل (نفس الفيديو ينتقل)
// 2. نقل الفيديو إلى صفحة التسجيل مع أنميشن انتقال
// 2. نقل الفيديو إلى صفحة التسجيل مع أنميشن انتقال
// 2. نقل الفيديو إلى صفحة التسجيل - النسخة الذكية
// 2. نقل الفيديو إلى صفحة التسجيل - النسخة المبسطة
// دالة موحدة للانتقال إلى شاشة التسجيل


// استبدل استدعاء moveVideoToAuthScreen بـ transitionToAuthScreen في جميع الأماكن
// (مثلاً داخل hideSplashAfterMessagesReady و checkUserRegistration)

// 3. مراقبة السكرول - النسخة المصححة
function initScrollTrigger() {
    ...
    if (!authScreen) {
        console.error("❌ welcomeAuthScreen not found!");
        return;
    }
    
    
    
    console.log("✅ Scroll trigger initialized");
    ...
    // دالة فحص التمرير
    const checkScroll = () => {
        // العنصر الذي يتم التمرير عليه هو authScreen نفسه
        const scrollTop = authScreen.scrollTop;
        console.log("📜 Scroll position:", scrollTop);
        
        // عند التمرير لأسفل 200px - إظهار الفيديو الثاني
        
            console.log("🎬 Second video should appear now!");
            secondVideo.classList.add('visible');
            welcomeVideo.play().catch(e => console.log("Video play error:", e));
            videoStarted = true;
        ...
        // عند التمرير لأسفل 400px - إظهار نموذج التسجيل
        
            console.log("📝 Registration form should appear now!");
            registerForm.classList.add('visible');
            formShown = true;
        
    };
    
    // تأكد من أن authScreen قابل للتمرير
    
    
    // إزالة أي مستمعين سابقين
    authScreen.removeEventListener('scroll', checkScroll);
    authScreen.removeEventListener('touchmove', checkScroll);
    
    // إضافة المستمعين
    authScreen.addEventListener('scroll', checkScroll);
    authScreen.addEventListener('touchmove', checkScroll);
    
    // فحص أولي بعد 300ms
    setTimeout(() => {
        console.log("🔍 Initial check - authScreen.scrollTop =", authScreen.scrollTop);
        console.log("🔍 authScreen.scrollHeight =", authScreen.scrollHeight, "clientHeight =", authScreen.clientHeight);
        if (authScreen.scrollHeight <= authScreen.clientHeight) {
            console.log("⚠️ Content is not tall enough! Add more height.");
        }
        checkScroll();
    }, 300);
}
...
document.getElementById('goToSigninBtn')?.addEventListener('click', showSigninScreen);
document.getElementById('goToRegisterBtn')?.addEventListener('click', showRegisterScreen);
document.getElementById('googleSignInBtn')?.addEventListener('click', signInWithGoogle);

// الحصول على فيديو شاشة التسجيل (الثاني) - مع التأكد من تحميله
function initsecondVideoLoop() {
    const welcomeVideo = document.getElementById('welcomeVideo');
    if (welcomeVideo) {
        
        
        welcomeVideo.removeEventListener('ended', welcomeVideoLoopHandler);
        welcomeVideo.removeEventListener('timeupdate', welcomeVideoTimeHandler);
        
        function welcomeVideoLoopHandler() {
            
            welcomeVideo.currentTime = 1.7;
            welcomeVideo.play().catch(e => console.log("Replay error:", e));
        }
        
        function welcomeVideoTimeHandler() {
            if (welcomeVideo.duration - welcomeVideo.currentTime <= 0.1) {
        ...
        welcomeVideo.addEventListener('ended', welcomeVideoLoopHandler);
        welcomeVideo.addEventListener('timeupdate', welcomeVideoTimeHandler);
        
        if (welcomeVideo.paused && welcomeVideo.currentTime === 0) {
            welcomeVideo.currentTime = 1.7;
            welcomeVideo.play().catch(e => console.log("Initial play error:", e));
        }
    } else {
        
        setTimeout(initsecondVideoLoop, 500);
    }
}
setTimeout(initsecondVideoLoop, 1500);

setTimeout(initsecondVideoLoop, 1000);

hideSplashAfterMessagesReady = async function() {
    const hasActiveConversation = activeId && convs.find(c => c.id === activeId);
    const isUserRegistered = await checkUserRegistration(); // التحقق من التسجيل
    
    if (hasActiveConversation || isUserRegistered) {
        console.log("📂 User registered or active conversation, hiding splash completely");
        const splash = document.getElementById('splashScreen');
        if (splash) splash.classList.add('hidden');
        document.body.style.overflow = '';
        document.body.style.touchAction = '';
        splashHiddenDueToMessages = true;
        return;
    }
    
    splashHiddenDueToMessages = true;
    console.log("🎬 No active conversation and not registered - showing auth screen");
    setTimeout(() => {
        SplashManager.transitionToAuthScreen();
    }, 300);
};

// 4. تعديل دالة hideSplashAfterMessagesReady
const originalHideSplashAfterMessagesReady = hideSplashAfterMessagesReady;


// 5. تعديل دالة checkUserRegistration
...
// 2. نقل الفيديو إلى صفحة التسجيل (في المكان الصحيح)
...
// 2. إخفاء السبلاش بالكامل (عند وصول الرسائل)


// 3. نقل الفيديو إلى الأعلى (يبقى مربعاً)


// 4. تعديل دالة hideSplashAfterMessagesReady


// 5. دالة جديدة لإخفاء الفيديو عند وصول أول رسالة


// 6. تعديل دالة sendMessage - عند إرسال أول رسالة، نخفي الفيديو

// 1. أنميشن تلاشي خلفية السبلاش


// 2. نقل نفس الفيديو إلى صفحة التسجيل (وليس نسخة جديدة)


// 3. مراقبة السكرول لتشغيل الفيديو الثاني وإظهار النموذج


// 4. تعديل دالة hideSplashAfterMessagesReady


// 5. تعديل دالة initSplashScreen


// 2. نقل نفس الفيديو إلى الأعلى (وليس فيديو جديد)


// 3. مراقبة السكرول لتشغيل الفيديو الثاني
...
// 6. تعديل دالة initSplashScreen - إضافة تأثير التلاشي
// ابحث عن الدالة واستبدلها بهذه النسخة أو أضف هذه التعديلات

// تأكد من أن دالة initSplashScreen موجودة بهذا الشكل:
...
function hideSplashAndShowWelcome() {
    
    
    // إذا كانت الرسائل جاهزة، قم بإخفاء السبلاش
    if (welcomeMessagesReady) {
        hideSplashAfterMessagesReady();
    }
    // إذا لم تكن الرسائل جاهزة، سننتظر حتى تجهز في fetchWelcomeMessages
}
...
function hideWelcomeElements() {
    ...
    // إيقاف أي مؤقتات كتابة
    if (welcomeTypingInterval) {
        clearInterval(welcomeTypingInterval);
        welcomeTypingInterval = null;
    }
    if (welcomeDeletingInterval) {
        clearInterval(welcomeDeletingInterval);
        welcomeDeletingInterval = null;
    }
    if (welcomeTimeout) {
...
function openFeedbackBottomSheet(convId, messageId, type) {
    currentFeedbackConvId = convId;
    currentFeedbackMessageId = messageId;
    currentFeedbackType = type;
    
    const sheet = document.getElementById('feedbackBottomSheet');
    const title = document.getElementById('feedbackTitle');
    const reasonsContainer = document.getElementById('reasonsContainer');
    const commentInput = document.getElementById('feedbackComment');
    
    const lang = localStorage.getItem('codeai_lang') || 'en';
    title.textContent = type === 'like' 
        ? (lang === 'ar' ? 'لماذا أعجبك هذا الرد؟' : 'Why do you like this response?')
        : (lang === 'ar' ? 'لماذا لم يعجبك هذا الرد؟' : 'Why do you dislike this response?');
    
    const reasons = type === 'like' 
        ? ['Accurate', 'Helpful', 'Creative', 'Clear', 'Fast']
        : ['Inaccurate', 'Off-topic', 'Too long', 'Not helpful', 'Harmful'];
    
    reasonsContainer.innerHTML = reasons.map(r => `<div class="reason-chip" data-reason="${r}">${r}</div>`).join('');
    ...
    newSubmit.onclick = () => submitFeedback();
    
    const closeBtn = sheet.querySelector('.close-sheet-btn');
    const newClose = closeBtn.cloneNode(true);
    closeBtn.parentNode.replaceChild(newClose, closeBtn);
    newClose.onclick = () => sheet.classList.remove('open');
}


async function submitFeedback() {
    const sheet = document.getElementById('feedbackBottomSheet');
    const selectedReasons = Array.from(document.querySelectorAll('.reason-chip.selected')).map(el => el.getAttribute('data-reason'));
    ...
    try {
        const response = await fetchWithAuth(`${API_BASE_URL}/api/feedback`, {
            ...
            body: JSON.stringify({
                userId,
                conversationId: currentFeedbackConvId,
        ...
        if (response.ok) {
            ...
            updateFeedbackButtonState(currentFeedbackConvId, currentFeedbackMessageId, currentFeedbackType, true);
...
async function shareMessage(fullText, modelName, convId, messageId, userMsgId) {
  const userId = localStorage.getItem('userId');
  if (!userId) {
    showToast('Please login to share', 'error');
    return;
  }

  try {
    const response = await fetchWithAuth(`${API_BASE_URL}/api/share`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        userId,
...
// تحديث عرض الإيميل في شاشة الحساب
async function updateEmailDisplay() {
    ...
    if (session?.user?.email) {
        ...
        // ✅ رسالة محسنة للمستخدمين بدون بريد
        emailSpan.textContent = 'Not set';
        emailSpan.style.opacity = '0.5';
        emailSpan.style.fontStyle = 'italic';
    }
}

// فتح نافذة تغيير الإيميل
// ==========================================
// دوال تغيير الإيميل وكلمة المرور (باستخدام المودال)
// ==========================================

// فتح مودال تغيير الإيميل
...
async function saveNewEmail() {
    ...
    try {
        ...
        if (!session) {
            // ✅ المستخدم مسجل بـ nickname فقط - ننشئ حساب جديد مع الاحتفاظ بـ userId
            const userId = localStorage.getItem('userId');
            const nickname = localStorage.getItem('codeai_nickname') || 'User';
            
            // إنشاء كلمة مرور افتراضية (يمكن تغييرها لاحقاً)
            const defaultPassword = 'User@' + Date.now().toString(36);
            
            // تسجيل حساب جديد مع نفس userId
            const { data, error: signUpError } = await supabaseClient.auth.signUp({
                ...
                options: {
                    data: { 
                        nickname: nickname,
                        legacy_userId: userId // حفظ userId القديم
                    }
                }
            });
            ...
            if (data?.user) {
                // لا نغير userId لأننا نريد الاحتفاظ بالبيانات القديمة
                // لكن نخزن الإيميل الجديد
                localStorage.setItem('codeai_email', newEmail);
            }
            
            closeChangeEmailModal();
            showToast('Account upgraded! Check your email to verify.', 'success');
            
            // إظهار كلمة المرور المؤقتة للمستخدم
            setTimeout(() => {
                showToast(`Your temporary password: ${defaultPassword}. Please change it.`, 'success');
            }, 2000);
            
        } else {
            // ✅ المستخدم مسجل ببريد - تحديث الإيميل مباشرة
        ...
        // تحديث عرض الإيميل
        setTimeout(() => updateEmailDisplay(), 2000);
        
    } catch (err) {
        if (errorDiv) {
            errorDiv.textContent = err.message || 'Failed to update email';
            errorDiv.style.display = 'block';
        }
    } finally {
        if (saveBtn) {
...
async function saveNewPassword() {
    ...
    try {
        ...
        if (!session) {
            // ✅ المستخدم ليس لديه جلسة - لا يمكن تغيير كلمة المرور
            // نقترح إضافة بريد أولاً
            closeChangePasswordModal();
            showToast('Please add an email first before setting a password', 'error');
        } else {
            // ✅ المستخدم لديه جلسة - تحديث كلمة المرور
            const { error: updateError } = await supabaseClient.auth.updateUser({ 
...
function updateAccountDisplay() {
    ...
    // داخل updateAccountDisplay()
const settingsBtnText = document.querySelector('#openSettings span[data-i18n="settings"]');
if (settingsBtnText) {
    const nickname = currentNickname || localStorage.getItem('codeai_nickname') || 'User';
    settingsBtnText.textContent = nickname;
}
...
// أزرار مودال تغيير الإيميل
document.getElementById('saveEmailBtn')?.addEventListener('click', saveNewEmail);
document.getElementById('cancelEmailBtn')?.addEventListener('click', closeChangeEmailModal);

// إغلاق مودال الإيميل عند النقر خارج الصندوق
document.getElementById('changeEmailModal')?.addEventListener('click', (e) => {
    if (e.target === e.currentTarget) closeChangeEmailModal();
});

// أزرار مودال تغيير كلمة المرور
...
// إغلاق مودال كلمة المرور عند النقر خارج الصندوق
document.getElementById('changePasswordModal')?.addEventListener('click', (e) => {
    if (e.target === e.currentTarget) closeChangePasswordModal();
});

// دعم زر Enter في حقول الإدخال
...
// ========== استبدال مستمع زر saveNicknameBtn ==========
if (saveNicknameBtn) {
    saveNicknameBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetchWithAuth(`${API_BASE_URL}/api/update-nickname`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    userId: userId,
            ...
            if (response.ok) {
                ...
                // تحديث الواجهة
                updateAccountDisplay();
                renderConversations(); // لضمان تحديث الاسم في القائمة
...
let lastErrorMessageElement = null;
...
let hasReceivedModelResponse = false;
...
// أضف هذه المتغيرات في بداية app.js (بعد المتغيرات الموجودة)
let welcomeMessages = [];
let currentWelcomeIndex = 0;
let welcomeTypingInterval = null;
let welcomeDeletingInterval = null;
let welcomeTimeout = null;
let isWelcomeTyping = false;
let currentWelcomeDisplay = "";
let welcomeTextElement = null;
let isWelcomeFetching = false;

// دالة لجلب النصوص الترحيبية من السيرفر
// أضف هذا المتغير في بداية الملف
let cachedWelcomeMessages = null; // لتخزين النصوص من السيرفر

// في بداية الملف، أضف متغير جديد
let welcomeMessagesReady = false;
let splashHiddenDueToMessages = false;
...
// 4. تعديل دالة hideSplashAfterMessagesReady لتدعم الفيديو المنتقل


// 5. تعديل دالة checkUserRegistration لتشغيل الفيديو المنقول


async function fetchWelcomeMessages() {
    const lang = localStorage.getItem('codeai_lang') || 'en';
    
    if (cachedWelcomeMessages) {
        console.log("📦 Using cached welcome messages");
        const messagesArray = cachedWelcomeMessages[lang];
        if (messagesArray && messagesArray.length >= 3) {
            welcomeMessages = messagesArray;
            welcomeMessagesReady = true;
            
            // ✅ إذا كانت الرسائل جاهزة والسplash ما زال ظاهراً، قم بإخفائه
            if (!splashHiddenDueToMessages) {
                hideSplashAfterMessagesReady();
            }
            return;
        }
    ...
    if (isWelcomeFetching) {
        console.log("⚠️ Already fetching welcome messages, skipping...");
        return;
    }
    isWelcomeFetching = true;
    
    try {
        console.log("📡 Fetching welcome messages from server for language:", lang);
        const response = await fetchWithAuth(`${API_BASE_URL}/api/welcome-messages`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (response.ok) {
            ...
            let arabicMessages = null;
            
            if (data.messages && typeof data.messages === 'object') {
                if (data.messages.en && Array.isArray(data.messages.en)) {
                    englishMessages = data.messages.en;
                }
                if (data.messages.ar && Array.isArray(data.messages.ar)) {
                    arabicMessages = data.messages.ar;
                }
            }
            
            if (englishMessages && englishMessages.length >= 3 && arabicMessages && arabicMessages.length >= 3) {
                cachedWelcomeMessages = {
                    en: englishMessages.map(msg => msg.replace(/^["'`]|["'`]$/g, '').replace(/[.,!?]$/, '').trim()),
                    ar: arabicMessages.map(msg => msg.replace(/^["'`]|["'`]$/g, '').replace(/[.,!?]$/, '').trim())
                };
                
                welcomeMessages = cachedWelcomeMessages[lang];
                welcomeMessagesReady = true;
                console.log("🎉 Cached bilingual messages:", cachedWelcomeMessages);
        ...
        isWelcomeFetching = false;
    }
    
    // ✅ استخدام fallback من الملف
    const fallbackMessages = lang === 'ar'
        ? ["✨ كيف يمكنني مساعدتك اليوم؟", "🚀 جاهز لبناء شيء رائع!", "💻 لنكتب كوداً جميلاً معاً", "🎯 ما هو مشروعك اليوم؟", "🤖 أنا هنا لأجلك!"]
        : ["✨ How can I help you today?", "🚀 Ready to build something awesome!", "💻 Let's write beautiful code together", "🎯 What's your project today?", "🤖 I'm here for you!"];
    
    welcomeMessages = fallbackMessages;
    welcomeMessagesReady = true;
    
    // ✅ إخفاء السبلاش حتى مع fallback
    if (!splashHiddenDueToMessages) {
        hideSplashAfterMessagesReady();
    }
}

// دالة جديدة لإخفاء السبلاش بعد تجهيز الرسائل


// تحديث دالة resetWelcomeMessages لاستخدام النصوص المخزنة
function resetWelcomeMessages() {
    if (welcomeTypingInterval) clearInterval(welcomeTypingInterval);
    if (welcomeDeletingInterval) clearInterval(welcomeDeletingInterval);
    if (welcomeTimeout) clearTimeout(welcomeTimeout);
    
    const lang = localStorage.getItem('codeai_lang') || 'en';
    
    // ✅ استخدام النصوص المخزنة إذا كانت موجودة
    if (cachedWelcomeMessages && cachedWelcomeMessages[lang]) {
        welcomeMessages = cachedWelcomeMessages[lang];
...
// دالة لبدء تأثير الكتابة - يتم استدعاؤها بعد اختفاء السبلاش
function startWelcomeTypingEffect() {
    welcomeTextElement = document.getElementById('dynamicWelcomeText');
    if (!welcomeTextElement) {
        console.error("❌ dynamicWelcomeText element not found!");
        return;
    }
    ...
    // ✅ مسح النص الحالي تماماً
    welcomeTextElement.textContent = '';
    currentWelcomeDisplay = '';
    ...
    if (welcomeMessages.length === 0) {
        console.warn("⚠️ No welcome messages available");
        return;
    }
    
    console.log("✅ Starting welcome typing effect with", welcomeMessages.length, "messages");
...
// كتابة الرسالة التالية
// كتابة الرسالة التالية
// كتابة الرسالة التالية
// كتابة الرسالة التالية (بدون إضافة الرمز >)
function typeNextWelcomeMessage() {
    if (!welcomeTextElement || welcomeMessages.length === 0) return;
    
    const message = welcomeMessages[currentWelcomeIndex];
    if (!message) return;
    
    // ✅ النص يظهر كما هو بدون إضافة رمز
    const fullMessage = message;
    
    currentWelcomeDisplay = "";
    welcomeTextElement.textContent = "";
    welcomeTextElement.classList.add('typing');
    ...
    welcomeTypingInterval = setInterval(() => {
        if (charIndex < fullMessage.length) {
            currentWelcomeDisplay += fullMessage[charIndex];
            welcomeTextElement.textContent = currentWelcomeDisplay;
            charIndex++;
        } else {
            clearInterval(welcomeTypingInterval);
            welcomeTypingInterval = null;
            welcomeTextElement.classList.remove('typing');
            
            welcomeTimeout = setTimeout(() => {
                deleteCurrentWelcomeMessage();
            }, 5000);
        }
    }, 60);
}
...
// حذف الرسالة الحالية
function deleteCurrentWelcomeMessage() {
    if (!welcomeTextElement) return;
    
    welcomeTextElement.classList.add('deleting');
    let currentText = welcomeTextElement.textContent || "";
    let charIndex = currentText.length;
    
    if (welcomeDeletingInterval) clearInterval(welcomeDeletingInterval);
    
    welcomeDeletingInterval = setInterval(() => {
        if (charIndex > 0) {
            charIndex--;
            welcomeTextElement.textContent = currentText.substring(0, charIndex);
        } else {
            clearInterval(welcomeDeletingInterval);
            welcomeDeletingInterval = null;
            welcomeTextElement.classList.remove('deleting');
            
            // الانتقال إلى الرسالة التالية
            currentWelcomeIndex = (currentWelcomeIndex + 1) % welcomeMessages.length;
            
            // انتظار 500ms ثم كتابة الرسالة التالية
            setTimeout(() => {
                typeNextWelcomeMessage();
            }, 500);
        }
    }, 40); // سرعة الحذف: 40ms لكل حرف
}

// دالة لإعادة تعيين النصوص الترحيبية (عند تغيير اللغة مثلاً)
// دالة لإعادة تعيين النصوص الترحيبية (عند تغيير اللغة مثلاً)


// استدعاء fetchWelcomeMessages بعد انتهاء السبلاش (في initSplashScreen)
// أضف هذا السطر داخل setTimeout بعد إخفاء السبلاش:
// fetchWelcomeMessages();

// تحديث دالة updateUIText لتعيد تحميل النصوص الترحيبية عند تغيير اللغة
// في دالة updateUIText، أضف هذا الكود لتحديث اتجاه النص الترحيبي عند تغيير اللغة
const originalUpdateUIText = updateUIText;
updateUIText = function() {
    originalUpdateUIText();
    
    // ✅ تحديث اتجاه الفيديو والنص حسب اللغة فقط
    updateWelcomeContainerDirection();
    
    // ✅ استخدام النصوص المخزنة بدلاً من إعادة الجلب
    const lang = localStorage.getItem('codeai_lang') || 'en';
    if (cachedWelcomeMessages && cachedWelcomeMessages[lang]) {
        welcomeMessages = cachedWelcomeMessages[lang];
...
if (currentTheme !== t) {
    ...
    // تحديث فيديو الأفاتار
    if (avatarVideo) {
        const newSrc = mode === 'light' ? 'assets/avatar-vid-light.mp4' : 'assets/avatar-vid-dark.mp4';
        if (avatarVideo.src !== newSrc) {
            avatarVideo.src = newSrc;
            avatarVideo.load();
            
            avatarVideo.oncanplay = function() {
                if (!videoPlayed) {
                    setTimeout(() => {
                        avatarVideo.play().then(() => {
                            videoPlayed = true;
...
codeArea.addEventListener('paste', (e) => {
    ...
    // حفظ حالة ما قبل اللصق
    recordChange(codeArea.value);

    const start = codeArea.selectionStart;
    const end = codeArea.selectionEnd;
    const text = codeArea.value;
    codeArea.value = text.substring(0, start) + pasteText + text.substring(end);
    codeArea.selectionStart = codeArea.selectionEnd = start + pasteText.length;

    projectFiles[activeFileIndex].content = codeArea.value;
    updateView();

    // حفظ حالة ما بعد اللصق
    recordChange(codeArea.value);
...
async function runCode() {
   ...
   /* } else if (ext === 'py') {
      ...
      await runInSandbox(detectLanguage());
        // ملفات غير مدعومة
...
// للموبايل - مراقبة آخر حرفين لمعرفة ما إذا كان إغلاقاً تلقائياً
// ===== الإغلاق التلقائي (يعمل على الديسكتوب والجوال) =====

// للديسكتوب - keydown يلتقط المفاتيح مباشرة
// ===== الإغلاق التلقائي لوسوم HTML =====
codeArea.addEventListener('keydown', (e) => {
    if (e.key === '>') {
        ...
        const lastOpenTag = text.lastIndexOf('<', pos - 1);
        if (lastOpenTag !== -1) {
            // الحصول على محتوى ما بعد <
            const afterBracket = text.substring(lastOpenTag + 1, pos);
            
            // إذا كان اسم وسم صالح (لا يحتوي مسافات أو =)
            if (afterBracket && /^[a-zA-Z][a-zA-Z0-9]*$/.test(afterBracket.trim())) {
                const tagName = afterBracket.trim();
                const closeTag = `</${tagName}>`;
                
                // التحقق من أن الإغلاق غير موجود مسبقاً
                const restOfText = text.substring(pos);
                if (!restOfText.startsWith(closeTag)) {
                    e.preventDefault();
                    e.stopPropagation();
                    ...
                    codeArea.value = before + closeTag + after;
                    // وضع المؤشر بين > و </tag>
                    codeArea.selectionStart = codeArea.selectionEnd = pos + 1;
...
codeArea.addEventListener('input', () => {
    ...
    // هل آخر حرف مكتوب هو >
    if (pos > 0 && text[pos - 1] === '>') {
        const beforeGt = text.substring(0, pos);
        const afterGt = text.substring(pos);
        
        // البحث عن < السابق
        const lastOpenTag = beforeGt.lastIndexOf('<', pos - 2);
        if (lastOpenTag !== -1) {
            const tagContent = beforeGt.substring(lastOpenTag + 1, pos - 1); // قبل >
            
            // هل هو اسم وسم صالح
            if (tagContent && /^[a-zA-Z][a-zA-Z0-9]*$/.test(tagContent.trim())) {
                const tagName = tagContent.trim();
                const closeTag = `</${tagName}>`;
                
                // هل الإغلاق غير موجود
                if (!afterGt.startsWith(closeTag)) {
                    codeArea.value = beforeGt + closeTag + afterGt;
                    codeArea.selectionStart = codeArea.selectionEnd = pos; // بين > و </
...
codeArea.addEventListener('touchmove', syncScroll);
    ...
    marked.setOptions({ 
        gfm: true,        // دعم GitHub Flavored Markdown (جداول، قوائم مهام)
        breaks: true,     // تحويل الأسطر الجديدة إلى <br>
        tables: true,     // تفعيل دعم الجداول بشكل صريح
        pedantic: false,  // عدم التقيد الصارم بقواعد Markdown الأصلية
        sanitize: false,  // عدم تعقيم HTML (نثق بمصدر النموذج)
        smartLists: true, // قوائم ذكية
        smartypants: false
    });
}

async function showEmailVerificationMessage(email) {
    // 1. إخفاء شاشة التسجيل
    const welcomeAuthScreen = document.getElementById('welcomeAuthScreen');
    if (welcomeAuthScreen) {
        welcomeAuthScreen.classList.remove('open');
        welcomeAuthScreen.style.display = 'none';
    }
    
    // 2. إظهار رسالة التحقق
    const signinScreen = document.getElementById('signinScreen');
    if (signinScreen) {
        signinScreen.classList.add('active');
        signinScreen.innerHTML = `
            <div class="register-card transparent-card">
                <div class="register-icon">📧</div>
                <h3 class="register-title">Verify your email</h3>
                <p class="register-subtitle" style="margin-bottom: 20px;">
                    We've sent a verification link to<br>
                    <strong>${email}</strong>
                </p>
                <p class="register-subtitle" style="font-size: 14px; opacity: 0.8; margin-bottom: 30px;">
                    Check your inbox and follow the link to activate your account.
                </p>
                <button id="verificationDoneBtn" class="welcome-auth-btn primary-btn" style="margin-top: 20px;">
                    I've verified my email
                </button>
                <button id="resendVerificationBtn" class="text-btn" style="margin-top: 15px;">
                    Resend verification email
                </button>
                <button id="backToSignupBtn" class="text-btn" style="margin-top: 5px;">
                    ← Back to sign up
                </button>
            </div>
        `;
        
        // ✅ زر "تم التحقق" - يحاول تسجيل الدخول
        document.getElementById('verificationDoneBtn')?.addEventListener('click', async () => {
    // 1. محاولة تحديث الجلسة من Supabase
    const { data: { session }, error } = await supabaseClient.auth.getSession();
    
    if (session) {
        // 2. التحقق مما إذا كان البريد مؤكداً
        const isEmailConfirmed = session.user.email_confirmed_at || session.user.confirmed_at;
        
        if (isEmailConfirmed) {
            // البريد مؤكد فعلاً - نفتح التطبيق
            signinScreen.classList.remove('active');
            signinScreen.innerHTML = '';
            hideAuthScreen();
            await loadConversations();
            ...
            // الجلسة موجودة لكن البريد لم يؤكد بعد
            showToast('Email not verified yet. Please check your inbox and click the link.', 'error');
        }
    } else {
        // لا توجد جلسة إطلاقاً
        showToast('No session found. Please complete the signup process again.', 'error');
    }
});
...
document.getElementById('registerBtn')?.addEventListener('click', async () => {
    ...
    if (!nickname || nickname.length < 2 || nickname.length > 30) {
        errorDiv.textContent = 'Please enter a valid nickname (2-30 characters)';
        errorDiv.style.display = 'block';
        return;
    }
...
// دالة مساعدة لإخفاء شاشة التسجيل
...
document.getElementById('newChatBtn').addEventListener('click', () => {
    ...
    // ✅ إعادة تعيين الملفات إلى الافتراضي
    ...
    // ✅ إخفاء معاينة الصور/المستندات إن وجدت
    hideImagePreview();
    ...
    // إعادة تشغيل الفيديو إذا لم يكن قيد التشغيل
    if (avatarVideo && videoPlayed) {
        avatarVideo.currentTime = 0;
        avatarVideo.play().catch(e => console.log("Video replay prevented:", e));
    }
    
    // إعادة بدء الكتابة
    if (welcomeMessagesReady && welcomeMessages.length > 0) {
        setTimeout(() => {
            startWelcomeTypingEffect();
        }, 500);
    }
...
imageFileInput?.addEventListener('change', (e) => {
    ...
    files.forEach(file => {
        ...
        reader.onload = (event) => {
            ...
            updateAttachmentsUI()
            checkInputState(inputEl); // لتحديث حالة زر الإرسال
        };
...
window.removeImageAttachment = function(id) {
    ...
    renderImageAttachments();
    updateAttachmentsUI()
    checkInputState(inputEl); // تحديث الزر
};
...
// ==========================================
    ...
    // ✅ إذا كان في وضع الإيقاف، قم بإيقاف الـ streaming
    if (mode === 'stop') {
        stopStreaming();
        return;
    }
...
realDeleteBtn.addEventListener('click', async () => {
    ...
    if (deleteMode === 'conv') {
        ...
        if (editingTabIndex > -1 && projectFiles.length > 1) {
            ...
            // إذا كان الملف النشط هو الأخير، ننقل التحديد للملف الذي قبله
            if (activeFileIndex >= projectFiles.length) {
                activeFileIndex = projectFiles.length - 1;
            }
            renderTabs();
            // عرض محتوى الملف الجديد
            const currentFile = projectFiles[activeFileIndex];
            if (isImageFile(currentFile.name)) {
                displayImageInEditor(currentFile);
            } else if (isDocumentFile(currentFile.name)) {
            ...
            // حفظ المحادثة بعد الحذف
            const conv = convs.find(c => c.id === activeId);
            if (conv) {
                conv.files = projectFiles;
                await saveConversationToServer(conv).catch(console.error);
            }
        }
    }
    ...
    editingTabIndex = -1;
    deleteMode = 'file'; // إعادة ضبط الوضع
});

document.getElementById('cancelDeleteConfirmBtn')?.addEventListener('click', () => {
    deleteConfirmModal.classList.remove('active');
    // لا نغير editingTabIndex هنا إذا كان المستخدم قد يريد المحاولة مرة أخرى
});
    ...
    try {
        ...
        if (isImageFile(fileName)) {
            if (currentFile.content && currentFile.content.startsWith('data:')) {
                ...
                const response = await fetchWithAuth(currentFile.content);
        ...
        // 2. للمستندات (PDF, DOCX, PPTX): تحميل من URL
            ...
            if (currentFile.url) {
                ...
                const response = await fetchWithAuth(currentFile.url);
...
function buildTaskListContainer(tasks, duration) {
    ...
    // المؤقت (إذا وجدت مدة)
    // داخل buildTaskListContainer
const timerInfo = document.createElement('div');
timerInfo.className = 'timer-info';
if (duration !== null && duration > 0) {
    timerInfo.innerHTML = `
        <span>Completed</span>
        <span class="running-time">Ran for ${duration}s</span>
    `;
} else {
    timerInfo.innerHTML = `
    ...
    tasksDiv.innerHTML = generateTasksHTML(tasks, duration);
    ...
    for (let userIdx = 0; userIdx < conv.messages.length; userIdx++) {
    const userMsg = conv.messages[userIdx];
    
    // عرض رسالة المستخدم
    appendUserMessage(userMsg.text);
    
    // عرض الرد الحالي (آخر رد)
    if (userMsg.replies.length > 0) {
        const replyIndex = userMsg.currentReplyIndex !== undefined && userMsg.currentReplyIndex >= 0 
            ? userMsg.currentReplyIndex 
            : userMsg.replies.length - 1;
        
        const aiReply = userMsg.replies[replyIndex];
        const isLast = (userIdx === conv.messages.length - 1);
        
        const d = document.createElement('div');
        d.className = 'msg ai';
        d.setAttribute('data-reply-index', replyIndex);
        d.setAttribute('data-user-id', userMsg.id);
        
        let displayText = aiReply.text || '';
        let tasksContainer = null;
        
        if (aiReply.tasks && (aiReply.tasks.built?.length || aiReply.tasks.modified?.length || aiReply.tasks.deleted?.length)) {
            const duration = aiReply.tasks.duration;
            tasksContainer = buildTaskListContainer(aiReply.tasks, duration);
            ...
            tasksContainer = buildTaskListContainer(tasks, null);
            displayText = extractDisplayText(displayText);
            aiReply.text = displayText;
        }
        ...
        d.innerHTML = `<div class="msg-header"></div><div class="ai-content">${htmlContent}</div>`;
        const contentEl = d.querySelector('.ai-content');
        contentEl.style.direction = dir;
        contentEl.style.textAlign = dir === 'rtl' ? 'right' : 'left';
        
        if (tasksContainer) d.insertBefore(tasksContainer, contentEl);
        
        if (isLast && !serverFinished && streamingConvId === activeId) {
            currentAiMsgElement = d;
            isStreaming = true;
            safeBuffer = aiReply.text;
        } else {
            addMessageActions(contentEl, displayText, aiReply.model, conv.id, aiReply.id, userMsg.id);
            if (aiReply.filesSnapshot && aiReply.filesSnapshot.length) {
                addRestoreButtonToExistingMessage(contentEl, d, aiReply.filesSnapshot);
            }
            const savedThought = getThoughtForMessage(activeId, userMsg.id);
            if (savedThought) injectThoughtButton(d, savedThought);
        }
...
// استبدل دالة showStatus بالكامل بهذه النسخة
function showStatus(baseText) {
    // ✅ إزالة القديم فوراً
    if (currentStatusEl) currentStatusEl.remove();
    ...
    // ✅ إدراج قبل الرسالة مباشرة (وليس في النهاية)
    if (currentAiMsgElement && currentAiMsgElement.parentNode) {
        currentAiMsgElement.parentNode.insertBefore(currentStatusEl, currentAiMsgElement);
    } else {
...
// استبدل appendUserMessage بهذه النسخة الأسرع
...
// ==========================================
// دوال المرفقات الموحدة (صور + أخطاء)
// ==========================================
...
function attachErrorToChat(fullErrorMsg) {
    ...
    // استدعاء دالة الرسم الموحدة
    renderAllAttachments();
    checkInputState(inputEl);
}

// 2. دالة رسم كل المرفقات (صور + أخطاء) في شريط واحد
// 2. دالة رسم كل المرفقات (صور + أخطاء) في شريط واحد
function renderAllAttachments() {
    ...
    const hasImages = activeImageAttachments.length > 0;
    const hasErrors = activeAttachments.length > 0;
    ...
    // عرض الصور
    activeImageAttachments.forEach((attachment, index) => {
        const item = document.createElement('div');
        item.className = 'img-attachment-item';
        // ✅ إضافة مسافة بين العناصر (ما عدا الأخير)
        if (index < activeImageAttachments.length - 1 || hasErrors) {
            item.style.marginRight = '6px';
        }
        item.innerHTML = `
            <img src="${attachment.dataUrl}" alt="attached image">
            <div class="img-delete-btn" onclick="window.removeImageAttachment('${attachment.id}')">✕</div>
        `;
        imageAttachmentsBar.appendChild(item);
    });
    ...
    activeAttachments.forEach((err, index) => {
        const item = document.createElement('div');
        item.className = 'img-attachment-item error-attachment-item';
        item.style.cssText = `
        ...
        // ✅ إضافة مسافة بين الأخطاء (ما عدا الأخير)
        if (index < activeAttachments.length - 1) {
            item.style.marginRight = '6px';
        }
        
        let displayText = err.text.replace(/^❌\s*/, '').substring(0, 20);
        
        item.innerHTML = `
            <span style="font-size: 14px; margin-right: 4px;">🐞</span>
            <span style="font-size: 11px; color: var(--text-secondary); max-width: 70px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${displayText}</span>
            <div class="img-delete-btn" onclick="window.removeAttachment('${err.id}')" style="margin-left: 4px;">✕</div>
        `;
...
// 3. استبدال الدوال القديمة بالجديدة
renderImageAttachments = renderAllAttachments;
renderAttachments = renderAllAttachments;
...
// 5. تحديث دالة إضافة الصور
const originalImageInputHandler = imageFileInput?.addEventListener('change', (e) => {
    // ... الكود الأصلي
});

// نعدل المستمع ليستخدم renderAllAttachments
if (imageFileInput) {
    const oldListener = imageFileInput.onchange;
    imageFileInput.addEventListener('change', (e) => {
        const files = Array.from(e.target.files);
        if (!files.length) return;
        ...
        files.forEach(file => {
            ...
            reader.onload = (event) => {
                ...
                renderAllAttachments();  // ✅ استدعاء الدالة الموحدة
                checkInputState(inputEl);
            };
            reader.readAsDataURL(file);
        });
...
// تعديل renderImageAttachments ليشمل الأخطاء أيضاً
...
// أضف هذه الدالة قبل sendMessage
function resetMessageState() {
    // تصفير جميع المتغيرات
    typeQueue = [];
    fullMarkdownBuffer = "";
    safeBuffer = "";
    streamCursor = 0;
    ...
    currentTaskType = 'code'; // ✅ إعادة تعيين نوع المهمة
    lastErrorMessageElement = null;
    pendingRetryMessage = null;
    hasReceivedModelResponse = false;
    // إزالة المؤقتات
    if (typeTimeout) {
...
function showDailyLimitReachedToast() {
    const toast = document.createElement('div');
    toast.style.cssText = `
        position:fixed; top:20px; left:50%; transform:translateX(-50%);
        background:#ff4444; color:white; padding:16px 24px;
        border-radius:16px; font-size:14px; z-index:10000;
        opacity:0; transition:opacity 0.3s; text-align:center;
        max-width:90%; box-shadow:0 8px 24px rgba(255,68,68,0.3);
    `;
    toast.innerHTML = `
        <div style="font-weight:700; margin-bottom:6px;">⚠️ Daily limit reached</div>
        <div style="opacity:0.9; font-size:13px;">You've used all ${currentDailyLimit} messages for today.</div>
        <button onclick="openPlansPage(); this.parentElement.parentElement.remove();" 
                style="margin-top:10px; padding:8px 20px; background:white; color:#ff4444; 
...
if (!isSubscribed() && !canSendMessage()) {
    showDailyLimitReachedToast();
    return;
}

// ✅ زيادة عداد الاستخدام اليومي (للمستخدمين المجانيين فقط)


    // في المحاولة الأولى فقط نقوم بتعطيل الواجهة وإضافة الرسائل
    isStreamingActive = true;
    isSending = true;
    allowMicWhenEmpty = false;
    checkInputState(inputEl); // سيظهر زر الإيقاف
    ...
    try {
        ...
        if (!saved) {
            ...
            finalizeError("Failed to create conversation. Please try again.", text);
            return;
        }
        ...
        const conv = convs.find(c=>c.id===activeId);
        // داخل sendMessage، بعد التأكد من وجود conv (محادثة نشطة)
if (!isRetry) {
    // 1. إضافة رسالة المستخدم (ككائن متفرع)
    ...
    const newUserMsg = {
        ...
        text: text,
        timestamp: Date.now(),
        replies: [],      // سيتم إضافة الرد لاحقاً
        currentReplyIndex: -1
    };
    conv.messages.push(newUserMsg);

    // 2. إضافة رسالة AI فارغة مؤقتاً في الـ replies
    ...
    newUserMsg.replies.push(newAiReply);
    newUserMsg.currentReplyIndex = 0;

    // عرض رسالة المستخدم فوراً
    appendUserMessage(text);

    // حفظ الحالة مؤقتاً (يمكن استدعاء saveConversationToServer بشكل غير متزامن)
    saveConversationToServer(conv).catch(e => console.warn("Auto-save error:", e));
} else {
          console.log("🔄 Retry mode: not adding duplicate messages");
    }
  // --- التعديل: دمج الخطأ المربوط مع الرسالة ---
    ...
    // ✅ إزالة أي status قديم
    removeStatus();
    
    // ✅ إزالة أي عنصر رسالة AI قديم لم ينتهِ
    if (currentAiMsgElement && currentAiMsgElement.parentNode) {
        // إذا كانت هناك رسالة قديمة غير مكتملة، نحذفها
        const conv = convs.find(c => c.id === activeId);
        if (conv && conv.messages.length > 0 && conv.messages[conv.messages.length-1].role === 'ai') {
            // نزيل آخر رسالة AI إذا كانت غير مكتملة
            if (!serverFinished) {
                conv.messages.pop();
                currentAiMsgElement.remove();
            }
        }
    }
    currentAiMsgElement = null;
...
const cleanedHistory = historyContext.slice(-12).map(msg => ({
        role: msg.role,
...
fd.append("settings", JSON.stringify({
    ...
    temperature: parseFloat(localStorage.getItem('codeai_temp') || 0.7),
    top_p: parseFloat(localStorage.getItem('codeai_top_p') || 1.0),
    native_mode: localStorage.getItem('codeai_native_mode') === 'true'
}));
...
console.log("user prompt:" + fd)
currentAbortController = new AbortController();

const response = await fetchWithAuth(RENDER_SERVER_URL + '/api/chat', {
  method: "POST",
  body: fd,
...
renderImageAttachments();
        ...
        // ✅ فحص خطأ تجاوز الحد اليومي
if (response.status === 429) {
    const errorData = await response.json().catch(() => ({}));
    if (errorData.error === 'daily_limit_reached') {
        dailyUsed = errorData.daily_count || currentDailyLimit;
        currentDailyLimit = errorData.daily_limit || currentDailyLimit;
        showDailyLimitReachedToast();
        resetAfterStop();
        return;
    }
        ...
        if (!navigator.onLine || err.message.includes('Failed to fetch')) {
             ...
             finalizeError("Sorry, an error occurred.", text);
        }
    }
...
sendMessage = async function(text, isRetry = false) {
    // إذا كانت هذه أول رسالة (لا توجد محادثة نشطة)
    if (!activeId && text && !isRetry) {
        // إخفاء فيديو السبلاش
        
    }
    
    // داخل sendMessage، قبل استدعاء originalSendMessage:
if (!activeId && text && !isRetry) {
    SplashManager.handleFirstMessage();
}
    
    // استدعاء الدالة الأصلية
    return originalSendMessage(text, isRetry);
};
...
function handle503Error(text) {
    ...
    if (retryCount < maxRetries) {
        ...
        retryCount = 0;
        finalizeError("Server error, please try again later.", text); // ✅ تمرير الرسالة الأصلية
    }
}
...
function finalizeError(errorMsg, originalMessage = null) {
    removeStatus();
    isStreaming = false;
    isSending = false; // ✅ تغيير إلى false للسماح بالإرسال
    allowMicWhenEmpty = true;
    
    // ✅ إعادة تعيين مؤشر الرد
    hasReceivedModelResponse = false;
    
    // تخزين الرسالة الأصلية لإعادة المحاولة
    if (originalMessage) {
    ...
    const errorText = isArabic 
        ? '⚠️ حدث خطأ في الخادم. يرجى المحاولة مرة أخرى لاحقاً.'
        : '⚠️ A server error occurred. Please try again later.';
    
    // إنشاء عنصر رسالة الذكاء الاصطناعي للخطأ
    const d = document.createElement('div');
    ...
    d.innerHTML = `
        <div style="display: flex; align-items: flex-start; gap: 12px;">
            ...
            ">⚠️</div>
            <div style="flex: 1;">
                <p style="margin: 0 0 12px 0; color: var(--text-color); font-size: 14px; line-height: 1.5;">${errorText}</p>
                <button class="retry-error-btn" style="
                    display: inline-flex;
                    align-items: center;
                    gap: 6px;
                    padding: 8px 20px;
    ...
    // حفظ مرجع لعنصر الخطأ
    lastErrorMessageElement = d;
    
    // إضافة مستمع لزر إعادة المحاولة
    const retryBtn = d.querySelector('.retry-error-btn');
    retryBtn.addEventListener('click', async () => {
        // التحقق من وجود رسالة للإعادة
        ...
        // حذف عنصر الخطأ
        if (lastErrorMessageElement) {
            lastErrorMessageElement.remove();
            lastErrorMessageElement = null;
        }
        ...
        // إزالة آخر رسالة مستخدم و AI فاشلة من المصفوفة
        const conv = convs.find(c => c.id === activeId);
        if (conv && conv.messages.length >= 2) {
            // إزالة آخر رسالتين (المستخدم + الذكاء الاصطناعي الفاشلة)
            const lastTwo = conv.messages.slice(-2);
            if (lastTwo[0]?.role === 'user' && lastTwo[1]?.role === 'ai') {
    ...
    retryBtn.addEventListener('mouseenter', () => {
        retryBtn.style.background = 'rgba(255, 68, 68, 0.25)';
        retryBtn.style.borderColor = '#ff4444';
    });
    
    retryBtn.addEventListener('mouseleave', () => {
        retryBtn.style.background = 'rgba(255, 68, 68, 0.15)';
        retryBtn.style.borderColor = 'rgba(255, 68, 68, 0.3)';
    });
...
async function checkUserRegistration() {
    // إذا كان وضع التصحيح مفعلاً، نعطي false دائمًا لإظهار شاشة التسجيل
    if (DEBUG_FORCE_REGISTER) return false;

    // 1. التحقق من localStorage أولاً (أسرع وأكثر موثوقية)
    const storedNickname = localStorage.getItem('codeai_nickname');
    if (storedNickname) {
        currentNickname = storedNickname;
        isRegistered = true;
        console.log("✅ User registered from localStorage:", storedNickname);
        return true;
    }

    // 2. إذا لم يكن هناك اسم محلي، نتحقق من السيرفر
    const userId = localStorage.getItem('userId');
    ...
    try {
        ...
        if (data.nickname) {
            currentNickname = data.nickname;
            isRegistered = true;
            localStorage.setItem('codeai_nickname', currentNickname); // حفظ للاستخدام المستقبلي
            return true;
        }
        return false;
    } catch (error) {
...
supabaseClient.auth.onAuthStateChange((event, session) => {
  if (event === 'SIGNED_IN') {
    hideSplash(); // إخفاء شاشة التسجيل
    loadConversations();
    updateAccountDisplay();
  }
});

const originalCheckUserRegistration = checkUserRegistration;
/*window.checkUserRegistration = async function() {
    if (DEBUG_FORCE_REGISTER) {
        const splash = document.getElementById('splashScreen');
        if (splash && !splash.classList.contains('hidden')) {
            ...
            if (authScreen) {
                ...
                setTimeout(() => authScreen.classList.add('open'), 50);
                setTimeout(() => initScrollTrigger(), 500);
            }
        }
        return false;
    }
    return originalCheckUserRegistrationForVideo();
};*/
...
async function registerUser(nickname) {
  ...
  // داخل registerUser، بعد نجاح التسجيل، قبل حفظ currentNickname
if (DEBUG_FORCE_REGISTER) {
    console.log("🐞 DEBUG MODE: Registration successful but not persisting");
    // لا تحفظ البيانات بشكل دائم
    return { success: true };
}
    ...
    try {
      ...
      // داخل دالة registerUser، بعد نجاح التسجيل:
// ✅ تأكد من وجود المستخدم في users
await fetchWithAuth(`${API_BASE_URL}/api/ensure-user`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
        userId: finalUserId
        ...
        const checkResponse = await fetchWithAuth(`${API_BASE_URL}/api/check-nickname`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
window.loadConversations = async function() {
    if (DEBUG_FORCE_REGISTER) {
        console.log("🐞 DEBUG MODE: Skipping conversations load");
        return;
    }
...
function renderConversations() {
    ...
    convs.forEach(cv => {
        ...
        // ✅ إضافة كلاس "نشط" للمحادثة الحالية
        if (cv.id === activeId) {
        ...
        // ✅ إنشاء المستطيل الصغير فوق العنصر النشط
        if (cv.id === activeId) {
            const topBar = document.createElement('div');
            topBar.className = 'conv-active-top-bar';
        ...
        // ✅ منع إعادة تحميل المحادثة المفتوحة حاليًا
        el.addEventListener('click', async () => {
            if (cv.id === activeId) {
                // المحادثة مفتوحة بالفعل – فقط أغلق القائمة إن كانت مفتوحة
                closeMenu();
                return;
            }
            
            // ... (باقي كود تحميل المحادثة الجديدة يبقى كما هو) ...
        ...
        // أحداث الضغط المطوَّل (تبقى دون تغيير)
        let pressTimer;
...
function renderCommandsPage() {
    ...
    // النص التوضيحي مع الخط
    const autoText = t.commandsAuto || (isArabic 
        ? "هذه الأوامر تلقائية — فقط اطلب من النموذج وستُفعّل مباشرة." 
        : "These commands are automatic — just ask the model and they'll activate instantly.");
    ...
    html += commandsData.map(cmd => {
        const title = isArabic ? cmd.titleAr : cmd.titleEn;
        const desc = isArabic ? cmd.descAr : cmd.descEn;
        ...
        return `
            ...
            ">
                <div style="text-align: ${isArabic ? 'right' : 'left'}; direction: ${isArabic ? 'rtl' : 'ltr'}; width: 100%;">
                    <h3 style="margin: 0 0 8px 0; font-size: 18px; font-weight: 600; color: var(--text-color);">${title}</h3>
                    <p style="margin: 0; font-size: 14px; opacity: 0.8; line-height: 1.5;">${desc}</p>
                </div>
            </div>
        `;
    }).join('');
...
function extractMarkdownCodeBlocks(fullText) {
    ...
    while ((match = codeBlockRegex.exec(fullText)) !== null) {
        ...
        const existing = projectFiles.find(f => f.name === fileName);
        if (existing) {
            existing.content = code;
        } else {
            projectFiles.push({ name: fileName, content: code });
        }
        updated = true;
    }
    ...
    if (updated) {
        renderTabs();
        const currentFile = projectFiles[activeFileIndex];
        if (currentFile && !isImageFile(currentFile.name)) {
            codeArea.value = currentFile.content;
            updateView();
        }
    }
...
function addSearchSourcesButton(msgElement, results) {
    console.log("🔘 [Sources] Called with", results?.length, "results");
    
    // البحث عن حاوية الأزرار
    let actionsDiv = msgElement.querySelector('.msg-actions');
    if (!actionsDiv) {
        // إذا لم توجد حاوية، أنشئ واحدة
        actionsDiv = document.createElement('div');
        actionsDiv.className = 'msg-actions';
    ...
    // منع التكرار
    if (actionsDiv.querySelector('.sources-btn')) {
        console.log("⚠️ [Sources] Button already exists");
        return;
    }

    // إنشاء الزر
    const sourcesBtn = document.createElement('button');
    sourcesBtn.className = 'action-btn sources-btn';
    sourcesBtn.textContent = `${results.length} sources`;
    sourcesBtn.title = 'Click to view sources';
...
`;
    ...
    // تنظيف النتائج وإضافتها
    dropdown.innerHTML = results.map(r => {
        const cleanTitle = (r.title || '').replace(/\n/g, ' ').replace(/\s+/g, ' ').trim();
        const cleanUrl = (r.url || '').trim();
        return `<a href="${cleanUrl}" target="_blank" rel="noopener" 
            style="display:block;color:#e0e0e0;text-decoration:none;
            padding:8px 10px;font-size:13px;border-radius:4px;
            border-bottom:1px solid #222;line-height:1.4;"
            onmouseover="this.style.background='#222'" 
            onmouseout="this.style.background='transparent'"
        >[${r.index}] ${cleanTitle}</a>`;
    }).join('');
    ...
    sourcesBtn.addEventListener('click', (e) => {
        ...
        // إغلاق أي قوائم أخرى
        document.querySelectorAll('.sources-dropdown').forEach(d => d.style.display = 'none');
        dropdown.style.display = isVisible ? 'none' : 'block';
    });
    
    // إغلاق عند النقر خارجاً
    document.addEventListener('click', function closeDropdown(e) {
        if (!sourcesBtn.contains(e.target) && !dropdown.contains(e.target)) {
            dropdown.style.display = 'none';
        }
    });
    
    // إضافة الزر والقائمة إلى حاوية الأزرار
    actionsDiv.style.position = 'relative';
    actionsDiv.appendChild(sourcesBtn);
    actionsDiv.appendChild(dropdown);
    
    console.log("✅ [Sources] Button added");
}

// استدعِ هذه الدالة في handleAssistantMessage عند اكتمال الرد
// داخل finishMessageProcessing أو بعد اكتمال safeBuffer

// متغير لتخزين AbortController الخاص بالطلب الحالي
...
async function stopStreaming() {
    ...
    if (hasReceivedModelResponse && !isSubscribed()) {
        incrementDailyUsage().catch(e => console.warn('Failed to increment usage:', e));
        console.log("✅ Usage incremented (response was received before stop)");
    }
    
    // إرسال طلب إيقاف إلى السيرفر
    try {
        await fetchWithAuth(`${API_BASE_URL}/api/stop`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ clientId: myClientId, convId: activeId })
        });
    } catch (e) {
        console.warn('Stop request failed:', e);
    }
    
    // إجهاض الـ fetch المحلي
...
// في دالة sendMessage، عند إنشاء fetch، قم بتخزين الـ AbortController
// داخل try/catch الخاص بـ fetch:
...
// أيقونات SVG المخزنة في template
let taskIconsTemplate = null;

// تهيئة أيقونات المهام


// تحديث دالة _generateTasksHTMLFromObject لاستخدام getTaskIcon
// استبدل دالة _generateTasksHTMLFromObject بهذه النسخة المحدثة
...
// متغير لتخزين نوع المهمة الحالية
let currentTaskType = 'code';

function handleTaskTypeDetected(payload) {
    const taskType = payload.task_type;
    const isCodeRelated = payload.is_code_related;
    
    console.log(`📋 Task type detected: ${taskType}, isCodeRelated: ${isCodeRelated}`);
    
    // تخزين النوع للاستخدام لاحقاً
    currentTaskType = taskType;
    
    // إذا كانت المهمة عامة (general) أو غير متعلقة بالكود
    if (taskType === 'general' || !isCodeRelated) {
        // إخفاء قائمة المهام إذا كانت موجودة
        if (currentTaskContainer) {
            currentTaskContainer.style.display = 'none';
        }
...
function finishMessageProcessing() {
    ...
    if (hasReceivedModelResponse && !isSubscribed() && activeId) {
        incrementDailyUsage().catch(e => console.warn('Failed to increment usage:', e));
        console.log("✅ Usage incremented (normal completion)");
    }
    ...
    if (activeId && currentAiMsgElement) {
        ...
        if (lastUserMsg && lastUserMsg.replies.length > 0) {
            // آخر رد AI في تلك الرسالة
            lastReply = lastUserMsg.replies[lastUserMsg.replies.length - 1];
            const displayText = extractDisplayText(fullMarkdownBuffer);

            lastReply.text = displayText;
            lastReply.model = currentStreamModel || "Gemini 3 Flash";

            // المهام واللقطة
            const tasks = extractTasksFromResponse(safeBuffer);
            if (currentTimerStartTime) {
                tasks.duration = Math.floor((Date.now() - currentTimerStartTime) / 1000);
            }
            lastReply.tasks = tasks;

            const currentFilesSnapshot = projectFiles.map(file => ({
                name: file.name,
                content: file.content,
                type: file.type || getMimeType(file.name)
            }));
            lastReply.filesSnapshot = currentFilesSnapshot;
            lastReply.is_latest = true;
lastUserMsg.currentReplyIndex = lastUserMsg.replies.length - 1;
            // حفظ المحادثة (بدون إظهار رسالة خطأ فاشلة)
            saveConversationToServer(conv).catch(err => {
                console.error("❌ Failed to save conversation:", err);
                // لا نعرض toast للمستخدم إلا إذا كان الخطأ حقيقياً ومؤثراً
                // يمكنك إلغاء التعليق على السطر التالي إذا أردت إعلام المستخدم:
                // showToast("Could not save message, but it's still visible now. Refresh might be needed.", 'warning');
            });
        }
    }
        
        // داخل finishMessageProcessing، بعد حساب tasks و durationSeconds
let tasksContainer = currentTaskContainer;

if (tasksContainer) {
    // تحديث محتوى القائمة بالمهام النهائية مع المدة
    const tasksListEl = tasksContainer.querySelector('.tasks-list');
    if (tasksListEl) {
        tasksListEl.innerHTML = generateTasksHTML(tasks, durationSeconds);
    }
    
    // تحديث رأس المؤقت
    const timerInfo = tasksContainer.querySelector('.timer-info');
    if (timerInfo) {
        const spinner = timerInfo.querySelector('.spinner');
        if (spinner) spinner.style.display = 'none';
        
        const runningTimeSpan = timerInfo.querySelector('.running-time');
        if (runningTimeSpan) {
            runningTimeSpan.textContent = `Ran for ${durationSeconds || 0}s`;
        }
        
        // تغيير "Working..." إلى "Completed"
        const statusSpan = timerInfo.querySelector('span:not(.running-time)');
        if (statusSpan) statusSpan.textContent = 'Completed';
    }
} else {
    // إنشاء حاوية جديدة (حالة نادرة)
    tasksContainer = buildTaskListContainer(tasks, durationSeconds);
    if (tasksContainer) {
        const contentEl = currentAiMsgElement.querySelector('.ai-content');
        currentAiMsgElement.insertBefore(tasksContainer, contentEl);
    }
}

// لا نحتاج إلى tasksHTML منفصلة بعد الآن
...
if (currentSearchResults && currentSearchResults.length) {
    html = processCitations(html, currentSearchResults);
}

const contentEl = currentAiMsgElement.querySelector('.ai-content');
if (contentEl && lastReply) {
    contentEl.innerHTML = html;
    addMessageActions(contentEl, extractDisplayText(fullMarkdownBuffer), currentStreamModel, activeId, lastReply.id, lastUserMsg.id);
    addRestoreButton(contentEl, currentAiMsgElement);
} else {
    console.warn("finishMessageProcessing: lastReply is null, cannot add actions");
}
        
        processFilesUpdate(safeBuffer);
        extractMarkdownCodeBlocks(safeBuffer);
    }
    
    // استخراج المصادر من نص الـ Markdown إن لم تصل عبر SSE

    
    // في نهاية finishMessageProcessing، بعد إضافة زر Restore
if (currentAiMsgElement && currentSearchResults && currentSearchResults.length) {
    addSearchSourcesButton(currentAiMsgElement, currentSearchResults);
}
...
function addMessageActions(msgElement, fullText, modelName, convId, messageId, userMsgId) {
...
const modelLabel = document.createElement('span');
    ...
    shareBtn.innerHTML = 'S';
    // عند إنشاء shareBtn
shareBtn.onclick = () => {
    const conv = convs.find(c => c.id === convId);
    if (!conv) return;
    // ابحث عن فهرس رسالة المستخدم التي تسبق هذه الرسالة مباشرة
    const currentMsgIndex = conv.messages.findIndex(m => m.id === messageId);
    let userMessage = '';
    if (currentMsgIndex > 0 && conv.messages[currentMsgIndex - 1].role === 'user') {
        userMessage = conv.messages[currentMsgIndex - 1].text;
    }
    shareMessage(fullText, modelName, convId, messageId, userMsgId);
};
    
    // زر ثلاث نقاط
    const moreBtn = document.createElement('button');
moreBtn.className = 'action-btn more-actions-btn';
moreBtn.innerHTML = '⋮';
moreBtn.style.position = 'relative'; // مهم

const popover = document.createElement('div');
popover.className = 'msg-more-popover';
popover.style.display = 'none'; // يبدأ مخفياً
popover.innerHTML = `
    <div class="popover-item like-item">👍 Like</div>
    <div class="popover-item dislike-item">👎 Dislike</div>
`;

// إضافة popover كأخ للزر (وليس كطفل للـ msg-actions مباشرة)
moreBtn.appendChild(popover);

moreBtn.onclick = (e) => {
    e.stopPropagation();
    const isVisible = popover.style.display === 'block';
    // إخفاء كل القوائم الأخرى أولاً
    document.querySelectorAll('.msg-more-popover').forEach(p => p.style.display = 'none');
...
document.addEventListener('click', (e) => {
    ...
    popover.querySelector('.like-item').onclick = () => {
        openFeedbackBottomSheet(convId, messageId, 'like');
        popover.classList.remove('show');
    };
    popover.querySelector('.dislike-item').onclick = () => {
        openFeedbackBottomSheet(convId, messageId, 'dislike');
        popover.classList.remove('show');
    };
    
    msgElement.setAttribute('data-conv-id', convId);
...
function handleIncomingFiles(files) {
    ...
    files.forEach(file => {
        ...
        if (!currentTasks.built.includes(file.name)) {
            currentTasks.built.push(file.name);
            if (currentTaskContainer) {
...
function processFilesUpdate(fullText) {
    ...
    // <FILE file="file.ext">...</FILE>
    // <FILE target="file.ext">...</FILE>
    // <FILE=file.ext>...</FILE>
    const fileRegex = /<FILE\s*(?:name|file|target)?\s*[=:]?\s*["']?([^\s"'<>]+)["']?\s*>([\s\S]*?)<\/FILE>/gi;
    let match;
    while ((match = fileRegex.exec(fullText)) !== null) {
    ...
    // أوامر REPLACE / ADD_TO
    const modifyRegex = /<(REPLACE|ADD_TO)\s+(?:name|file|target)="([^"]+)"(?:\s+position="([^"]+)")?\s*>([\s\S]*?)<\/\1>/gi;
    while ((match = modifyRegex.exec(fullText)) !== null) {
        ...
        if (!targetFile) {
            targetFile = { name: fileName, content: '' };
            projectFiles.push(targetFile);
        }
...
function applyQuickReplace(original, diffBlock) {
    ...
    if (existingIndex !== -1) {
        // تحديث المحتوى بالكامل - بدون شروط
        projectFiles[existingIndex].content = content;
        
        // تحديث المحرر إذا كان هذا هو الملف النشط
        if (existingIndex === activeFileIndex) {
            ...
            // codeArea.setSelectionRange(selectionStart, selectionStart); // اختياري
        }
    } else {
        // إنشاء ملف جديد
    ...
    // --- SSE Handling ---
    // --- SSE Handling with Auto-Reconnect ---
let sse = null;
let sseReconnectAttempts = 0;
let sseReconnectTimeout = null;
let sseIsIntentionalClose = false;

function initSSE() {
    if (sse) {
        sseIsIntentionalClose = true;
        sse.close();
        sse = null;
    }
    sseIsIntentionalClose = false;
    
    const url = RENDER_SERVER_URL + '/api/events?clientId=' + myClientId;
    console.log("🔌 Initializing SSE connection:", url);
    ...
    // تأكد من إنهاء الرسالة الحالية
    if (currentAiMsgElement) {
        const contentEl = currentAiMsgElement.querySelector('.ai-content');
        if (contentEl) {
            const currentText = contentEl.innerHTML;
            contentEl.innerHTML = currentText + '<br><br><em style="color: #ff9800;">Response stopped</em>';
        }
    }
    serverFinished = true;
    window.fastTyping = true;
    ...
    break;
                ...
                case 'conversation_summary':
                    ...
                    break;
                case 'task_type_detected':
                     handleTaskTypeDetected(payload);
                    break;
                case 'continuation_started':
                      handleContinuationStarted(payload);
                    break;
                case 'continuation_completed':
                     handleContinuationCompleted(payload);
                    break;
                case 'search_status':
    if (payload.status === 'searching') {
        // عرض "Searching..." مكان الـ stage
        removeStatus();
        showStatus("Searching");
    } else if (payload.status === 'completed') {
        removeStatus();
        // إذا كان هناك status سابق، استعده أو تجاهل
        updateStatusText("Thinking"); // أو أي حالة تالية
        ...
        updateStatusText("Thinking");
                    ...
                    break;
               case 'search_results':
    console.log("📥 [SSE] search_results arrived:", payload.results?.length, "items");
    currentSearchResults = payload.results;
    
    // استدعاء فوري
    if (currentAiMsgElement && currentSearchResults?.length) {
        setTimeout(() => {
            addSearchSourcesButton(currentAiMsgElement, currentSearchResults);
        }, 200);
    }
    ...
    sse.onerror = (err) => {
        ...
        if (sseIsIntentionalClose) return;
...
// Listen for online/offline events to reconnect immediately when network returns
...
window.addEventListener('offline', () => {
    console.log("📡 Network offline, SSE will reconnect when online returns");
    // No need to close; EventSource will error and our reconnection logic will handle it
});
...
// ==========================================
// دالة معالجة استمرار الـ Stream
// ==========================================
// ==========================================
// معالجة حدث بدء الاستمرار
// ==========================================
function handleContinuationStarted(payload) {
    console.log(`🔄 [CONTINUATION] Started: ${payload.primaryModel} → ${payload.continuationModel}`);
    
    if (!currentTaskContainer) return;
    
    // تحديث رأس قائمة المهام ليعرض النماذج
    const header = currentTaskContainer.querySelector('.task-list-header');
    if (!header) return;
    
    // إخفاء المؤقت القديم
    const oldTimerInfo = header.querySelector('.timer-info');
    if (oldTimerInfo) {
        oldTimerInfo.style.display = 'none';
    }
    
    // إنشاء أو تحديث عرض النماذج
    let modelsStack = header.querySelector('.models-stack');
    if (!modelsStack) {
        modelsStack = document.createElement('div');
        modelsStack.className = 'models-stack';
        header.appendChild(modelsStack);
    }
    
    // النموذج الأول - مكتمل (علامة صح)
    modelsStack.innerHTML = `
        <div class="model-row completed">
            <svg class="model-status-icon check" viewBox="0 0 24 24" fill="none">
            ...
            <span class="model-name">${payload.primaryModel}</span>
        </div>
        <div class="model-separator">│</div>
        <div class="model-row running">
            <span class="spinner"></span>
            <span class="model-name">${payload.continuationModel}</span>
        </div>
    `;
    
    // إظهار الحاوية
    modelsStack.style.display = 'block';
}

// ==========================================
// معالجة حدث اكتمال الاستمرار
// ==========================================
function handleContinuationCompleted(payload) {
    console.log(`✅ [CONTINUATION] Completed: ${payload.primaryModel} + ${payload.continuationModel}`);
    ...
    const modelsStack = header.querySelector('.models-stack');
    if (!modelsStack) return;
    
    // تحديث النموذج الثاني ليكون مكتملاً
    const runningRow = modelsStack.querySelector('.model-row.running');
    if (runningRow) {
        runningRow.classList.remove('running');
        runningRow.classList.add('completed');
        
        // استبدال السبينر بعلامة صح
        const spinner = runningRow.querySelector('.spinner');
        if (spinner) {
            spinner.outerHTML = `
                <svg class="model-status-icon check" viewBox="0 0 24 24" fill="none">
                    <circle cx="12" cy="12" r="10" stroke="#4caf50" stroke-width="1.5"/>
    ...
    // إظهار المؤقت النهائي (مدة التشغيل)
    const elapsed = Math.floor((Date.now() - (currentTimerStartTime || Date.now())) / 1000);
    const timerInfo = header.querySelector('.timer-info');
    if (timerInfo) {
        timerInfo.style.display = 'flex';
        timerInfo.innerHTML = `
            <span>Completed</span>
            <span class="running-time">Ran for ${elapsed}s</span>
        `;
    }
}

// ==========================================
// تحديث دالة createTaskListContainer
// ==========================================
function createTaskListContainer(msgElement) {
   
   if (currentTaskType === 'general') {
        console.log("📋 Skipping task list for general conversation");
        return null;
    }
    ...
    // حاوية النماذج (مخفية في البداية)
    const modelsStack = document.createElement('div');
    modelsStack.className = 'models-stack';
    modelsStack.style.display = 'none';
...
// ==========================================
// تحديث SSE handler في handleAssistantMessage
// ==========================================
// أضف هذه الحالات في switch(payload.type):




function handleAssistantMessage(payload) {
    console.log("🔍 [DEBUG] handleAssistantMessage CALLED");
    console.log("🔍 [DEBUG] payload:", payload);
    console.log("🔍 [DEBUG] streamingConvId:", streamingConvId);
    console.log("🔍 [DEBUG] activeId:", activeId);
    console.log("🔍 [DEBUG] safeBuffer.length:", safeBuffer.length);
    // 1. تحديد المحادثة المستهدفة (التي بدأنا فيها التوليد)
    const targetConv = convs.find(c => c.id === streamingConvId);
    if (!targetConv) {
        console.warn("🔍 [DEBUG] No targetConv found for streamingConvId:", streamingConvId);
        return;
    }
    console.log("🔍 [DEBUG] targetConv found");
    
    const isBackground = (activeId !== streamingConvId);
    console.log("🔍 [DEBUG] isBackground:", isBackground);//;تخدم في محادثة أخرى؟
     // مؤشر لوصول رد من النموذج
    console.log("📨 handleAssistantMessage called, streamingConvId:", streamingConvId, "activeId:", activeId);
  
    if (payload.text && payload.text !== '\n[STREAM COMPLETE]' && !hasReceivedModelResponse) {
        hasReceivedModelResponse = true;
        console.log("✅ Model response started - usage will be counted");
    }
    // في بداية handleAssistantMessage، أضف هذا
if (safeBuffer.length === 0 && payload.text && payload.text !== '\n[STREAM COMPLETE]') {
    if (!isBackground) {
        ...
        if (!currentAiMsgElement) {
            ...
            // ✅ إدراج بعد status مباشرة (وليس قبله)
            if (currentStatusEl && currentStatusEl.parentNode) {
                currentStatusEl.parentNode.insertBefore(d, currentStatusEl.nextSibling);
            } else {
                messagesEl.appendChild(d);
            }
...
// --- منطق المعاينة والكونسول ---
    ...
    // ترتيب الأولويات
    const priorityNames = ['game.js', 'index.js', 'main.js', 'script.js', 'app.js'];
    
    for (const name of priorityNames) {
        const found = jsFiles.find(f => f.name.toLowerCase() === name.toLowerCase());
        if (found) return found;
    }
    
    // ابحث عن ملف يحتوي على init, start, addEventListener
    const found = jsFiles.find(f => 
        f.content.includes('function init') || 
        f.content.includes('function start') ||
        f.content.includes('addEventListener') ||
        f.content.includes('document.addEventListener')
    );
    if (found) return found;
    
    // أكبر ملف (غالباً هو الرئيسي)
    return jsFiles.reduce((a, b) => 
...
function cleanModuleCode(code) {
    ...
    // ===== إزالة جميع أنواع import =====
    // import x from 'y'
    cleaned = cleaned.replace(/import\s+[\w\s{},*]+\s+from\s+['"][^'"]+['"]\s*;?/g, '');
    // import('y')
    cleaned = cleaned.replace(/import\s*\([^)]+\)/g, '');
    // import 'y'
    cleaned = cleaned.replace(/import\s+['"][^'"]+['"]\s*;?/g, '');
    
    // ===== إزالة جميع أنواع export =====
    // export { x, y }
    ...
    cleaned = cleaned.replace(/export\s+default\s+class/g, 'class');
    // export function
    cleaned = cleaned.replace(/export\s+function/g, 'function');
    // export const
    cleaned = cleaned.replace(/export\s+const/g, 'const');
    // export let
    cleaned = cleaned.replace(/export\s+let/g, 'let');
    // export var
    cleaned = cleaned.replace(/export\s+var/g, 'var');
    // export class
    cleaned = cleaned.replace(/export\s+class/g, 'class');
    // export type (TypeScript)
    cleaned = cleaned.replace(/export\s+type/g, 'type');
    // export interface (TypeScript)
    cleaned = cleaned.replace(/export\s+interface/g, 'interface');
    // export enum (TypeScript)
    cleaned = cleaned.replace(/export\s+enum/g, 'enum');
    // export * from 'y'
    cleaned = cleaned.replace(/export\s*\*\s*from\s+['"][^'"]+['"]\s*;?/g, '');
    // export { x } from 'y'
    cleaned = cleaned.replace(/export\s*\{\s*[^}]*\s*\}\s*from\s+['"][^'"]+['"]\s*;?/g, '');
    
    // أي كلمة export متبقية (كحالة أخيرة)
...
function updatePreviewTitle() {
    ...
    // ابحث عن ملف HTML واحصل على عنوانه
...
`;
    ...
    let bundledJs = roundRectPolyfill + '\n';
    
    // ===== نظام Source Maps =====
    // مصفوفة لتخزين معلومات كل ملف
    const sourceMapInfo = [];
    let totalLinesBefore = 0;
    
    // دالة تنظيف الكود من import/export (شاملة)
    function cleanModuleCode(code) {
        let cleaned = code;
    ...
    // دالة لإضافة ملف مع تتبع الأسطر
    function addFileToBundle(fileName, content) {
        ...
        sourceMapInfo.push({
            name: fileName,
            startLine: totalLinesBefore + 1,
            endLine: totalLinesBefore + lines,
            lines: lines
        });
        ...
        bundledJs += `// ===== ${fileName} =====\n`;
        bundledJs += cleanedContent + '\n\n';
    }
    
    // utils.js أولاً
    const utilsFile = jsFiles.find(f => f.name.toLowerCase().includes('utils'));
    if (utilsFile) {
        addFileToBundle(utilsFile.name, utilsFile.content);
    }
    
    // باقي الملفات
    jsFiles.forEach(f => {
        if (f === utilsFile) return;
        addFileToBundle(f.name, f.content);
    });

    // ===== إنشاء دالة البحث عن الملف الأصلي =====
    const sourceMapCode = `
// Source Map for debugging
window.__sourceMap = ${JSON.stringify(sourceMapInfo)};

window.__getOriginalLocation = function(line) {
    const sourceMap = window.__sourceMap;
    for (let i = 0; i < sourceMap.length; i++) {
        const file = sourceMap[i];
        if (line >= file.startLine && line <= file.endLine) {
            const originalLine = line - file.startLine + 1;
            return { file: file.name, line: originalLine };
        }
    }
    return { file: 'unknown', line: line };
};
`;

    bundledJs = sourceMapCode + '\n' + bundledJs;

    // ===== Console forwarding script مع Source Maps =====
    const consoleScript = `
<script>
(function(){
    ...
    // دالة تنسيق الخطأ مع اسم الملف ورقم السطر الأصلي
    function formatErrorWithSourceMap(message, url, line, col) {
        if (window.__getOriginalLocation && line) {
            const loc = window.__getOriginalLocation(line);
            if (loc.file !== 'unknown') {
    ...
    window.onerror = function(msg, url, line, col, error) {
        ...
        if (line) {
            formattedMsg = formatErrorWithSourceMap(msg, url, line, col);
        } else {
            formattedMsg = "❌ " + msg;
        }
        if (col) formattedMsg += " [Col: " + col + "]";
    ...
    window.addEventListener('unhandledrejection', function(event) {
        ...
        var msg = "⚠️ Unhandled Promise Rejection: " + (reason?.message || reason);
...
<\/script>`;
    ...
    if (cssFiles.length > 0) {
        let cssBlock = '<style>\n';
        cssFiles.forEach(f => cssBlock += `/* ${f.name} */\n${f.content}\n`);
        cssBlock += '</style>';
        if (finalHtml.includes('</head>')) {
...
<\/script>`;
    ...
    if (finalHtml.includes('</body>')) {
        finalHtml = finalHtml.replace('</body>', bundledScript + '\n</body>');
    } else {
...
async function runInSandbox(language) {
    const currentFile = projectFiles[activeFileIndex];
    if (!currentFile) return;

    showStatus("Creating sandbox...");

    try {
        const response = await fetchWithAuth(`${API_BASE_URL}/api/sandbox/create`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        if (data.success) {
            // عرض الناتج في شاشة الكونسول بدلاً من preview
            const consoleView = document.getElementById('consoleOutputView');
            if (consoleView && data.output) {
                consoleView.style.display = 'block';
                const div = document.createElement('div');
                div.className = 'console-log-item log';
            ...
            if (data.output) {
                ...
                if (consoleView) {
                    consoleView.style.display = 'block';
                    const div = document.createElement('div');
                    div.className = 'console-log-item error';
                    div.style.whiteSpace = 'pre-wrap';
                    div.textContent = data.output;
                    consoleView.appendChild(div);
                }
...
function findMainJsFile() {
    ...
    // ابحث عن game.js أولاً
    let main = jsFiles.find(f => f.name.toLowerCase() === 'game.js');
    if (main) return main;
    
    // ثم index.js
    main = jsFiles.find(f => f.name.toLowerCase() === 'index.js');
    if (main) return main;
    
    // ثم main.js
    main = jsFiles.find(f => f.name.toLowerCase() === 'main.js');
    if (main) return main;
    
    // أخيراً، أي ملف JS يحتوي على init أو start أو main
    main = jsFiles.find(f => 
        f.content.includes('function init') || 
        f.content.includes('function start') ||
        f.content.includes('addEventListener')
...
function detectLanguage() {
    ...
    if (files.some(f => f.name === 'package.json' && 
        (f.content.includes('react') || f.content.includes('next')))) {
        return 'react';
    }
    if (files.some(f => f.name.endsWith('.vue'))) return 'vue';
    if (files.some(f => f.name.endsWith('.py'))) return 'python';
    if (files.some(f => f.name.endsWith('.go'))) return 'go';
    if (files.some(f => f.name.endsWith('.rs'))) return 'rust';
    if (files.some(f => f.name === 'pubspec.yaml')) return 'flutter';
...
// ============================================================
    ...
    document.addEventListener('touchmove', (e) => {
      ...
      if (e.target.closest('.examples-horizontal-scroll')) {
        return;   // لا نمنع الحدث، نتركه للمتصفح
    }
    
    // في بداية كل مستمع touchmove
const authScreen = document.getElementById('welcomeAuthScreen');
if (authScreen && authScreen.classList.contains('open')) {
    return; // اسمح بالتمرير الطبيعي
}
    
    if (e.target.closest('#welcomeAuthScreen')) return;
...
function recordChange(newContent) {
    if (historyStack.length === 0 || historyStack[historyStack.length - 1] !== newContent) {
        historyStack.push(newContent);
        if (historyStack.length > MAX_HISTORY) historyStack.shift();
        redoStack = []; // مسح redo لأن التغيير جديد
    }
}
...
const ext = mimeToExt[blobMime] || 'webm';
      ...
      const res = await fetchWithAuth(RENDER_SERVER_URL + '/api/stt', {
        method: 'POST',
        body: fd
      });
...
(async function init() {
  // التحقق من رابط المشاركة
const path = window.location.pathname;
if (path.startsWith('/share/')) {
    const shareCode = path.split('/').pop();
    if (shareCode && shareCode !== 'share') {
        console.log('🔗 Loading shared message with code:', shareCode);
        // منع تحميل المحادثات العادية
        // نستدعي loadSharedMessage مباشرة
        await loadSharedMessage(shareCode);
        return; // نخرج مبكراً حتى لا يتم تحميل المحادثات العادية
    }
}
    // تحميل المحادثات من السيرفر
    const isRegistered = localStorage.getItem('codeai_nickname') || await checkUserRegistration();
    if (isRegistered) {
        await loadConversations();
    }
    ...
    // ✅ تحميل الحدود اليومية (أضف هنا)
    await fetchUserLimits();
    createLimitProgressBar();
...
// ==========================================
// إصلاح التمرير في شاشة التسجيل - الحل النهائي
// ==========================================

// هذه الدالة تضمن أن التمرير يعمل على شاشة التسجيل فقط
function fixAuthScreenScroll() {
    const authScreen = document.getElementById('welcomeAuthScreen');
    if (!authScreen) return;
    
    // منع انتشار حدث اللمس من العناصر الداخلية إلى body
    authScreen.addEventListener('touchstart', function(e) {
        e.stopPropagation();
    }, { passive: false });
    
    authScreen.addEventListener('touchmove', function(e) {
      // في بداية كل مستمع touchmove
...
if (authScreen && authScreen.classList.contains('open')) {
    ...
    // التأكد من أن التمرير يعمل
    authScreen.style.overflowY = 'auto';
    authScreen.style.overflowX = 'hidden';
    authScreen.style.webkitOverflowScrolling = 'touch';
    
    console.log("✅ Auth screen scroll fixed");
}

// استدعاء الدالة عند فتح شاشة التسجيل
const originalOpenAuthScreen = () => {
    const authScreen = document.getElementById('welcomeAuthScreen');
    if (authScreen && authScreen.classList.contains('open')) {
        fixAuthScreenScroll();
    }
...
// مراقبة التغييرات في كلاس open




// ==========================================
// إصلاح التمرير باللمس بشكل نهائي
// ==========================================

function fixTouchScroll() {
    const authScreen = document.getElementById('welcomeAuthScreen');
    if (!authScreen || authScreen._scrollFixed) return;
    authScreen._scrollFixed = true;

    // تأكد من الإعدادات مرة واحدة
    authScreen.style.touchAction = 'pan-y';
    authScreen.style.overflowY = 'auto';
    authScreen.style.webkitOverflowScrolling = 'touch';

    // لا حاجة لإضافة مستمعين متكررين
    console.log("✅ Touch scroll fixed once");
}

// استدعاء الدالة عند فتح الشاشة


// أيضاً، قم بتعديل دالة moveVideoToAuthScreen لإضافة التمرير بعد الفتح
/*const originalMoveVideo = transitionToAuthScreen;
if (typeof originalMoveVideo === 'function') {
    window.transitionToAuthScreen = function() {
        originalMoveVideo();
...
authScreen.addEventListener('touchmove', (e) => {
    e.stopPropagation(); // يمنع وصول الحدث إلى document حيث توجد preventDefault
}, { passive: false });

window.debugAuthScroll = function() {
    const el = document.getElementById('welcomeAuthScreen');
...
/**
 * تشغيل أنميشن ظهور تدريجي لعناصر الشريحة الأولى
 */


// ===== نظام التنقل بين شرائح التسجيل (محسّن) =====
function initOnboardingSlides() {
    const slides = document.querySelectorAll('.onboarding-slide-content');
    const nextBtn = document.getElementById('nextSlideBtn');
    const dots = document.querySelectorAll('.dot');
    
    if (!slides.length || !nextBtn) {
        console.warn('Onboarding elements not ready');
        return false;
    }
    
    // منع التهيئة المكررة
    if (window._onboardingReady) return true;
    window._onboardingReady = true;
   ...
   // slides[0].style.display = 'flex'; // تأكد من أنها مرئية
    
    // إخفاء جميع الشرائح أولاً
// إخفاء جميع الشرائح أولاً
slides.forEach(slide => slide.style.display = 'none');

// تفعيل الشريحة الأولى فقط
slides[0].style.display = 'block';   // أو 'flex' حسب التصميم
slides[0].classList.add('active');
currentSlide = 0;
updateUI(0);
    
    // إخفاء جميع الشرائح في البداية
slides.forEach(slide => slide.style.display = 'none');
    ...
    function updateUI(index) {
        ...
        else if (index === 1) nextBtn.textContent = 'Next →';
        else nextBtn.textContent = 'Create Account';
        
        // إظهار/إخفاء الفيديو العلوي
        const topVideo = document.getElementById('transferredVideoContainer');
if (topVideo) {
    // الفيديو العلوي يظهر فقط في الشريحة الأولى (index === 0)
    topVideo.style.display = index === 0 ? 'flex' : 'none';
}
        
        // تحميل اللعبة عند الوصول للشريحة الثانية
        if (index === 1) {
    if (!window._gameInitialized) {
        // setupExampleGame(); // لم نعد بحاجة لهذا
        window._gameInitialized = true;
    }
    // تهيئة البطاقات ورسم المعاينات المصغرة
    setTimeout(() => {
        initExampleCards();
        drawMiniPreviews();
    }, 100);
    ...
    const prevSlideEl = currentSlide !== -1 ? slides[currentSlide] : null;

    // إظهار الشريحة الجديدة (لكن بشفافية للأنيميشن)
    nextSlideEl.style.display = 'block';
    nextSlideEl.classList.add('entering');

    requestAnimationFrame(() => {
        if (prevSlideEl) {
            prevSlideEl.classList.add('hiding');
            prevSlideEl.classList.remove('active');
        }

        nextSlideEl.classList.remove('entering');
        nextSlideEl.classList.add('active');

        // بعد 300ms (مدة الأنيميشن) – إخفاء الشريحة السابقة تماماً
        setTimeout(() => {
            if (prevSlideEl) {
                prevSlideEl.style.display = 'none';      // ✅ هذا السطر هو الحل
                prevSlideEl.classList.remove('hiding');
            }
    ...
    // السحب (swipe) - على المحتوى فقط وليس على الثوابت
    const container = document.querySelector('.onboarding-content-container');
    let touchStartX = 0;
    ...
    document.addEventListener('keydown', (e) => {
        if (!document.getElementById('welcomeAuthScreen')?.classList.contains('open')) return;
        if (e.key === 'ArrowRight') goToSlide(currentSlide + 1);
        else if (e.key === 'ArrowLeft') goToSlide(currentSlide - 1);
    });
    
    updateUI(0);
    console.log('✅ Onboarding slides initialized (content-only transition)');
    return true;
}

/**
 * تشغيل أنميشن ظهور تدريجي لعناصر الشريحة الأولى
 * بنفس منطق entering → active
 */
function animateFirstSlideElements() {
    const slide1 = document.getElementById('slideContent1');
    if (!slide1) return;
    
    const elements = [
        slide1.querySelector('.welcome-pre-title'),
        slide1.querySelector('.welcome-message-first'),
        slide1.querySelector('.welcome-auth-second-video'),
        slide1.querySelector('.welcome-message-second')
    ].filter(el => el !== null);
    
    // تحويل كل عنصر من entering إلى active بتأخير متزايد
...
// دالة إعداد اللعبة المصغرة في الشريحة الثانية
function setupExampleGame() {
    const canvas = document.getElementById('exampleGameCanvas');
    if (!canvas) return;
    
    const ctx = canvas.getContext('2d');
    let bird = { y: 200, vy: 0 };
    let pipes = [{ x: 300, top: 150 }];
    let score = 0;
    ...
    function update() {
        ...
        if (pipes[0].x < -50) {
            pipes.shift();
            pipes.push({ x: 350, top: Math.random() * 150 + 70 });
            score++;
        }
        
        const pipe = pipes[0];
        if (bird.y > 380 || bird.y < 0) gameRunning = false;
        if (pipe.x < 70 && pipe.x > 20) {
            if (bird.y < pipe.top || bird.y > pipe.top + 120) gameRunning = false;
        }
    }
    ...
    function draw() {
        ...
        pipes.forEach(p => {
            ...
            ctx.strokeRect(p.x, p.top + 120, 30, 400);
        });
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.ellipse(50, bird.y, 15, 12, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.stroke();
        
        ctx.fillStyle = '#FFF';
        ctx.beginPath();
        ctx.arc(55, bird.y - 3, 5, 0, Math.PI * 2);
        ctx.fill();
        ctx.stroke();
        ctx.fillStyle = '#000';
        ctx.beginPath();
        ctx.arc(57, bird.y - 3, 2, 0, Math.PI * 2);
        ctx.fill();
        ...
        if (!gameRunning) {
            ...
            ctx.fillText('Tap to restart', 150, 200);
            ctx.textAlign = 'left';
        }
    }
...
// استدعاء الدالة عند تحميل الصفحة أو عند فتح شاشة التسجيل
setTimeout(() => {
    if (document.getElementById('welcomeAuthScreen')?.classList.contains('open')) {
        initOnboarding();
    }
}, 300);

// ربطها بدالة الانتقال الأصلية
const originalTransition = window.transitionToAuthScreen;
...
const examplesData = {
    flappy: {
        title: 'Flappy Bird',
        userMsg: 'Create a simple flappy bird like game',
        aiResponse: `Hey there! 🚀 I'm excited to help you build this Flappy Bird clone. I've designed it with a sleek dark aesthetic, smooth physics, and some extra juice like screen shakes and particle effects to make it feel really polished.

Here's what I've packed into this version:
1.  **Smooth Physics**: The bird has realistic gravity and rotation based on its velocity.
2.  **Dynamic Background**: A scrolling parallax-style background to give a sense of speed.
3.  **Particle System**: When you hit a pipe, the bird "explodes" into particles for a more impactful game over.
4.  **Screen Shake**: A subtle camera shake when you lose to add feedback.
5.  **High Score System**: Your best score is saved locally so you can always try to beat it.
6.  **Responsive Design**: Works perfectly on both desktop (Space/Click) and mobile (Tap).

Let's get into the code! 📄✨`,
        htmlCode: `<!DOCTYPE html>
<html>
<head><style>canvas{background:#70C5CE;}</style></head>
<body>
  <canvas id="gameCanvas" width="300" height="400"></canvas>
  <script src="game.js"></script>
</body>
</html>`,
        jsCode: `// Flappy Bird game logic
const canvas = document.getElementById('gameCanvas');
const ctx = canvas.getContext('2d');
let bird = { y: 200, vy: 0 };
let pipes = [{ x: 300, top: 150 }];
...
gameLoop();`
    ...
    snake: {
        title: 'Snake',
        userMsg: 'Build a classic snake game',
        aiResponse: '🐍 Here is a classic Snake game implemented in JavaScript.',
        htmlCode: `<canvas id="snakeCanvas" width="300" height="300"></canvas>`,
        jsCode: `// Snake game
...
`
    ...
    doc: {
        title: 'Word document',
        userMsg: 'Create a word document about AI',
        aiResponse: `🚀 I've crafted a comprehensive Word document all about the fascinating world of Artificial Intelligence! ✨ It covers everything from its core technologies to the ethical considerations of the future. I've made sure it's substantial and well-structured with a prominent header section just for you! 📄

What would you like to do next?
1️⃣ Add a specific section on Generative AI (like ChatGPT)?
2️⃣ Change the tone to be more academic or more casual?
3️⃣ Add a table of contents at the beginning?
4️⃣ Include specific AI use cases for a particular industry?

Just let me know what you'd like to adjust! 😊`,
        docUrl: 'https://slchljnlapixdtxqucny.supabase.co/storage/v1/object/public/documents/c811071a-f166-4b65-9d19-81bd1f29596e/1776946919634/file_1776946938623.docx'
    }
};

// فتح modal مع تفاصيل المثال
function openExampleModal(exampleId) {
    const modal = document.getElementById('exampleDetailModal');
    const body = document.getElementById('exampleDetailBody');
    const data = examplesData[exampleId];
    if (!data) return;

    body.innerHTML = `
        <h2 style="margin-bottom:16px; text-align:center;">${data.title}</h2>
        <div class="chat-demo">
            <div class="chat-bubble user">${data.userMsg}</div>
<div class="chat-bubble ai">${marked.parse(data.aiResponse)}</div>
        </div>

            <div class="preview-mini">
                <div class="preview-header"> Preview</div>
                <div class="game-preview-container" id="modalGamePreview">
                    <!-- سيتم حقن اللعبة هنا -->
                </div>
                <p class="preview-hint">👆 Interact with the preview</p>
            </div>
        </div>
        <p class="example-footnote">Generated in seconds with smooth physics.</p>
    `;

    // تشغيل المعاينة داخل المودال
    setTimeout(() => {
        const previewContainer = document.getElementById('modalGamePreview');
        if (exampleId === 'flappy') {
    previewContainer.innerHTML = '';
    const iframe = document.createElement('iframe');
    iframe.style.cssText = 'width:100%; height:450px; border:none;';   // ضع ارتفاعًا ثابتًا وليس 100%
    iframe.src = 'examples/bird/index.html';
    previewContainer.appendChild(iframe);
    } else if (exampleId === 'snake') {
            // ...
    } else if (exampleId === 'doc') {
        // ✅ عرض ملف DOCX باستخدام Google Docs Viewer
        previewContainer.innerHTML = '';
        
        const docUrl = data.docUrl || data.url;
        if (docUrl) {
            // استخدام Google Docs Viewer للمعاينة
            const viewerUrl = `https://docs.google.com/gview?embedded=true&url=${encodeURIComponent(docUrl)}`;
            ...
            // إذا لم يوجد رابط، أظهر رسالة
            previewContainer.innerHTML = '<div style="padding:20px; text-align:center; color:var(--text-color);">Document preview not available</div>';
        }
    }
    }, 100);
...
// تهيئة Flappy Bird في modal
function initFlappyGame(canvasId) {
    const canvas = document.getElementById(canvasId);
    if (!canvas) return;
    // نفس كود اللعبة الموجود في setupExampleGame ولكن باستخدام canvasId المحدد
    // يمكن استدعاء دالة موحدة
    setupFlappyGame(canvas);
}

// تهيئة النقر على البطاقات
function initExampleCards() {
    const cards = document.querySelectorAll('.example-card-item');
    cards.forEach(card => {
        card.addEventListener('click', () => {
            const exampleId = card.getAttribute('data-example');
            openExampleModal(exampleId);
        });
    });
}

// إغلاق modal عند النقر على زر الإغلاق أو خارج المحتوى
document.addEventListener('click', (e) => {
    const modal = document.getElementById('exampleDetailModal');
    if (!modal) return;
    if (e.target.classList.contains('example-detail-close') || e.target === modal) {
        closeExampleModal();
    }
});

// استدعاء التهيئة عند فتح الشريحة الثانية (يمكن ربطها بـ goToSlide)
// سنقوم بتعديل updateUI في initOnboardingSlides لاستدعاء initExampleCards عند index === 1


function drawMiniPreviews() {
    // رسم Flappy Bird مصغر
    const flappyPreview = document.querySelector('.example-card-item[data-example="flappy"] .example-preview-mini');
if (flappyPreview) {
    flappyPreview.innerHTML = '';
    const img = document.createElement('img');
    img.src = 'assets/bird-pic.png';
    img.style.cssText = 'width:100%;height:100%;object-fit:cover;';
    flappyPreview.appendChild(img);
}
    // رسم Snake مصغر
    const snakeCanvas = document.getElementById('miniSnakeCanvas');
    if (snakeCanvas) {
...
// ==========================================
// إصلاح السكرول الأفقي في شريحة الأمثلة
// ==========================================

// 1. منع المستمعات العامة من منع الحدث داخل الحاوية الأفقية
function fixGlobalTouchListeners() {
    // تخزين المستمعات الأصلية مؤقتاً
    const originalMenuHandler = document._menuTouchHandler;
    const originalCodeHandler = document._codeTouchHandler;
    
    // إزالة القديمة
    if (originalMenuHandler) document.removeEventListener('touchmove', originalMenuHandler);
    if (originalCodeHandler) document.removeEventListener('touchmove', originalCodeHandler);
    
    // إضافة جديدة مع شرط الاستثناء
    const newMenuHandler = (e) => {
        if (e.target.closest('.examples-horizontal-scroll')) return;
        if (originalMenuHandler) originalMenuHandler(e);
    };
    
    const newCodeHandler = (e) => {
        if (e.target.closest('.examples-horizontal-scroll')) return;
        if (originalCodeHandler) originalCodeHandler(e);
    };
    
    document.addEventListener('touchmove', newMenuHandler, { passive: false });
    document.addEventListener('touchmove', newCodeHandler, { passive: false });
    
    document._menuTouchHandler = newMenuHandler;
    document._codeTouchHandler = newCodeHandler;
}

// 2. منع تكرار initScrollTrigger
let scrollTriggerFixed = false;
const originalInitScrollTrigger = window.initScrollTrigger;
window.initScrollTrigger = function() {
    if (scrollTriggerFixed) return;
    scrollTriggerFixed = true;
    if (originalInitScrollTrigger) originalInitScrollTrigger();
};

// 3. إضافة touch-action للحاوية الأفقية
const style = document.createElement('style');
style.textContent = `
...
// تطبيق الإصلاحات
setTimeout(() => {
    fixGlobalTouchListeners();
    console.log("✅ Fixed horizontal scroll listeners");
}, 500);

// ==========================================
// الحل النهائي للسكرول الأفقي - override كامل
// ==========================================

(function fixHorizontalScrollDefinitively() {
    // 1. تعطيل كل مستمعات touchmove القديمة مؤقتاً
    const originalAddEventListener = document.addEventListener;
    let blocked = false;
    
    // 2. منع أي مستمع جديد من منع الحدث داخل الحاوية الأفقية
    document.addEventListener = function(type, listener, options) {
        if (type === 'touchmove') {
            const wrappedListener = function(e) {
                if (e.target.closest('.examples-horizontal-scroll')) {
                    // السماح بالسكرول دون أي تدخل
                    return;
                }
                return listener(e);
            };
    ...
    // 3. إزالة المستمعات الموجودة يدوياً (بقدر الإمكان)
    // لا يمكن إزالة المستمعات المجهولة، لكننا نضيف مستمعاً ذا أولوية عالية
    const highPriorityHandler = function(e) {
        if (e.target.closest('.examples-horizontal-scroll')) {
            e.stopImmediatePropagation();
            // لا نمنع الحدث، نتركه للمتصفح
        }
    };
    document.addEventListener('touchmove', highPriorityHandler, { capture: true, passive: false });
    
    // 4. إضافة CSS إضافي لضمان السلوك
    ...
    extraStyle.textContent = `
        ...
        .examples-horizontal-scroll::-webkit-scrollbar {
            display: none !important;
        }
        /* منع أي عنصر أب من منع التمرير */
        #slideContent2, 
    ...
    console.log("✅ Horizontal scroll - FINAL FIX applied");
})();

// ==========================================
// تحسين السكرول الأفقي - نهائي ومخصص للشريحة الثانية فقط
// ==========================================
(function finalHorizontalScrollFix() {
    const scrollContainer = document.querySelector('.examples-horizontal-scroll');
    if (!scrollContainer) return;

    // 1. منع التنقل بين الشرائح عند السحب داخل الحاوية الأفقية
    const originalGoToSlide = window.goToSlide;
    const container = document.querySelector('.onboarding-content-container');
    if (container) {
        let touchStartX = 0;
        let isHorizontalScrollIntent = false;

        container.addEventListener('touchstart', (e) => {
            if (e.target.closest('.examples-horizontal-scroll')) {
                touchStartX = e.touches[0].clientX;
                isHorizontalScrollIntent = true;
                // تعطيل scroll-snap مؤقتًا لتحسين السحب
                scrollContainer.style.scrollSnapType = 'none';
            } else {
                isHorizontalScrollIntent = false;
            }
        }, { passive: true });

        container.addEventListener('touchend', (e) => {
            if (isHorizontalScrollIntent && scrollContainer) {
                const diff = touchStartX - e.changedTouches[0].clientX;
                // إذا كانت الحركة أفقية وكافية، نمنع تغيير الشريحة
                if (Math.abs(diff) > 20) {
                    e.stopImmediatePropagation();
                }
                // إعادة scroll-snap بعد انتهاء السحب
                setTimeout(() => {
    ...
    // 2. منع أي مستمع عام من منع الحدث داخل الحاوية الأفقية
    const highPriorityHandler = function(e) {
    const target = e.target.closest('.examples-horizontal-scroll');
    if (target && target.closest('#slideContent2')) {
        // داخل الحاوية الأفقية في الشريحة الثانية فقط – نسمح بالسكرول
        e.stopImmediatePropagation(); // منع المستمعات الأخرى من التدخل
        // لا نستدعي preventDefault
    } else {
        // خارج الحاوية – نمنع أي تأثير (اختياري)
        // يمكنك هنا منع السحب الأفقي إذا أردت
        if (Math.abs(e.touches[0].clientX - startX) > 10) {
            e.preventDefault(); // يمنع السحب الأفقي في باقي الصفحات
        }
    }
};
document.addEventListener('touchmove', highPriorityHandler, { capture: true, passive: false });

    // 3. إجبار الخصائص CSS بشكل آمن
    const style = document.createElement('style');
    style.textContent = `
        .examples-horizontal-scroll {
            overflow-x: auto !important;
            ...
            scroll-snap-type: none !important; /* مؤقتًا، سنعيدها عبر JS بعد السحب */
            -webkit-overflow-scrolling: touch !important;
        }
        /* التأكد من أن العناصر الأب لا تمنع السكرول */
        #slideContent2, .onboarding-slide-content, .onboarding-content-container {
            overflow: visible !important;
        }
    `;
    document.head.appendChild(style);

    console.log("✅ Horizontal scroll optimized - works only in slide 2");
})();


// إخفاء الشريحة الثانية عند مغادرتها
(function fixSlide2Visibility() {
    ...
    if (originalGoToSlide) {
        window.goToSlide = function(index) {
            ...
            // إخفاء الشريحة الثانية نهائياً إذا لم تكن النشطة
            const slide2 = document.getElementById('slideContent2');
            const activeSlide = document.querySelector('.onboarding-slide-content.active');
            
            if (slide2 && (!activeSlide || activeSlide.id !== 'slideContent2')) {
            ...
            // التأكد من أن الشريحة النشطة مرئية
            if (activeSlide && activeSlide.id === 'slideContent2') {
    ...
    // إخفاء الشريحة الثانية فوراً عند التحميل إذا لم تكن نشطة
    const slide2 = document.getElementById('slideContent2');
    const activeSlide = document.querySelector('.onboarding-slide-content.active');
    if (slide2 && (!activeSlide || activeSlide.id !== 'slideContent2')) {
        slide2.style.display = 'none';
    ...
    console.log("✅ Slide 2 visibility fix applied");
})();

// --- متغيرات التحكم ---
const DEBUG_SIMULATE_SUBSCRIPTION = false; // محاكاة الاشتراك للتطوير
let currentPlan = localStorage.getItem('codeai_plan') || 'free'; // 'free' | 'extra'
let simulatedSubscription = localStorage.getItem('codeai_sim_sub') === 'true';
// ==========================================
// نظام الحدود اليومية (Daily Limits)
// ==========================================
...
let currentDailyLimit = FREE_DAILY_LIMIT;
let limitProgressBar = null;

// --- النماذج المدفوعة (Premium Models) ---
...
/**
 * التحقق مما إذا كان المستخدم مشتركاً في خطة مدفوعة
 */
function isSubscribed() {
    if (DEBUG_SIMULATE_SUBSCRIPTION) {
...
/**
 * التحقق مما إذا كان نموذج معين مدفوعاً
 */
...
function closePlansPage() {
    const plansPage = document.getElementById('plansPage');
    if (plansPage) {
        plansPage.classList.add('closing');
        setTimeout(() => {
            plansPage.classList.remove('open', 'closing');
...
/**
 * تحديث أزرار الخطط حسب الحالة الحالية
 */
function updatePlanButtons() {
    const freeBtn = document.querySelector('.free-plan .plan-btn');
    const extraBtn = document.querySelector('.extra-plan .plan-btn');
    const freeBadge = document.querySelector('.free-plan .plan-badge');
    const extraBadge = document.querySelector('.extra-plan .plan-badge');
    
    if (!freeBtn || !extraBtn) return;
    
    if (isSubscribed()) {
        // المستخدم مشترك في Extra
        freeBtn.textContent = 'Downgrade';
        freeBtn.className = 'plan-btn subscribe-btn';
        freeBtn.disabled = false;
        freeBtn.style.opacity = '1';
        freeBtn.onclick = () => switchToFreePlan();
        
        extraBtn.textContent = 'Current Plan';
        extraBtn.className = 'plan-btn current-btn';
        extraBtn.disabled = true;
        extraBtn.style.opacity = '0.7';
        
        if (extraBadge) extraBadge.textContent = 'Current Plan';
        if (extraBadge) extraBadge.className = 'plan-badge popular-badge';
        if (freeBadge) freeBadge.style.display = 'none';
    } else {
        // المستخدم على الخطة المجانية
        freeBtn.textContent = 'Current Plan';
        freeBtn.className = 'plan-btn current-btn';
        freeBtn.disabled = true;
        freeBtn.style.opacity = '0.7';
        freeBtn.onclick = null;
        
        extraBtn.innerHTML = '<span class="btn-text">Subscribe to Extra</span><div class="btn-spinner" style="display:none;"></div>';
        extraBtn.className = 'plan-btn subscribe-btn';
        extraBtn.disabled = false;
        extraBtn.onclick = () => subscribeToExtra();
        
        if (freeBadge) freeBadge.style.display = '';
        if (extraBadge) extraBadge.textContent = 'Most Popular';
        if (extraBadge) extraBadge.className = 'plan-badge popular-badge';
    }
}

/**
 * الاشتراك في خطة Extra (محاكاة)
 */


/**
 * العودة إلى الخطة المجانية
 */
async function switchToFreePlan() {
    const confirmed = confirm('Are you sure you want to switch back to the Free plan? You will lose access to premium features.');
    if (!confirmed) return;
    
    // تعطيل الاشتراك
    simulatedSubscription = false;
    localStorage.setItem('codeai_sim_sub', 'false');
    currentPlan = 'free';
    localStorage.setItem('codeai_plan', 'free');
    
    showToast('Switched back to Free plan', 'success');
    
    // تحديث القائمة المنسدلة للنماذج
    updateModelListPremiumStatus();
    
    updatePlanButtons();
    
    // إغلاق الصفحة بعد لحظة
    setTimeout(() => closePlansPage(), 1500);
}

/**
 * تحديث النماذج في القائمة المنسدلة لإظهار/إخفاء علامة Premium
 */
function updateModelListPremiumStatus() {
    const modelItems = document.querySelectorAll('.model-item');
    
    modelItems.forEach(item => {
        const modelType = item.getAttribute('data-model');
        if (PREMIUM_MODELS.includes(modelType)) {
            item.classList.add('premium-model');
            if (!isSubscribed()) {
...
/**
 * فتح نافذة تنبيه الترقية عند النقر على نموذج مدفوع
 */
function showUpgradePrompt(modelName) {
    ...
    if (!modal) {
        ...
        modal.innerHTML = `
            <div class="upgrade-prompt-box">
                <div class="upgrade-prompt-icon">🔒</div>
                <h3 class="upgrade-prompt-title">Premium Feature</h3>
                <p class="upgrade-prompt-desc">
                    <strong>${modelName || 'This model'}</strong> is available exclusively on the <strong>Extra</strong> plan.
                    Upgrade now to unlock powerful AI models and boost your productivity!
                </p>
                <button class="upgrade-prompt-btn" id="upgradePromptSubscribeBtn">Upgrade to Extra ⚡</button>
                <button class="upgrade-prompt-cancel" id="upgradePromptCancelBtn">Maybe later</button>
            </div>
        `;
        document.body.appendChild(modal);
        
        // ربط الأزرار
        document.getElementById('upgradePromptSubscribeBtn').addEventListener('click', () => {
            modal.classList.remove('active');
            modal.style.display = 'none';
            openPlansPage();
        });
        
        document.getElementById('upgradePromptCancelBtn').addEventListener('click', () => {
            modal.classList.remove('active');
            setTimeout(() => modal.style.display = 'none', 300);
        });
...
function openDonationsPage() {
    ...
    if (donationsPage) {
        donationsPage.classList.add('open');
        donationsPage.style.display = 'flex';
        document.body.style.overflow = 'hidden';
    }
}
 ...
 * إغلاق صفحة التبرعات
 */
function closeDonationsPage() {
    const donationsPage = document.getElementById('donationsPage');
    if (donationsPage) {
        donationsPage.classList.add('closing');
        setTimeout(() => {
            donationsPage.classList.remove('open', 'closing');
...
// استرجاع الاستخدام المحلي مع إعادة التعيين اليومي
function getLocalDailyUsage() {
    const stored = {
        used: parseInt(localStorage.getItem('codeai_daily_used') || '0'),
        date: localStorage.getItem('codeai_last_active'),
        streak: parseInt(localStorage.getItem('codeai_streak') || '0')
    };
    const today = new Date().toISOString().split('T')[0];
    if (stored.date !== today) {
        // يوم جديد → تصفير الاستخدام مع الاحتفاظ بالـ Streak (يُحسب لاحقاً)
        stored.used = 0;
        stored.date = today;
        localStorage.setItem('codeai_daily_used', stored.used);
        localStorage.setItem('codeai_last_active', stored.date);
    }
    return stored;
}

function setLocalDailyUsage(used) {
    const today = new Date().toISOString().split('T')[0];
    localStorage.setItem('codeai_daily_used', used);
    localStorage.setItem('codeai_last_active', today);
}

function incrementLocalDailyAndStreak() {
    let { used, streak, date } = getLocalDailyUsage();
    const today = new Date().toISOString().split('T')[0];
    
    // إذا كان آخر يوم نشط هو أمس → زيادة الـ Streak
    const yesterday = new Date(Date.now() - 86400000).toISOString().split('T')[0];
    if (date === yesterday) {
        streak = Math.min(streak + 1, MAX_BONUS_QUOTA);
    } else if (date !== today) {
        streak = 1; // يوم جديد ليس بعد يومين → بداية Streak جديدة
    }
    
    used = Math.min(used + 1, currentDailyLimit);
    
    localStorage.setItem('codeai_daily_used', used);
    localStorage.setItem('codeai_last_active', today);
    localStorage.setItem('codeai_streak', streak);
...
async function fetchUserLimits() {
    ...
    const local = getLocalDailyUsage();
    dailyUsed = local.used;
    currentStreak = local.streak;
    lastActiveDate = local.date;
    currentDailyLimit = FREE_DAILY_LIMIT + (bonusQuota || 0);
    updateLimitProgressBar();
    updateAccountLimitsDisplay();
    
    try {
        const response = await fetchWithAuth(`${API_BASE_URL}/api/user-limits?userId=${userId}`);
        if (response.ok) {
            const data = await response.json();
            dailyUsed = data.daily_count || dailyUsed;
            currentStreak = data.streak || currentStreak;
            bonusQuota = data.bonus_quota || bonusQuota;
            lastActiveDate = data.last_active_date || new Date().toISOString().split('T')[0];
            currentDailyLimit = FREE_DAILY_LIMIT + bonusQuota;
            
            // مزامنة التخزين المحلي
            setLocalDailyUsage(dailyUsed);
            localStorage.setItem('codeai_streak', currentStreak);
        }
    } catch (e) {
        console.warn('Failed to fetch limits from server, using local data');
    }
...
async function incrementDailyUsage() {
    ...
    try {
        const response = await fetchWithAuth(`${API_BASE_URL}/api/increment-usage`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (response.ok) {
            ...
            bonusQuota = data.bonus_quota;
            currentDailyLimit = FREE_DAILY_LIMIT + bonusQuota;
            setLocalDailyUsage(dailyUsed);
            localStorage.setItem('codeai_streak', currentStreak);
        } else {
            // احتياطي محلي
            const local = incrementLocalDailyAndStreak();
            dailyUsed = local.used;
            currentStreak = local.streak;
            currentDailyLimit = FREE_DAILY_LIMIT; // لا بونص محلي
        }
    } catch (e) {
        const local = incrementLocalDailyAndStreak();
        dailyUsed = local.used;
        currentStreak = local.streak;
        currentDailyLimit = FREE_DAILY_LIMIT;
    }
...
function canSendMessage() {
    ...
    return dailyUsed < currentDailyLimit;
}

function getRemainingMessages() {
    if (isSubscribed()) return Infinity;
    return Math.max(0, currentDailyLimit - dailyUsed);
}

function createLimitProgressBar() {
    if (limitProgressBar) limitProgressBar.remove();
    if (isSubscribed()) return;
    
    limitProgressBar = document.createElement('div');
    limitProgressBar.id = 'limitProgressBar';
    limitProgressBar.className = 'limit-progress-bar';
    limitProgressBar.innerHTML = `
        <div class="limit-bar-container">
            <div class="limit-bar-fill" id="limitBarFill"></div>
            <div class="limit-bar-text" id="limitBarText"></div>
        </div>
    `;
    document.body.appendChild(limitProgressBar);
    updateLimitProgressBar();
}

function updateLimitProgressBar() {
    if (!limitProgressBar || isSubscribed()) return;
    
    const fill = document.getElementById('limitBarFill');
    const text = document.getElementById('limitBarText');
    if (!fill || !text) return;
    
    const used = dailyUsed || 0;
    const limit = currentDailyLimit || FREE_DAILY_LIMIT;
    const usedPercentage = Math.min(100, (used / limit) * 100);
    
    fill.style.width = `${usedPercentage}%`;  // <-- الآن يمثل المستخدم
    ...
    if (usedPercentage >= 100) {
        ...
        text.textContent = `${used}/${limit} messages used`;
    } else {
        fill.style.background = 'linear-gradient(90deg, #42A5F5, #64B5F6)';
        text.textContent = `${used}/${limit} messages used`;
    }
    
    if (limitProgressBar.style.display === 'none') {
        limitProgressBar.style.display = 'block';
    }
    
    clearTimeout(window._limitBarTimeout);
    window._limitBarTimeout = setTimeout(() => {
        if (limitProgressBar) limitProgressBar.style.display = 'none';
    }, 5000);
}

function showLimitProgressBar() {
    if (!limitProgressBar || isSubscribed()) return;
    limitProgressBar.style.display = 'block';
    updateLimitProgressBar();
}

function updateAccountLimitsDisplay() {
    const limitsSection = document.getElementById('accountLimitsSection');
    const limitsFill = document.getElementById('accountLimitsFill');
    const limitsText = document.getElementById('accountLimitsText');
    const upgradeBtnInLimits = document.getElementById('accountUpgradeBtn');
    
    if (!limitsSection) return;
    
    // ✅ إخفاء القسم للمشتركين
    if (isSubscribed()) {
    ...
    // ✅ شريط التقدم (يمتلئ من اليسار لليمين)
    const used = dailyUsed || 0;
    const limit = currentDailyLimit || FREE_DAILY_LIMIT;
    ...
    if (limitsFill) {
        ...
        limitsFill.style.width = `${percentage}%`;
        
        if (remaining <= 3) {
            limitsFill.style.background = 'linear-gradient(90deg, #ff6b6b, #ff4444)';
        } else if (remaining <= 5) {
            limitsFill.style.background = 'linear-gradient(90deg, #ffa726, #ff9800)';
        } else {
            limitsFill.style.background = 'linear-gradient(90deg, #64B5F6, #42A5F5)';
        }
    }
    
    if (limitsText) {
        limitsText.textContent = `${remaining} of ${limit} messages left today`;
    }
    
    // ✅ إنشاء أو تحديث مربعات الـ Streak
    let streakContainer = document.getElementById('accountStreakContainer');
    if (!streakContainer) {
        streakContainer = document.createElement('div');
        streakContainer.id = 'accountStreakContainer';
        streakContainer.className = 'account-streak-container';
        
        const barContainer = limitsFill?.parentNode;
        if (barContainer && barContainer.parentNode) {
            barContainer.parentNode.insertBefore(streakContainer, upgradeBtnInLimits);
        }
    }
    
    const filledBoxes = Math.min(currentStreak || 0, MAX_BONUS_QUOTA);
    const emptyBoxes = MAX_BONUS_QUOTA - filledBoxes;
    
    // ✅ أيقونة SVG لعلامة الصح البيضاء (أكثر احترافية)
    const checkSVG = `
    ...
    let boxesHTML = '<div class="streak-boxes-row">';
    
    // إضافة المربعات الممتلئة (حمراء + علامة صح بيضاء)
    for (let i = 0; i < filledBoxes; i++) {
        boxesHTML += `<span class="streak-box filled">${checkSVG}</span>`;
    }
    
    // إضافة المربعات الفارغة (نقطة باهتة)
    for (let i = 0; i < emptyBoxes; i++) {
        boxesHTML += '<span class="streak-box empty"><span class="streak-dot"></span></span>';
    }
    
    boxesHTML += '</div>';
    
    // نص الـ Streak
    const streakText = (currentStreak || 0) > 0 
        ? `🔥 ${currentStreak} day streak` 
        : `✨ Send a message each day to build a streak`;
    ...
    // ✅ زر الترقية
    if (upgradeBtnInLimits) {
        upgradeBtnInLimits.style.display = 'block';
        upgradeBtnInLimits.onclick = () => openPlansPage();
    }
}

// --- ربط الأحداث (Event Listeners) ---

// زر Upgrade في القائمة الجانبية
document.getElementById('upgradeBtn')?.addEventListener('click', () => {
    closeMenu();
    openPlansPage();
});

// زر Donate في القائمة الجانبية
document.getElementById('donateBtn')?.addEventListener('click', () => {
    closeMenu();
    openDonationsPage();
});

// زر إغلاق صفحة الباقات
document.getElementById('closePlansBtn')?.addEventListener('click', closePlansPage);

// زر إغلاق صفحة التبرعات
document.getElementById('closeDonationsBtn')?.addEventListener('click', closeDonationsPage);

// مستمع حدث احتياطي لزر الترقية في صفحة الحساب
document.getElementById('accountUpgradeBtn')?.addEventListener('click', () => {
    openPlansPage();
});

// --- نظام التبرعات (محاكاة) ---
...
// المبلغ المخصص
document.getElementById('customDonationInput')?.addEventListener('input', function() {
    if (this.value) {
        document.querySelectorAll('.donation-card').forEach(c => c.classList.remove('selected'));
        selectedDonationAmount = parseInt(this.value) || 0;
        
        // تحديد بطاقة Custom بصرياً
        const customCard = document.querySelector('.donation-card.custom-donation');
        if (customCard && this.value) {
...
document.getElementById('submitDonationBtn')?.addEventListener('click', async () => {
    ...
    showToast(`Thank you for your $${amount} donation! You're awesome! 🌟`, 'success');
...
// --- تعديل اختيار النموذج ليشمل التحقق من Premium ---
// يتم استدعاء هذا بعد initModelSelector
setTimeout(() => {
    updateModelListPremiumStatus();
    
    // اعتراض النقر على النماذج المدفوعة
    const modelItems = document.querySelectorAll('.model-item.premium-model');
    modelItems.forEach(item => {
        item.addEventListener('click', function(e) {
            const modelType = this.getAttribute('data-model');
            const modelName = this.querySelector('.model-info h3')?.textContent || 'This model';
        ...
        }, true); // capture phase للتنفيذ قبل المستمع الأصلي
    });
}, 1000);

// --- تحديث زر Upgrade في القائمة ---
function updateUpgradeButton() {
    const upgradeBtn = document.getElementById('upgradeBtn');
    if (!upgradeBtn) return;
    
    if (isSubscribed()) {
        upgradeBtn.innerHTML = '<span class="upgrade-icon">⭐</span> Extra Plan';
        upgradeBtn.style.background = 'linear-gradient(135deg, #4caf50, #2e7d32)';
        upgradeBtn.style.boxShadow = '0 4px 12px rgba(76, 175, 80, 0.3)';
    } else {
        upgradeBtn.innerHTML = 'Upgrade to EXTRA';
        upgradeBtn.style.background = 'background: var(--bg-header);';
        upgradeBtn.style.border = '1px solid #089cff;'
        upgradeBtn.style.boxShadow = '0 4px 12px rgba(0, 152, 255, 0.3);';
    }
}

// تحديث الزر عند التحميل
updateUpgradeButton();

// ==========================================
// نظام الباقات الحقيقي – Stripe Integration
// ==========================================

// ✅ متغير للتفرع بين المحاكاة والحقيقي
const ENABLE_REAL_STRIPE = false; // true = يستخدم Stripe, false = محاكاة

/**
 * الاشتراك في خطة Extra (يدعم المحاكاة وStripe)
 */
async function subscribeToExtra() {
    if (!ENABLE_REAL_STRIPE) {
        // استخدام المحاكاة الحالية
        return subscribeToExtraSimulated();
    }
    
    // وإلا فتحويل المستخدم إلى Stripe Checkout
    const btn = document.getElementById('subscribeExtraBtn');
    if (!btn) return;
    ...
    try {
        const response = await fetchWithAuth(`${API_BASE_URL}/api/create-checkout-session`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                priceId: 'price_XXXXXXXX', // سيتم استبداله بمعرف السعر الحقيقي من Stripe
                userId: localStorage.getItem('userId'),
        ...
        // تحويل المستخدم إلى صفحة دفع Stripe
...
// الدالة الأصلية (محاكاة) معاد تسميتها للتوضيح
async function subscribeToExtraSimulated() {
    const btn = document.getElementById('subscribeExtraBtn');
    if (!btn) return;
    ...
    // تفعيل الاشتراك
    simulatedSubscription = true;
    localStorage.setItem('codeai_sim_sub', 'true');
    currentPlan = 'extra';
    localStorage.setItem('codeai_plan', 'extra');
    ...
    showToast('Welcome to Extra! All premium features unlocked! 🎉', 'success');
    ...
    setTimeout(() => {
        closePlansPage();
        btn.classList.remove('success');
        btnText.textContent = 'Subscribe to Extra';
        btn.disabled = false;
        updatePlanButtons();
    }, 2000);
}

console.log("✅ Plans & Donations system initialized");


// ---- زر المزيد (ثلاث نقاط) ----
const fileMoreBtn = document.getElementById('fileMoreBtn');
const fileMorePopover = document.getElementById('fileMorePopover');

if (fileMoreBtn && fileMorePopover) {
    fileMoreBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        fileMorePopover.classList.toggle('show');
        // ضبط الموضع بجانب الزر
        const rect = fileMoreBtn.getBoundingClientRect();
        fileMorePopover.style.top = (rect.bottom - 35) + 'px';
        fileMorePopover.style.right = (window.innerWidth - rect.right) + 'px';
    });

    // إغلاق popover عند الضغط خارجه
    document.addEventListener('click', (e) => {
        if (!fileMorePopover.contains(e.target) && e.target !== fileMoreBtn) {
            fileMorePopover.classList.remove('show');
        }
    });

    // خيارات القائمة
    document.getElementById('moreRename')?.addEventListener('click', () => {
        fileMorePopover.classList.remove('show');
        openTabModal(activeFileIndex);
    });

    document.getElementById('moreSearch')?.addEventListener('click', () => {
    fileMorePopover.classList.remove('show');
    ...
    document.getElementById('moreGoToLine')?.addEventListener('click', () => {
        fileMorePopover.classList.remove('show');
        document.getElementById('goToLineModal').classList.add('active');
        document.getElementById('goToLineModal').style.display = 'flex';
    });
}

// ---- البحث والاستبدال ----
// ---- البحث والاستبدال (شريط سفلي) ----
// ---- البحث والاستبدال (شريط سفلي) ----
(function() {
    const bar = document.getElementById('searchReplaceBar');
    ...
    // فتح مودال الإعدادات
document.getElementById('searchSettingsBtn')?.addEventListener('click', () => {
...
document.getElementById('closeSearchSettingsBtn')?.addEventListener('click', () => {
...
// إغلاق عند النقر خارج الصندوق
document.getElementById('searchSettingsModal')?.addEventListener('click', (e) => {
    ...
    function updateMatchCount() {
        ...
        const current = lastMatchIndex >= 0 && lastMatchIndex < total ? allMatches.indexOf(lastMatchIndex) + 1 : 0;
        matchCountEl.textContent = total > 0 ? `${current}/${total}` : '0/0';
    }
    ...
    function findNext() {
        ...
        // البحث عن أول تطابق بعد المؤشر الحالي
        let nextIdx = -1;
        ...
        if (nextIdx === -1) nextIdx = allMatches[0]; // نلف من البداية
        
        lastMatchIndex = nextIdx;
        codeArea.focus();
        codeArea.setSelectionRange(nextIdx, nextIdx + searchInput.value.length);
        const lines = text.substring(0, nextIdx).split('\n').length;
        codeArea.scrollTop = Math.max(0, (lines - 5) * 22);
        updateMatchCount();
    }
    ...
    function findPrev() {
        ...
        // البحث عن أول تطابق قبل المؤشر الحالي (بالعكس)
        let prevIdx = -1;
        for (let i = allMatches.length - 1; i >= 0; i--) {
            if (allMatches[i] < lastMatchIndex) {
                prevIdx = allMatches[i];
                break;
            }
        }
        if (prevIdx === -1) prevIdx = allMatches[allMatches.length - 1];
        
        lastMatchIndex = prevIdx;
        codeArea.focus();
        codeArea.setSelectionRange(prevIdx, prevIdx + searchInput.value.length);
        const lines = text.substring(0, prevIdx).split('\n').length;
        codeArea.scrollTop = Math.max(0, (lines - 5) * 22);
        updateMatchCount();
    }
    
    // تحديث التطابقات عند تغيير النص أو تبديل case sensitive
    ...
    document.getElementById('searchNextBtn')?.addEventListener('click', findNext);
    document.getElementById('searchPrevBtn')?.addEventListener('click', findPrev);
    ...
    document.getElementById('replaceOneBtn')?.addEventListener('click', () => {
        ...
        const start = lastMatchIndex >= 0 ? lastMatchIndex : 0;
        const idx = caseSensitive ? text.indexOf(search, start) : text.toLowerCase().indexOf(search.toLowerCase(), start);
        
        if (idx !== -1) {
            recordChange(codeArea.value);
            codeArea.value = text.substring(0, idx) + replace + text.substring(idx + search.length);
            lastMatchIndex = idx + replace.length;
            projectFiles[activeFileIndex].content = codeArea.value;
            updateView();
            recordChange(codeArea.value);
            findAllMatches();
            ...
            lastMatchIndex = -1;
            document.getElementById('replaceOneBtn').click();
        }
    });
    ...
    document.getElementById('replaceAllBtn')?.addEventListener('click', () => {
        ...
        if (caseSensitiveCheck.checked) {
            newText = codeArea.value.split(search).join(replace);
        } else {
            const escaped = search.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            newText = codeArea.value.replace(new RegExp(escaped, 'gi'), replace);
        }
        codeArea.value = newText;
        projectFiles[activeFileIndex].content = newText;
        updateView();
        recordChange(codeArea.value);
        lastMatchIndex = -1;
        findAllMatches();
...
(function() {
    ...
    const lineNum = parseInt(document.getElementById('lineNumberInput').value);
    ...
    // التمرير لعرض السطر (نستخدم line-height ديناميكي إن أمكن)
    const lineHeight = parseFloat(getComputedStyle(codeArea).lineHeight) || 22;
    codeArea.scrollTop = Math.max(0, targetLine * lineHeight - codeArea.clientHeight / 2);

    // إغلاق المودال
    document.getElementById('goToLineModal').classList.remove('active');
    document.getElementById('goToLineModal').style.display = 'none';
});

    document.getElementById('closeGoToLineBtn')?.addEventListener('click', () => {
        modal.classList.remove('active');
        modal.style.display = 'none';
    });
...
// للاختبار فقط - يُظهر الشريط بعد 2 ثانية من تحميل الصفحة
...
// ==========================================
// ==========================================
// Control Panel Logic - الإصدار المصحح
// ==========================================
// ==========================================
// Control Panel Logic - رفع الكتابة لأعلى
// ==========================================
// ==========================================
// لوحة التحكم - نسخة نظيفة ونهائية
// ==========================================
let rotationdeg = 0;
(function initControlPanel() {
    // 1. إنشاء الحاوية السفلية الموحدة
    const bottomSection = document.createElement('div');
    bottomSection.id = 'bottomSection';
    bottomSection.style.cssText = 'position:fixed;bottom:0;left:0;right:0;z-index:200;transition:transform 0.3s ease;pointer-events:none;';
    document.body.appendChild(bottomSection);

    // 2. نقل شريط الاقتراحات، صندوق الإدخال، ولوحة التحكم إلى الحاوية
    const suggestionBar = document.getElementById('suggestionBar');
    const inputBar = document.querySelector('.input-bar');
    const controlPanel = document.getElementById('controlPanel');

    [suggestionBar, inputBar, controlPanel].forEach(el => {
        if (el) {
    ...
    // 3. ضبط padding للرسائل
    const messagesEl = document.getElementById('messages');
    if (messagesEl) messagesEl.style.paddingBottom = '120px';

    // 4. منطق الفتح والإغلاق
    // 4. منطق الفتح والإغلاق مع أنيميشن
// 4. منطق الفتح والإغلاق مع أنيميشن من تحت الشاشة
// 4. منطق الفتح والإغلاق مع أنيميشن
const toggleBtn = document.getElementById('controlPanelToggleBtn');
let isOpen = false;

toggleBtn.style.transition = 'transform 0.8s cubic-bezier(0.4, 0, 0.2, 1)';
...
toggleBtn.addEventListener('click', () => {
    ...
    rotationdeg += 135;
    const icon = toggleBtn.querySelector('svg');  // أيقونة الـ SVG داخل الزر
if (icon) {
    ...
    if (isOpen) {
        // 1. وسّع اللوحة
        controlPanel.style.maxHeight = '220px';
        controlPanel.style.padding = '14px 18px';
        controlPanel.style.opacity = '1';
        
        // 2. انتظر 30ms حتى يُحسب الارتفاع الجديد
        setTimeout(() => {
            const move = controlPanel.scrollHeight / 10;
            bottomSection.style.transition = 'transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)';
           
            bottomSection.style.transform = `translateY(-${move}px)`;
            bottomSection.style.transform = 'translateY(0)';
        }, 30);
        
       // toggleBtn.innerHTML = '<svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>';
    } else {
        // إغلاق: حرّك للأسفل أولاً
        const move = controlPanel.scrollHeight / 10;
        bottomSection.style.transition = 'transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)';
        bottomSection.style.transform = 'translateY(0)';
        
        // أخفِ اللوحة بعد انتهاء الأنيميشن
        setTimeout(() => {
            controlPanel.style.maxHeight = '0';
            controlPanel.style.padding = '0 18px';
            controlPanel.style.opacity = '0';
        }, 20);
        
       // toggleBtn.innerHTML = '<svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>';
    }
});

    // 5. ربط عناصر التحكم الداخلية (درجة الحرارة، Top-P، الوضع)
    document.getElementById('cpAttachBtn')?.addEventListener('click', () => document.getElementById('imageFileInput').click());

    const tempSlider = document.getElementById('temperatureSlider');
    const tempValue = document.getElementById('tempValue');
    tempSlider.value = localStorage.getItem('codeai_temp') || 0.7;
    tempValue.textContent = parseFloat(tempSlider.value).toFixed(1);
    tempSlider.addEventListener('input', (e) => {
        const v = parseFloat(e.target.value).toFixed(1);
        tempValue.textContent = v;
        localStorage.setItem('codeai_temp', v);
    });

    const topPSlider = document.getElementById('topPSlider');
    const topPValue = document.getElementById('topPValue');
    topPSlider.value = localStorage.getItem('codeai_top_p') || 1.0;
    ...
    topPSlider.addEventListener('input', (e) => {
        ...
        localStorage.setItem('codeai_top_p', v);
    });

    const codeaiBtn = document.getElementById('codeaiModeBtn');
    const nativeBtn = document.getElementById('nativeModeBtn');
    const setMode = (mode) => {
        const isNative = mode === 'native';
        nativeBtn.classList.toggle('active', isNative);
        codeaiBtn.classList.toggle('active', !isNative);
        localStorage.setItem('codeai_native_mode', isNative);
    };
    if (localStorage.getItem('codeai_native_mode') === 'true') setMode('native');
    codeaiBtn.addEventListener('click', () => setMode('codeai'));
    nativeBtn.addEventListener('click', () => setMode('native'));

    document.getElementById('resetControlPanelBtn')?.addEventListener('click', () => {
        tempSlider.value = 0.7;
        tempValue.textContent = '0.7';
        localStorage.setItem('codeai_temp', '0.7');
        topPSlider.value = 1.0;
        topPValue.textContent = '1.0';
        localStorage.setItem('codeai_top_p', '1.0');
        setMode('codeai');
    });
})();
...
// ---- فتح صفحة الأوامر ----
// ---- فتح صفحة الأوامر ----
document.getElementById('openCommandsBtn')?.addEventListener('click', () => {
    const page = document.getElementById('commandsPage');
    if (page) {
        page.style.display = 'block';  // ✅ أولاً: إظهار العنصر
        // استخدام requestAnimationFrame لضمان تطبيق display قبل إضافة الكلاس
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                page.classList.add('open');  // ✅ ثانياً: إضافة كلاس الأنميشن
            });
        });
        renderCommandsPage();
    }
...
// ==========================================
// صفحة سجل التغييرات (Version History)
// ==========================================
...
// دالة لاستخراج ملخص التغييرات بين لقطتين
function getChangesSummary(prevSnapshot, currentSnapshot) {
    if (!prevSnapshot || !currentSnapshot) {
        return { created: [], modified: [], deleted: [] };
    }
    
    const prevMap = new Map(prevSnapshot.map(f => [f.name, f.content]));
    const currentMap = new Map(currentSnapshot.map(f => [f.name, f.content]));
    
    const created = [];
    const modified = [];
    const deleted = [];
    
    // الملفات في اللقطة الجديدة فقط = تم إنشاؤها
    ...
    // الملفات في اللقطة القديمة فقط = تم حذفها
...
// دالة لاستخراج جميع نقاط التفتيش من المحادثة
function extractCheckpoints(conversation) {
    const checkpoints = [];
    
    if (!conversation) return checkpoints;
    
    // 1. استخراج من رسائل AI التي تحتوي على filesSnapshot
    for (let i = 0; i < conversation.messages.length; i++) {
        const userMsg = conversation.messages[i];
        if (userMsg.replies && userMsg.replies.length) {
            for (let j = 0; j < userMsg.replies.length; j++) {
                const reply = userMsg.replies[j];
                if (reply.filesSnapshot && reply.filesSnapshot.length) {
                    // الحصول على اللقطة السابقة (إن وجدت)
                    let prevSnapshot = null;
                    if (j > 0 && userMsg.replies[j-1].filesSnapshot) {
                        prevSnapshot = userMsg.replies[j-1].filesSnapshot;
                    } else if (i > 0 && conversation.messages[i-1].replies && 
                               conversation.messages[i-1].replies.length > 0) {
                        const prevReply = conversation.messages[i-1].replies[conversation.messages[i-1].replies.length - 1];
                        prevSnapshot = prevReply.filesSnapshot;
                    }
                    
                    const changes = getChangesSummary(prevSnapshot, reply.filesSnapshot);
                    
                    checkpoints.push({
                        id: `ai_${reply.id || Date.now()}_${j}`,
                        timestamp: reply.timestamp || Date.now(),
                        source: 'ai',
                        model: reply.model || 'AI',
                        description: reply.text ? reply.text.substring(0, 100) : 'AI response',
                        filesSnapshot: reply.filesSnapshot,
                        changes: changes
                    });
                }
            }
    ...
    // 2. استخراج من النقاط اليدوية (checkpoints)
    if (conversation.checkpoints && conversation.checkpoints.length) {
        for (const cp of conversation.checkpoints) {
            checkpoints.push({
                id: cp.id,
                timestamp: cp.timestamp,
                source: 'manual',
                model: 'Manual',
                description: cp.description || 'Manual checkpoint',
                filesSnapshot: cp.filesSnapshot,
                changes: cp.changes || { created: [], modified: [], deleted: [] }
            });
        }
    }
...
// دالة استعادة ملفات من لقطة
async function restoreFromSnapshot(snapshot, checkpointId) {
    ...
    try {
        // استعادة الملفات
        projectFiles = snapshot.map(file => ({
            name: file.name,
            content: file.content || '',
            url: file.url || null,
            previewUrl: file.previewUrl || null,
...
// دالة إضافة نقطة تفتيش يدوية
async function addManualCheckpoint(description = '') {
    const conv = convs.find(c => c.id === activeId);
    if (!conv) {
    ...
    const checkpoint = {
        id: 'ck_' + Date.now() + '_' + Math.random().toString(36).substr(2, 6),
        timestamp: Date.now(),
        description: description || `Manual save at ${new Date().toLocaleTimeString()}`,
        filesSnapshot: projectFiles.map(file => ({
            name: file.name,
            content: file.content,
    ...
    // إضافة إلى مصفوفة checkpoints
    if (!conv.checkpoints) conv.checkpoints = [];
    conv.checkpoints.push(checkpoint);
    
    // حفظ المحادثة
    await saveConversationToServer(conv);
    
    // تحديث الصفحة إذا كانت مفتوحة
    if (historyPage && historyPage.classList.contains('open')) {
        renderHistoryPage();
    }
...
function renderHistoryPage() {
    ...
    if (!conv || (!conv.messages.length && !conv.checkpoints?.length)) {
        container.innerHTML = `
            <div style="text-align: center; padding: 40px; opacity: 0.5;">
                ${isArabic ? 'لا توجد نقاط تفتيش بعد' : 'No checkpoints yet'}
            </div>
        `;
        return;
    }
    
    const checkpoints = extractCheckpoints(conv);
    ...
    for (const cp of checkpoints) {
        ...
        const timeStr = date.toLocaleString(lang === 'ar' ? 'ar-SA' : 'en-US');
        
        const sourceIcon = cp.source === 'ai' ? '' : '';
        const sourceText = cp.source === 'ai' ? (isArabic ? 'الذكاء الاصطناعي' : 'AI') : (isArabic ? 'يدوي' : 'Manual');
        
        const changesSummary = [];
        if (cp.changes.created?.length) {
            changesSummary.push(`${isArabic ? 'تم الإنشاء' : 'Created'}: ${cp.changes.created.length}`);
        }
        if (cp.changes.modified?.length) {
            changesSummary.push(`${isArabic ? 'تم التعديل' : 'Modified'}: ${cp.changes.modified.length}`);
        }
        if (cp.changes.deleted?.length) {
            changesSummary.push(`${isArabic ? 'تم الحذف' : 'Deleted'}: ${cp.changes.deleted.length}`);
        }
        
        const changesText = changesSummary.length ? changesSummary.join(' · ') : (isArabic ? 'لا تغييرات' : 'No changes');
        
        html += `
            <div class="history-item" data-checkpoint-id="${cp.id}" style="
                background: var(--bg-secondary);
                border-radius: 16px;
                padding: 16px;
                margin-bottom: 12px;
            ...
            ">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 12px;">
                    <div style="display: flex; align-items: center; gap: 10px;">
                        ...
                        <div>
                            <div style="font-weight: 600; font-size: 14px;">${sourceText}</div>
                            <div style="font-size: 11px; opacity: 0.6;">${timeStr}</div>
                        </div>
                    </div>
                    <button class="history-restore-btn" data-checkpoint-id="${cp.id}" style="
                        background: transparent;
                        border: 1px solid var(--border-color);
                        border-radius: 20px;
                        padding: 6px 16px;
                        ...
                        cursor: pointer;
                        transition: all 0.2s;
                    ">${isArabic ? '⏪ استعادة' : '⏪ Restore'}</button>
                </div>
                <div style="font-size: 13px; opacity: 0.8; margin-bottom: 8px;">
                    ${cp.model ? `<span style="background: var(--bg-primary); padding: 2px 8px; border-radius: 12px; font-size: 11px;">${cp.model}</span> ` : ''}
                </div>
                <div style="font-size: 13px; opacity: 0.7;">
                    📝 ${cp.description.substring(0, 80)}${cp.description.length > 80 ? '...' : ''}
                </div>
    ...
    document.querySelectorAll('.history-restore-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            ...
            if (checkpoint && checkpoint.filesSnapshot) {
                btn.textContent = isArabic ? 'جاري...' : 'Restoring...';
                btn.disabled = true;
                await restoreFromSnapshot(checkpoint.filesSnapshot, checkpointId);
                btn.textContent = isArabic ? '⏪ استعادة' : '⏪ Restore';
                btn.disabled = false;
            }
        });
    });
    ...
    if (!historyPage) {
        ...
        historyPage.innerHTML = `
            <div class="history-header" style="
                ...
                z-index: 10;
            ">
                <button class="history-back-btn" style="
                    background: none;
                    border: none;
                    font-size: 28px;
                    color: var(--text-color);
                    ...
                    border-radius: 50%;
                ">←</button>
                <h2 style="margin: 0; font-size: 20px; font-weight: 600;">${isArabic ? 'سجل التغييرات' : 'Version History'}</h2>
                <div style="width: 40px;"></div>
            </div>
            <div style="padding: 20px;">
                <div id="historyContainer" style="max-width: 600px; margin: 0 auto;"></div>
                <button id="addCheckpointBtn" style="
                    width: 100%;
                    max-width: 600px;
                    ...
                    cursor: pointer;
                    transition: all 0.2s;
                ">${isArabic ? '+ إضافة نقطة تفتيش' : '+ Add Checkpoint'}</button>
            </div>
        `;
        ...
        // ربط الأحداث
        const backBtn = historyPage.querySelector('.history-back-btn');
        backBtn.addEventListener('click', () => closeHistoryPage());
...
if (addBtn) {
    addBtn.addEventListener('click', async () => {
        const lang = localStorage.getItem('codeai_lang') || 'en';
        const isArabic = lang === 'ar';
        const title = isArabic ? 'إضافة نقطة تفتيش' : 'Add Checkpoint';
        const placeholder = isArabic ? 'وصف اختياري...' : 'description... (Optional)';
        
        showInputModal(title, placeholder, async (description) => {
            await addManualCheckpoint(description || undefined);
        });
    });
}
        ...
        historyPage.addEventListener('click', (e) => {
            if (e.target === historyPage) closeHistoryPage();
        });
    }
    ...
    setTimeout(() => {
        ...
        historyPage.style.transform = 'translateY(0)';
    }, 10);
}
...
function closeHistoryPage() {
    ...
    historyPage.style.transform = 'translateY(20px)';
    setTimeout(() => {
...
// إضافة زر السجل في قائمة الثلاث نقاط (fileMorePopover)
const moreHistoryBtn = document.createElement('div');
moreHistoryBtn.id = 'moreHistory';
moreHistoryBtn.className = 'more-menu-item';
moreHistoryBtn.innerHTML = 'Change Log';
moreHistoryBtn.style.cssText = `
    padding: 10px 16px;
    cursor: pointer;
...
moreHistoryBtn.addEventListener('click', () => {
    // إغلاق popover
    const fileMorePopover = document.getElementById('fileMorePopover');
    if (fileMorePopover) fileMorePopover.classList.remove('show');
    // فتح صفحة التاريخ
    openHistoryPage();
});
...
if (fileMorePopover) {
    // إضافة قبل زر Go to line أو في نهاية القائمة
    const moreGoToLine = document.getElementById('moreGoToLine');
    if (moreGoToLine && moreGoToLine.parentNode === fileMorePopover) {
        fileMorePopover.insertBefore(moreHistoryBtn, moreGoToLine);
    } else {
        fileMorePopover.appendChild(moreHistoryBtn);
    }
}

let activeInputModal = null;

// ==========================================
// مودال إدخال النص المخصص (بديل عن prompt) مع أنميشن
// ==========================================
...
function showInputModal(title, placeholder, callback) {
    ...
    saveBtn.textContent = isArabic ? 'حفظ' : 'Save';
    saveBtn.style.flex = '1';
    saveBtn.style.padding = '12px';
    saveBtn.style.borderRadius = '30px';
    saveBtn.style.cursor = 'pointer';
    saveBtn.style.background = 'var(--text-color)';
    saveBtn.style.color = 'var(--bg-primary)';
    saveBtn.style.border = 'none';
    saveBtn.style.fontWeight = '600';
    saveBtn.style.transition = 'transform 0.2s, opacity 0.2s';
    ...
    cancelBtn.style.transition = 'transform 0.2s, background 0.2s';
    ...
    document.body.appendChild(modal);
    activeInputModal = modal;
    
    // تشغيل أنميشن الدخول (مطابق لأنميشن المودالات الأخرى)
    requestAnimationFrame(() => {
        requestAnimationFrame(() => {
    ...
    // وظيفة الإغلاق مع أنميشن
    const closeModal = () => {
        if (!modal || !modal.parentNode) return;
        
        // إضافة كلاس closing للتلاشي
        modal.classList.add('closing');
        modalBox.style.opacity = '0';
        modalBox.style.transform = 'translateY(20px)';
        modal.style.opacity = '0';
        ...
        setTimeout(() => {
            ...
            if (activeInputModal === modal) activeInputModal = null;
        }, 300);
    };
...
// دالة تصدير المشروع كملف ZIP
async function exportProjectAsZip() {
    if (!isSubscribed()) {
        showToast('✨ This feature is available for Extra plan users only. Upgrade to export your project as ZIP.', 'error');
        openPlansPage();
        return false;
    }
    ...
    for (const file of projectFiles) {
        ...
        // الملفات الثنائية (مثل PDF, DOCX) التي قد تحتوي على url
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    console.warn(`Could not fetch ${file.name}, skipping`);
                    continue;
                }
            } catch (err) {
                console.warn(`Error fetching ${file.name}:`, err);
                continue;
            }
        }
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `project_${Date.now()}.zip`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
...
// ربط الزر (بعد أن يتم إنشاء popover)
setTimeout(() => {
    const exportZipBtn = document.getElementById('moreExportZip');
    if (exportZipBtn) {
        exportZipBtn.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            // إغلاق الـ popover
            const popover = document.getElementById('fileMorePopover');
...
const feedbackBtn = document.getElementById('feedbackFloatBtn');
const feedbackModal = document.getElementById('feedbackModal');
const feedbackComment = document.getElementById('feedbackCommentInput');
...
feedbackBtn?.addEventListener('click', () => {
  ...
  setTimeout(() => feedbackModal.classList.add('active'), 10);
});
...
sendFeedbackBtn?.addEventListener('click', async () => {
  ...
  try {
    const response = await fetchWithAuth(`${API_BASE_URL}/api/feedback/general`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
