const APP_VERSION = 'v1.282.31'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
async function fetchConversationFromServer(convId) {
    ...
    try {
        ...
        // خطوة 1: فصل الرسائل حسب الدور
        const userMessagesMap = new Map();  // id -> كائن رسالة المستخدم
        const aiMessages = [];              // قائمة مؤقتة لرسائل AI
        
        // حفظ جميع الرسائل في مصفوفة مؤقتة مع تحويل created_at إلى timestamp
        const allMessages = (data.messages || []).map(msg => ({
            ...msg,
            timestamp: new Date(msg.created_at).getTime()
        }));
        
        // فرز جميع الرسائل حسب created_at (لضمان الترتيب)
        allMessages.sort((a, b) => a.timestamp - b.timestamp);
        
        // بناء خريطة لرسائل المستخدم
        ...
        // تجميع ردود AI تحت رسائل المستخدم المناسبة
        allMessages.forEach(msg => {
        ...
        // ترتيب الردود لكل رسالة مستخدم حسب reply_index أو timestamp
        ...
        userMessages.forEach(userMsg => {
            ...
            // تحديد آخر رد (is_latest === true) أو آخر عنصر في المصفوفة
            const latestReply = userMsg.replies.find(r => r.is_latest === true);
            if (latestReply) {
                userMsg.currentReplyIndex = userMsg.replies.indexOf(latestReply);
            } else if (userMsg.replies.length > 0) {
        ...
        // ترتيب رسائل المستخدم حسب timestamp
        userMessages.sort((a, b) => a.timestamp - b.timestamp);
        
        // معالجة الملفات (نفس الكود القديم)
        const files = (data.files || []).map(file => ({
            name: file.name,
            content: file.content || '',
            url: file.url || null,
...
if (duration !== null && duration > 0) {
    ...
    conv.messages.forEach((m, index) => {
        // ✅ إنشاء مفتاح فريد للرسالة
        const msgKey = `${m.role}_${m.text}`;
        
        // ✅ تخطي إذا تم عرضها مسبقاً
        if (displayedMessages.has(msgKey)) {
            console.warn(`⚠️ Skipping duplicate message in render:`, m.role, m.text?.substring(0, 30));
            return;
        }
...
if (m.tasks && (m.tasks.built?.length || m.tasks.modified?.length || m.tasks.deleted?.length)) {
    ...
    // إنشاء حاوية المهام الكاملة
    tasksContainer = buildTaskListContainer(m.tasks, duration);
}
            ...
            if (tasksContainer) {
    d.insertBefore(tasksContainer, contentEl);
}
