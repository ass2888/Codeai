const APP_VERSION = 'v1.281.19'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
function saveState() {
    ...
    conv.files = projectFiles.map(file => {
        if (isImageFile(file.name)) {
            if (!file.content || !file.content.startsWith('data:')) {
                console.warn('⚠️ Image lost content:', file.name);
            }
            return {
                name: file.name,
...
function updateActiveTab() {
    ...
    if (previewUrl) {
        // عرض صور PNG
        fetch(`${previewUrl}/page_1.png`, { method: 'HEAD' })
            .then(() => {
                let pages = '';
                // نبني الصور ديناميكياً — نحاول حتى نفشل
...
function handleIncomingFiles(files) {
    files.forEach(file => {
        // منع التكرار
        const exists = projectFiles.some(f => f.name === file.name);
        if (exists) return;

        projectFiles.push({
        name: file.name,
        type: file.type || file.kind,
        url: file.url || file.path,   // ← fallback للـ path
        isBinary: true
      });
    });
