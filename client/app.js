...
async function fetchConversationFromServer(convId) {
    ...
    try {
        ...
        // خريطة لرسائل المستخدم (id -> كائن)
        const userMessagesMap = new Map();
        
        // تحويل جميع الرسائل إلى كائنات مع timestamp
        const allMessages = (data.messages || []).map(msg => ({
            ...msg,
            timestamp: new Date(msg.created_at).getTime()
        }));
        ...
        // إنشاء رسائل المستخدم
        allMessages.forEach(msg => {
            if (msg.role === 'user') {
                userMessagesMap.set(msg.id, {
                    id: msg.id,
                    role: 'user',
                    text: msg.content || '',
                    timestamp: msg.timestamp,
        ...
        // إضافة ردود AI إلى رسائل المستخدم المناسبة
        allMessages.forEach(msg => {
            if (msg.role === 'ai' && msg.parent_id && userMessagesMap.has(msg.parent_id)) {
                const parent = userMessagesMap.get(msg.parent_id);
                parent.replies.push({
                    id: msg.id,
                    text: msg.content || '',
                    model: msg.model,
        ...
        // ترتيب الردود وتحديد currentReplyIndex
        const userMessages = Array.from(userMessagesMap.values());
        userMessages.forEach(userMsg => {
            ...
            const latestReply = userMsg.replies.find(r => r.is_latest === true);
            userMsg.currentReplyIndex = latestReply
                ? userMsg.replies.indexOf(latestReply)
                : (userMsg.replies.length > 0 ? userMsg.replies.length - 1 : -1);
        });
        
        // ترتيب رسائل المستخدم زمنياً
        ...
        return {
            id: data.conversation?.id || convId,
            title: data.conversation?.title || 'New Conversation',
            messages: userMessages,   // ← الهيكل المتفرع
            files: files.length ? files : [{ name: 'index.html', content: '// Start coding...' }],
            hasActivity: false,
            created_at: data.conversation?.created_at || new Date().toISOString(),
            updated_at: data.conversation?.updated_at || new Date().toISOString()
...
async function loadSharedMessage(shareCode) {
    try {
        ...
        if (data.error || !data.message_content) {
            showToast('Message not found or expired', 'error');
            return;
        }
    ...
    messages: [
        {
            id: 'shared_user_msg',
            role: 'user',
            text: data.user_message,  // ✅ رسالة المستخدم الحقيقية
    ...
    files: data.files_snapshot || []
        ...
        // ✅ استبدال convs المؤقت
        const originalConvs = convs;
        convs = [tempConv];
        ...
        // ✅ استخدام renderMessages() كما هي دون تغيير
        renderMessages();
        
        // إخفاء عناصر غير ضرورية للمشاهدة فقط
        ...
        // إضافة footer للمشاركة (بعد renderMessages)
        ...
        // استعادة convs الأصلية (اختياري، لمنع الخلط)
        // convs = originalConvs;
...
async function shareMessage(fullText, modelName, convId, messageId, userMessage) {
    if (navigator.share) {
        try {
            await navigator.share({
                title: 'CodeAI Response',
...
if (duration !== null && duration > 0) {
    ...
    if (userMsg.replies.length > 0) {
        ...
        if (isLast && !serverFinished && streamingConvId === activeId) {
            ...
            addMessageActions(contentEl, displayText, aiReply.model, conv.id, aiReply.id);
...
if (contentEl && lastReply) {
    ...
    addMessageActions(contentEl, extractDisplayText(fullMarkdownBuffer), currentStreamModel, activeId, lastReply.id);
...
function addMessageActions(msgElement, fullText, modelName, convId, messageId) {
...
shareBtn.onclick = () => {
    ...
    shareMessage(fullText, modelName, convId, messageId, userMessage);
};
    ...
    document.addEventListener('click', (e) => {
        if (!moreBtn.contains(e.target) && !popover.contains(e.target)) {
            popover.classList.remove('show');
        }
    });
