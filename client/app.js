const APP_VERSION = 'v1.282.34'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
async function saveConversationToServer(conversation) {
    // تحويل الهيكل الجديد إلى الرسائل المسطحة مع parent_id
    const flatMessages = [];
    for (const userMsg of conversation.messages) {
        ...
        for (let i = 0; i < userMsg.replies.length; i++) {
            ...
            flatMessages.push({
                ...
                parent_id: userMsg.id,          // ✅ هذا العمود الجديد
                reply_index: i,
...
console.log("🔍 token:", localStorage.getItem('token')); // أو اسم الـ token عندك
    
    // إرسال flatMessages إلى السيرفر (مع تحديث الرسائل القديمة والجديدة)
...
if (duration !== null && duration > 0) {
    ...
    if (userMsg.replies.length > 0) {
        ...
        if (isLast && !serverFinished && streamingConvId === activeId) {
            ...
            addMessageActions(contentEl, displayText, m.model, conv.id, m.id);
...
function finishMessageProcessing() {
    ...
    if (activeId && currentAiMsgElement) {
        ...
        if (conv && conv.messages.length > 0) {
            ...
            // ✅ استخدام currentStreamModel الذي تم تحديثه من السيرفر
            lastMsg.model = currentStreamModel || "Gemini 3 Flash";
            
            
            
            tasks.duration = durationSeconds;
            lastMsg.tasks = tasks;
            ...
            lastMsg.filesSnapshot = currentFilesSnapshot;
...
const contentEl = currentAiMsgElement.querySelector('.ai-content');
if (contentEl) {
    // نعرض فقط محتوى النص (بدون tasksHTML لأن الحاوية موجودة بالفعل)
    contentEl.innerHTML = html;
    addMessageActions(contentEl, extractDisplayText(fullMarkdownBuffer), currentStreamModel, activeId, lastMsg.id);
