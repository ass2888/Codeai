const APP_VERSION = 'v1.281.54'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
function updatePreviewButtonVisibility() {
    ...
    if (isImageFile(currentFile.name)) {
        ...
        runFab._pdfHandler = () => window.open(fileUrl, '_blank');
...
function updateActiveTab() {
    ...
    if (ext === "docx" || ext === "pptx") {
        ...
        // عرض المستند في iframe باستخدام Google Docs Viewer
...
function isArabic(text) {
    ...
    conv.messages.forEach((m, index) => {
        ...
        // إذا كانت هذه هي الرسالة الأخيرة والستريمنج ما زال جارياً
        ...
        if (m.role === 'user') {
            ...
            // ✅ استخراج النص المنقى (بدون أكواد الملفات)
            let displayText = m.text || '';
            
            // إذا كان النص يحتوي على أكواد (للمحادثات القديمة)، قم بتنقيته
            if (displayText.includes('<FILE') || displayText.includes('<REPLACE') || displayText.includes('<ADD_TO')) {
                displayText = extractDisplayText(displayText);
                
                // ✅ تحديث النص في الذاكرة فقط (لا نرسل للسيرفر هنا)
                m.text = displayText;
            }
            
            // ✅ تحويل النص المنقى إلى HTML
            let htmlContent = typeof marked !== 'undefined' ? marked.parse(displayText) : displayText;
            
            // ✅ تحديد الاتجاه بناءً على النص المنقى
            ...
            if (isStreaming && index === conv.messages.length - 1) {
                ...
                // للمحادثات القديمة: إضافة الأزرار (استخدم displayText المنقى)
...
function finishMessageProcessing() {
    ...
    if (activeId && currentAiMsgElement) {
        ...
        if (conv && conv.messages.length > 0) {
            ...
            // ✅ استخراج النص المعروض فقط (بدون أكواد الملفات)
            const displayText = extractDisplayText(fullMarkdownBuffer);
            
            lastMsg.text = displayText;  // حفظ النص المنقى فقط
            lastMsg.model = currentStreamModel || "Gemini 3 Flash";
            
            // ✅ حفظ النص المنقى فقط في Supabase
