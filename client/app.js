const APP_VERSION = 'v1.282.02'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
function extractReferencesFromText(markdownText) {
    const refRegex = /^\[(\d+)\]\s+(.+?)\s*-\s*(https?:\/\/\S+)/gm;
    const results = [];
    let match;
    while ((match = refRegex.exec(markdownText)) !== null) {
        results.push({
...
function addSearchSourcesButton(msgElement, results) {
    ...
    // منع التكرار
    if (actionsDiv.querySelector('.sources-toggle-btn')) return;
    ...
    dropdown.innerHTML = results.map(r => 
        `<a href="${r.url}" target="_blank">[${r.index}] ${r.title}</a>`
    ).join('');
...
if (!currentSearchResults || !currentSearchResults.length) {
    currentSearchResults = extractReferencesFromText(fullMarkdownBuffer);
}
