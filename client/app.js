const APP_VERSION = 'v1.281.80'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
...
function createTaskListContainer(msgElement) {
    ...
    // نضيف مكان لقائمة المهام
    const tasksDiv = document.createElement('div');
    tasksDiv.className = 'tasks-list';
    tasksDiv.innerHTML = ''; // فارغ في البداية
    content.appendChild(tasksDiv);

    container.appendChild(header);
    container.appendChild(content);

    // إدراج الحاوية قبل الـ ai-content
    const aiContent = msgElement.querySelector('.ai-content');
    if (aiContent) {
        msgElement.insertBefore(container, aiContent);
    } else {
