  ...
  try {
    ...
    const rawTitle = await callGemmaWithProtection({
      ...
      modelConfig: {
        ...
        maxOutputTokens: 10,    // صغير جدًا – لا يسمح بالكلام الزائد
        topP: 0.9,
        topK: 40
      },
      systemPrompt: systemInstruction,
...
async function generateMemorySummary(messages, existingSummary = '') {
  console.log(`🧠 [MEMORY-DEBUG] generateMemorySummary called. messages.length=${messages?.length}, existingSummary length=${existingSummary?.length}`);
  
  if (!messages || messages.length === 0) {
    console.log(`🧠 [MEMORY-DEBUG] No messages, returning existing summary.`);
    return existingSummary;
  }
  
  const gemmaKey = getSafeKeyForModel('gemma');
  if (!gemmaKey) {
    console.warn("🧠 [MEMORY-DEBUG] No Gemma key for memory summary, returning existing");
    return existingSummary;
  }
  ...
  console.log(`🧠 [MEMORY-DEBUG] Conversation text length: ${conversationText.length} characters`);
...
Memory summary:`;
  ...
  console.log(`🧠 [MEMORY-DEBUG] Sending request to Gemma API. Prompt length: ${prompt.length}`);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`🧠 [MEMORY-DEBUG] Gemma API error ${response.status}: ${errorText.substring(0, 200)}`);
      throw new Error(`API error ${response.status}`);
    }
    
    const data = await response.json();
    const newSummary = data.candidates?.[0]?.content?.parts?.[0]?.text || existingSummary;
    console.log(`🧠 [MEMORY-DEBUG] Gemma returned summary length: ${newSummary.length}`);
    console.log(`🧠 [MEMORY-DEBUG] Summary content: "${newSummary.substring(0, 200)}..."`);
    return newSummary.trim();
  } catch (err) {
    console.error("🧠 [MEMORY-DEBUG] Memory summary error:", err.message);
    return existingSummary;
  }
}
  ...
  generateMemorySummary(chunkToSummarize, mem.summary)
    .then(newSummary => {
      ...
      if (newSummary && newSummary !== mem.summary) {
        ...
        const { error } = await supabase.from('conversations').update({ summary: newSummary }).eq('id', convId);
        if (error) {
