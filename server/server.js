...
async function buildPPTX(content) {
  ...
  slides.forEach((slide, index) => {
    ...
    // إذا كان هناك فقرات متعددة
    if (slide.paragraphs && Array.isArray(slide.paragraphs)) {
      slide.paragraphs.forEach((paragraph, pIndex) => {
        const textOptions = {
          x: 0.5,
          y: yPosition,
          w: 9,
    ...
    // إذا كان هناك نص واحد فقط
      ...
      const textOptions = {
        ...
        w: 9,
        h: 6.5,
        fontSize: slide.fontSize || 18,
        bold: slide.bold || false,
        italic: slide.italic || false,
        color: slide.color || "000000",
        align: slide.align || (hasArabic ? "right" : "left"),
        valign: "top",
        wrap: true,
        rtl: hasArabic || slide.align === "right"
      };
      
      s.addText(slide.text, textOptions);
    }
    ...
    if (slide.title) {
      const titleOptions = {
        ...
        w: 9,
        h: 0.6,
        fontSize: slide.titleFontSize || 24,
        bold: true,
        color: slide.titleColor || "000000",
        align: slide.titleAlign || (hasArabic ? "right" : "center")
      };
      s.addText(slide.title, titleOptions);
    }
  });
