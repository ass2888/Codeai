...
import { extract } from 'pptx-extract';
...
`;
    ...
    // ✅ إضافة محتوى الملفات المرفقة (لأن النموذج يحتاج لرؤيتها لتعديلها)
...
async function extractTextFromPPTX(pptxBuffer) {
  try {
    // pptx-extract يعمل مع المسارات
    const tempPath = `/tmp/pptx_${Date.now()}.pptx`;
    fs.writeFileSync(tempPath, pptxBuffer);
    
    const result = await extract(tempPath);
    fs.unlinkSync(tempPath);
    
    // استخراج النص من جميع الشرائح
    let allText = "";
    if (result.slides) {
      result.slides.forEach(slide => {
        if (slide.text) allText += slide.text + "\n";
      });
    }
    return allText;
  } catch (error) {
...
async function buildPPTX(content) {
  ...
  content.slides.forEach((slide, index) => {
    ...
    if (options.bullet) {
      // ✅ للقوائم النقطية: نقسم النص إلى أسطر ونضيفها كقائمة
      ...
      // ✅ للنص العادي: نضيف النص كاملاً مع الاحتفاظ بالأسطر الجديدة
      // مكتبة pptxgenjs تدعم \n تلقائياً
