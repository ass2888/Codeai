...
async function buildPPTX(content) {
  ...
  for (const slide of content.slides) {
    ...
    // تقسيم النص إلى فقرات مع الحفاظ على المسافات
    const paragraphs = textToAdd.split(/\n\s*\n/);
    
    // حساب ارتفاع كل فقرة
    let currentY = options.y;
    const lineHeightInches = options.fontSize * options.lineSpacing / 72;
    const paraSpacingInches = 0.15;
    ...
    for (let i = 0; i < paragraphs.length; i++) {
      ...
      // إذا تجاوزت الشريحة، أضف شريحة جديدة
      if (currentY + lineHeightInches > options.y + 6.5 && i < paragraphs.length - 1) {
...
broadcast({
  ...
  console.log("🛠 Building PDF via DOCX for better formatting...");
  
  // 1. تحويل المحتوى إلى صيغة DOCX أولاً
  let docxContent;
  
  if (typeof finalContent === 'string') {
    // النص العادي - تقسيمه إلى فقرات
  ...
  } else if (finalContent.paragraphs) {
    // بالفعل في صيغة DOCX
    docxContent = finalContent;
  } else {
    // افتراضي
    docxContent = {
      paragraphs: [{ text: finalContent.text || "No content", size: 12 }]
    };
  }
  
  // 2. بناء ملف DOCX
  const docxBuffer = await buildDOCX(docxContent);
  console.log("✅ DOCX buffer created, size:", docxBuffer?.length);
  
  // 3. تحويل DOCX إلى PDF
  try {
    fileBuffer = await convertDocxToPdf(docxBuffer);
    console.log("✅ PDF buffer created via conversion, size:", fileBuffer?.length);
  } catch (conversionError) {
    console.error("❌ DOCX to PDF conversion failed, falling back to direct PDF:", conversionError);
    // Fallback: استخدام buildPDF المباشر
    fileBuffer = await buildPDF({ text: typeof finalContent === 'string' ? finalContent : finalContent.text || "" });
  }
