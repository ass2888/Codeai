...
async function authenticateUser(req, res, next) {
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    // fallback للمستخدمين غير المسجلين: اقرأ userId من query أو body
  ...
  try {
    // التحقق من التوكن عبر Supabase باستخدام مفتاح JWT السري (يمكن استخدام مكتبة supabase-js على الخادم)
...
Output ONLY a valid JSON object with the following exact structure (no markdown, no extra text):
{
  ...
  "contract": {
    ...
    "api": {
      "functions": {
        "init": "initApp()",
        "render": "renderUI(data)",
        "handleClick": "handleButton(event)"
      },
      "data_structures": {
        "state": "{ count: number, items: [] }",
...
- Assign phases accordingly; files with no dependencies go in phase 1, then those depending only on phase 1 files go in phase 2, etc.
...
- Every file from project structure MUST appear in dependency_graph.`;
  ...
  try {
    ...
    // fallback to a minimal contract
    return {
 ...
 */
async function executePhases(blueprint, projectStructure, message, conversationContext, files, clientId) {
  ...
  const auxModelConfig = MODEL_CONFIGS[auxModelKey];
  const maxParallel = PROGRAM_CONFIGS['codeai-code-r'].maxAuxModels;
  
  for (const phase of phaseNumbers) {
    const tasks = phases[phase];
    console.log(`🔄 Phase ${phase}: ${tasks.join(', ')}`);
    broadcast({ type: 'phase_start', phase, files: tasks }, clientId);
    
    // Execute tasks in this phase concurrently (limited)
    const promises = tasks.map(file => {
      return generateAuxFile(file, contract, generatedFiles, projectStructure, conversationContext, message);
    });
...
async function generateAuxFile(targetFile, contract, previouslyGenerated, projectStructure, historyContext, userMessage) {
  const auxModelKey = PROGRAM_CONFIGS['codeai-code-r'].auxModel;
  ...
  // Prepare context: contract + already generated files
  const contextText = Object.entries(previouslyGenerated).map(([name, content]) => `--- ${name} ---\n${content}`).join('\n\n');
  const template = `You are a developer implementing a single file of a web project. Use the provided contract and already created files to write EXACTLY the content of **${targetFile}**. Only output the file's code, nothing else. No explanations. No markdown.
...
PROJECT DESCRIPTION:
${projectStructure.find(p => p.file === targetFile)?.description || 'No description'}
 ...
 */
async function executeCodeR({ message, files, history, settings, visionImages, searchContext, projectStructure, clientId, convId, userId }) {
  // 1. الحصول على العقد والمخطط من المُفكّر
  ...
  const generated = await executePhases(blueprint, projectStructure, message, { history, settings }, files, clientId);
  
  // 4. تجميع الرد النهائي
  let fullResponse = '';
  for (const [file, content] of Object.entries(generated)) {
    fullResponse += `<FILE name="${file}">\n${content}\n</FILE>\n`;
  }
  ...
  await saveToSupabase(
    ...
    filesArray,
    'Code‑R',
    thoughtText,       // ← رسالة المستخدم من النموذج نفسه
    extractedTasks, 
    filesSnapshot
  );
...
// ✅ التحقق من صحة حقل البحث
if (parsed.search && parsed.search.needs) {
    if (!parsed.search.queries || !Array.isArray(parsed.search.queries) || parsed.search.queries.length === 0) {
        // لو ما وضع استعلامات نستخدم رسالة المستخدم
...
broadcast({ 
    ...
    message, 
    fullResponse,
    files,                           // ✅ استخدام الملفات الفعلية
    finalModelName,
    executionContext.thoughtText,
    extractedTasks,                  // ✅ تم استخراجها سابقاً
    filesSnapshot                    // ✅ تم إنشاؤها سابقاً
);
...
app.post('/api/increment-usage', async (req, res) => {
    ...
    try {
        await ensureUser(userId);
        const today = new Date().toISOString().split('T')[0];
        const { data } = await supabase.from('user_limits').select('*').eq('user_id', userId).maybeSingle();
        ...
        if (!data || data.last_reset_date !== today) {
            ...
            const newBonusQuota = Math.min(newStreak, MAX_BONUS_QUOTA);
            const newRecord = { user_id: userId, daily_count: 1, last_reset_date: today, streak: newStreak, last_active_date: today, bonus_quota: newBonusQuota };
            await supabase.from('user_limits').upsert(newRecord);
            return res.json(newRecord);
        }
        
        const newCount = data.daily_count + 1;
        await supabase.from('user_limits').update({ daily_count: newCount, last_active_date: today, updated_at: new Date().toISOString() }).eq('user_id', userId);
        res.json({ ...data, daily_count: newCount });
    } catch (err) {
        console.error("❌ Error:", err);
        res.status(500).json({ error: err.message });
    }
...
for (const msg of messages) {
  await supabase.from('messages').insert({
    ...
    tasks: msg.tasks || null,           // ← أضف هذا
    files_snapshot: msg.filesSnapshot || null, // ← أضف هذا
