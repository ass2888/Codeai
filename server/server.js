...
- "edit_type" MUST be "edit" if:
  ...
  * There are existing files in the ATTACHED FILES (not empty placeholders).
...
if (parsed.search && parsed.search.needs) {
    ...
    if (parsed.task_type === "code" && parsed.project_structure) {
      ...
      // تأكد من وجود edit_type، وإذا لم يحدده النموذج نستنتجه
if (!parsed.edit_type) {
  const hasContentfulFiles = files && files.some(f => 
    f.content && f.content.trim().length > 20 && 
    f.content.trim() !== '// Start coding...'
  );
  parsed.edit_type = hasContentfulFiles ? 'edit' : 'build';
}
...
return {
  ...
  project_structure: parsed.project_structure
    ...
    parsed.is_quiz = parsed.is_quiz || false;
    ...
    // ⚡️ فحص احتياطي للاختبارات قبل كل شيء
if (/اختبار|quiz|test|exam|أسئلة|multiple.choice/i.test(message)) {
  return {
    is_code_related: false,
    task_type: "general",   // أو "code" إذا أردت – لا يؤثر لأن is_quiz هو الفيصل
    is_quiz: true,
    edit_type: "build",
    project_structure: []
  };
...
// ثم باقي الكود الحالي للمشاريع الجديدة... (نفس ما كان)
...
if (!conversationMemory[convId]) {
  conversationMemory[convId] = {
    summary: "",
    lastSummarizedCount: 0,   // عدد الرسائل التي تم تلخيصها آخر مرة
    messageCount: 0
  };
}

// تحديث عدد الرسائل (نعتمد على length من history المرسل)
const currentMessageCount = history.length;
const mem = conversationMemory[convId];
mem.messageCount = currentMessageCount;

// إذا وصلنا لمضاعف جديد من 6 رسائل (أو تجاوزنا العدد)، ننشئ/نحدّث الذاكرة
if (currentMessageCount >= CHUNK_SIZE && currentMessageCount % CHUNK_SIZE === 0 &&
    mem.lastSummarizedCount !== currentMessageCount) {
  const startIdx = currentMessageCount - CHUNK_SIZE;
  const chunkToSummarize = history.slice(startIdx, currentMessageCount);
  
  // تشغيل التلخيص والحفظ في الخلفية (لا ننتظرها)
  ...
  console.log(`🧠 Memory summary update started in background.`);
}
