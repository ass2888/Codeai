...
app.post('/api/stt', upload.single('audio'), async (req, res) => {
  console.log("🎤 ===== STT REQUEST RECEIVED =====");
    ...
    console.log("🔑 Checking R1 key:", process.env.R1 ? '✅ Exists' : '❌ Missing');
    if (!req.file) {
  ...
  try {
    ...
    if (!fs.existsSync(filePath)) {
            ...
            return res.status(500).json({ error: 'FILE_NOT_FOUND' });
    }
    ...
    const groqRes = await fetch(
      'https://api.groq.com/openai/v1/audio/transcriptions',
      {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${process.env.R1}`,
          ...fd.getHeaders()
        },
        body: (() => {
          ...
          fd.append('model', 'whisper-large-v3-turbo'); // أو whisper-large-v3
          return fd;
        })()
      }
    );
    ...
    if (!groqRes.ok) {
            ...
            return res.status(groqRes.status).json({ error: 'GROQ_API_ERROR', details: errorText });
        }
        
        
    const data = await groqRes.json();
    
    console.log("✅ Groq API success, transcription:", data.text ? data.text.substring(0, 100) : 'EMPTY');
