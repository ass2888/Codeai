...
async function ensureArabicFont() {
  ...
  const allFonts = fs.existsSync(fontDir) ? fs.readdirSync(fontDir) : [];
allFonts.forEach(f => fs.unlinkSync(path.join(fontDir, f)));
if (!fs.existsSync(fontPath)) {
    console.log("📥 Downloading Arabic font...");
    const https = await import('https');
    const file = fs.createWriteStream(fontPath);
    await new Promise((resolve, reject) => {
  const url = 'https://fonts.gstatic.com/s/notosansarabic/v18/nwpxtLGrOAZMl5nJ_wfgRg3DrWFZWsnVBJ_sS6tlqHHFlhQ5l3sQWIHPqzCfyGyvu3CBFQLaig.ttf';
  https.default.get(url, (res) => {
    if (res.statusCode === 301 || res.statusCode === 302) {
      https.default.get(res.headers.location, (res2) => {
        res2.pipe(file);
