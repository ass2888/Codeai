...
// ==========================================
async function migrateFilesSnapshotToCheckpoints() {
  console.log('🔄 [MIGRATION] Checking for legacy files_snapshot data...');
  
  try {
    // 1. جلب جميع الرسائل التي تحتوي على files_snapshot غير فارغ
    const { data: messagesWithSnapshot, error: fetchError } = await supabase
      .from('messages')
      .select('id, conv_id, files_snapshot, created_at, role, model')
      .not('files_snapshot', 'is', null)
      .order('created_at', { ascending: true });

    if (fetchError) {
      console.error('❌ [MIGRATION] Failed to fetch messages:', fetchError.message);
      return;
    }

    if (!messagesWithSnapshot || messagesWithSnapshot.length === 0) {
      console.log('✅ [MIGRATION] No legacy files_snapshot data found.');
      return;
    }

    console.log(`📦 [MIGRATION] Found ${messagesWithSnapshot.length} messages with files_snapshot.`);

    // 2. تجميع اللقطات حسب المحادثة
    const checkpointsByConv = {};
    for (const msg of messagesWithSnapshot) {
      if (!checkpointsByConv[msg.conv_id]) {
        checkpointsByConv[msg.conv_id] = [];
      }

      // إنشاء كائن checkpoint من بيانات الرسالة
      const checkpoint = {
        id: `ai_${msg.id}`,
        timestamp: new Date(msg.created_at).getTime(),
        source: 'ai',
        model: msg.model || 'AI',
        description: `Checkpoint from AI response (${new Date(msg.created_at).toLocaleString()})`,
        filesSnapshot: msg.files_snapshot,
        changes: { created: [], modified: [], deleted: [] } // سنحسبها لاحقاً أو نتركها فارغة
      };
      checkpointsByConv[msg.conv_id].push(checkpoint);
    }

    // 3. حفظ checkpoints في جدول conversations لكل محادثة
    for (const [convId, checkpoints] of Object.entries(checkpointsByConv)) {
      // ترتيب الـ checkpoints من الأقدم إلى الأحدث
      checkpoints.sort((a, b) => a.timestamp - b.timestamp);

      // حساب التغييرات لكل checkpoint (بمقارنة مع السابق)
      for (let i = 0; i < checkpoints.length; i++) {
        const prevSnapshot = i > 0 ? checkpoints[i-1].filesSnapshot : null;
        const currentSnapshot = checkpoints[i].filesSnapshot;
        if (prevSnapshot) {
          checkpoints[i].changes = getChangesSummary(prevSnapshot, currentSnapshot);
        }
      }

      // جلب الـ checkpoints الحالية للمحادثة (إن وجدت)
      ...
      if (convError && convError.code !== 'PGRST116') { // PGRST116 = not found
        console.error(`❌ [MIGRATION] Error fetching conversation ${convId}:`, convError.message);
        continue;
      }

      const existingCheckpoints = conv?.checkpoints || [];
      // دمج الـ checkpoints الجديدة مع القديمة (تجنب التكرار حسب id)
      const mergedMap = new Map();
      [...existingCheckpoints, ...checkpoints].forEach(cp => mergedMap.set(cp.id, cp));
      const mergedCheckpoints = Array.from(mergedMap.values());
      mergedCheckpoints.sort((a, b) => a.timestamp - b.timestamp);

      // تحديث المحادثة
      const { error: updateError } = await supabase
        .from('conversations')
        .update({ checkpoints: mergedCheckpoints })
        .eq('id', convId);

      if (updateError) {
        console.error(`❌ [MIGRATION] Failed to update conversation ${convId}:`, updateError.message);
      } else {
        console.log(`✅ [MIGRATION] Migrated ${checkpoints.length} checkpoints to conversation ${convId}`);
      }
    }

    // 4. (اختياري) حذف عمود files_snapshot من messages بعد الترحيل
    // يتم التعليق عليه حالياً، يمكنك تنفيذه يدوياً بعد التأكد من نجاح الترحيل
    // await supabase.rpc('drop_files_snapshot_column'); // تحتاج دالة SQL مخصصة
...
setTimeout(() => {
  migrateFilesSnapshotToCheckpoints().catch(console.error);
}, 3000);
