...
async function buildNativeSystemPrompt({ message, history, files, visionImages }) {
  let systemPrompt = `You are a helpful AI assistant. Answer directly and naturally in the user's language.`;

  let historyText = "";
  if (history && Array.isArray(history)) {
    historyText = history.slice(-2).map(msg => {
      const role = msg.role === 'user' ? 'User' : 'Assistant';
      return `${role}: ${(msg.text || msg.content || '').substring(0, 300)}`;
    }).join("\n");
  }
...
if (isNativeMode) {
  ...
  built = await buildNativeSystemPrompt({ message, history, files, visionImages });
} else {
