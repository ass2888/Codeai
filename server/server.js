...
console.log(`🔑 Using Gemma Key: ${gemmaKeyInfo.id} (Different from Gemini key)`);
...
console.log(`📝 User message: ${message.substring(0, 100)}...`);
  ...
  try {
   ...
   console.log(`🤖 Selected Gemini model: gemini-3-flash`);
...
console.log(`📊 Final Gemini usage for ${activeKeyInfo.id}: RPM=${usageStats[activeKeyInfo.id].gemini.rpm}, TPM=${usageStats[activeKeyInfo.id].gemini.tpm}`);
    ...
    const isFirstAIResponse = conversation && conversation.history && conversation.history.length === 1;
