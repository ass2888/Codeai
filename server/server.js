...
CRITICAL: 
- DO NOT regenerate files that already exist (${generatedFileNames.join(', ')}).
- Output EXACTLY ONE <FILE name="..."> block for each of the remaining files above.
- READ the existing files CAREFULLY to ensure perfect integration.
- Use the same class names, function names, and conventions.
`;
    } else {
        continuationTask = `
...
⭐ ALL EXISTING PROJECT FILES (READ CAREFULLY):
${filesContext}
...
4. Output ONLY the required <FILE> blocks, with NO extra explanations.
...
Continue now by generating the remaining files. Start immediately with <FILE name="...">.`,
        ...
        userPrompt: `Create the remaining files as specified.`
    };
}
...
function buildContinuationPromptForRemainingFiles(pendingFiles, currentFiles, originalSystemPrompt) {
    ...
    const filesContext = currentFiles.map(f => 
        `--- FILE: ${f.name} (EXISTS - DO NOT RECREATE) ---\n${f.content}\n--- END FILE ---`
    ).join('\n\n');
...
6. Do NOT add any introductory text, explanations, or summaries.

Original system context: ${originalSystemPrompt?.substring(0, 500)}...`,

        userPrompt: `Create the remaining files: ${pendingFiles.map(p => p.file).join(', ')}. Start immediately with <FILE name="...">.`
    };
}
