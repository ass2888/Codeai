...
async function authenticateUser(req, res, next) {
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    // إذا لم يوجد توكن، نسمح بمرور مؤقت للتوافق مع المستخدمين غير المسجلين (اختياري)
    // لكن لاحظ أن المستخدم لن يتم التعرف عليه
    req.userId = null;
    return next();
  }
...
app.get('/api/conversation', async (req, res) => {
  ...
  // استخدم userId بدلاً من req.query.userId
});
  if (!convId || !userId) {
