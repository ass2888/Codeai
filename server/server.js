...
if (searchContext) {
  systemPrompt += `\n\n--------------------------------------------------\nCITATION RULES: Use [1], [2] for facts from search results. Add ## References at the end.`;
}
...
- At the very end of your entire response, add a "## References" section listing each cited number with its title and URL:
  [1] Title - URL
  [2] Title - URL
...
async function performSearch(query, maxResults = 5) {
  ...
  try {
    ...
    console.log(`📦 [SEARCH] Response data keys: ${Object.keys(data).join(', ')}`);
    console.log(`📝 [SEARCH] AbstractText: "${data.AbstractText?.substring(0, 100) || 'EMPTY'}"`);
    console.log(`📝 [SEARCH] RelatedTopics count: ${data.RelatedTopics?.length || 0}`);
    ...
    if (data.AbstractText && data.AbstractText.trim()) {
      ...
      console.log(`✅ [SEARCH] Added Abstract result`);
    }
    ...
    if (data.RelatedTopics && Array.isArray(data.RelatedTopics)) {
      ...
      console.log(`✅ [SEARCH] Added ${results.length} total results`);
    }
    
    if (results.length === 0) {
      console.warn(`⚠️ [SEARCH] No results found for "${query}"`);
      return null;
    }
    ...
    console.log(`📋 [SEARCH] Formatted results length: ${formatted.length} chars`);
    console.log(`📋 [SEARCH] First 200 chars: ${formatted.substring(0, 200)}`);
