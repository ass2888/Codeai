...
async function buildPPTX(content) {
  ...
  const defaultOptions = {
    ...
    lineSpacing: 1.2,
    valign: "top",
    wrap: true,           // احتياطي للالتفاف التلقائي
    autoFit: 'shrink'     // احتياطي لمنع تجاوز النص للصندوق
  };

  for (const slideData of content.slides) {
    const options = { ...defaultOptions, ...(slideData.options || {}) };

    // 1. تنظيف النص وتوحيد فواصل الأسطر
    const rawText = (slideData.text || "").trim();
    // 2. تقسيم النص إلى أسطر (تدعم \n و \r\n)
    const lines = rawText ? rawText.split(/\r?\n/) : [" "];
    // 3. تحويل الأسطر إلى مصفوفة يفهمها PptxGenJS كفقـرات منفصلة
    const textToAdd = lines.map(line => ({ text: line || " ", options: { breakLine: true } }));
