...
app.get('/api/share/:code', async (req, res) => {
  ...
  try {
    // 1. جلب بيانات المشاركة من جدول shared_messages
    ...
    // 2. جلب محتوى رسالة الذكاء الاصطناعي
    const { data: aiMsg, error: aiError } = await supabase
      .from('messages')
      .select('content, model, files_snapshot, created_at')
    ...
    // 3. جلب محتوى رسالة المستخدم (اختياري)
    ...
    // 4. إرجاع البيانات للواجهة الأمامية
    res.json({
      message_content: aiMsg.content,
      user_message: userMessageContent,
      model_name: aiMsg.model || 'AI',
      files_snapshot: aiMsg.files_snapshot || [],
