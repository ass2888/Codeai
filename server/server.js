  ...
  if (jsonMatch) {
    try {
      ...
      if (parsed.type === 'document' && parsed.document) {
        if (parsed.user_message) onChunk(parsed.user_message);
...
async function handleDocumentGeneration({
  ...
  previewUrl: document_type === "pdf" 
   ...
   await saveToSupabase(userId, convId, null, null, [{
  name: path.basename(filePath),
  url: filePath,
...
try {
  ...
  await sendResponseUnified({
    ...
    provider,
    keyInfo: { ...finalKeyInfo, clientId, convId, userId },
    prompt: fullPrompt,
    ...
    onComplete: (full) => {
      ...
      saveToSupabase(userId, convId, message, full);
    },
    maxRetries: 1
  });
} else {
