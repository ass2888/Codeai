...
async function buildPDF(content) {
  ...
  for (let i = 0; i < pageCount; i++) {
    await page.evaluate((i) => window.scrollTo(0, i * 1123), i);
    const screenshot = await page.screenshot({
      type: "png",
      clip: { x: 0, y: i * 1123, width: 794, height: 1123 }
    });
    previewImages.push(screenshot.toString("base64"));
  }

  await browser.close();

  return { pdfBuffer, previewImages };
}
...
async function handleDocumentGeneration({
  ...
  const { pdfBuffer, previewImages } = await buildPDF({ text: finalContent });
  ...
  previewImages.forEach((img, i) => {
    fs.writeFileSync(path.join(previewDir, `page_${i + 1}.png`), Buffer.from(img, "base64"));
  });
}
...
try {
  ...
  await sendResponseUnified({
    ...
    provider,
    keyInfo: { ...finalKeyInfo, clientId, convId },
    prompt: fullPrompt,
