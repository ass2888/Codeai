...
async function sendResponseUnified({
  ...
  if (jsonMatch) {
    ...
    console.log("🔍 parsed.user_message:", parsed.user_message);
      if (parsed.type === 'document' && parsed.document) {
        console.log("📤 Sending user_message chunk:", parsed.user_message);
        onChunk(parsed.user_message || "✅ تم إنشاء الملف بنجاح!");
  ...
  if (!fullResponse.includes('"type": "document"') && !fullResponse.includes('"type":"document"')) {
    onChunk(text);
  }
}
          }
  ...
  let handled = false;
    ...
    : keyInfo.clientId,
              ...
              onChunk("\n\n✅ Document created! Check the editor tabs.");
              onComplete?.("");
              handled = true;
            }
          } catch (e) {
  ...
  return new Promise((resolve, reject) => {
    ...
    lines.forEach(line => {
      if (line.trim() === "") {
        ...
        doc.fontSize(13).text(line, { align: "left" });
      }
    });
...
async function buildPDFPreviews(pdfPath, outputDir) {
...
broadcast({
  ...
  console.log("📂 Final PDF path:", filePath);
    ...
    if (!fs.existsSync(dir)) {
      console.log("📁 Generated folder not found, creating...");
      fs.mkdirSync(dir);
    }
    ...
    if (document_type === "pdf") {
  const safeName = path.basename(filePath, ".pdf");
  const previewDir = path.join(PUBLIC_DIR, "generated", safeName + "_preview");
  console.log("🖼 Building previews...");
  try {
    await buildPDFPreviews(fullPath, previewDir);
    ...
    console.log("📤 Broadcasting file to client...");
  ...
  previewUrl: document_type === "pdf" 
    ? `/generated/${path.basename(filePath, ".pdf")}_preview` 
    : null
},
      convId
    }, clientId);
  ...
  name: path.basename(filePath),
  url: filePath,
  previewUrl: document_type === "pdf" ? `/generated/${path.basename(filePath, ".pdf")}_preview` : null,
  type: document_type
}]);
