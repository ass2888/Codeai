...
async function buildPPTX(content) {
  ...
  content.slides.forEach((slide, index) => {
    ...
    if (options.bullet) {
      // قائمة نقطية - تقسيم النص إلى أسطر
      const lines = textToAdd.split('\n').filter(line => line.trim());
      const bulletPoints = lines.map(line => ({
        text: line.replace(/^[•\-*\d+\.\s]*/, ''), // إزالة الرموز النقطية والترقيم
        options: { indentLevel: 0, bullet: true }
      }));
      
      s.addText(bulletPoints, textOptions);
    } else {
      // نص عادي - نضيف النص كاملاً مع الاحتفاظ بالأسطر الجديدة
      // استخدام \n داخل النص يعمل تلقائياً مع pptxgenjs
