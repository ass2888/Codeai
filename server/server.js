...
{
  "type": "document",
  "user_message": "Your friendly response to the user",
  "document": {
    ...
    "content": {
      ...
      // For PPTX - FULL CONTROL:
      "slides": [
        {
          "text": "Slide content with formatting",
          "options": {
            "fontSize": 24,
            "bold": true,
            "italic": false,
...
PPTX SPACING OPTIONS:
- margin: number (in inches) - space around the text box (default: 0.2)
- lineSpacing: number (multiplier) - space between lines (default: 1.2)
- paraSpacing: number (in points) - space between paragraphs (default: 12)

DOCX SPACING OPTIONS:
- spacing.before: number (in twips, 1/20 point) - space before paragraph (default: 200)
- spacing.after: number (in twips) - space after paragraph (default: 200)
- spacing.line: number (in twips) - line spacing (default: 276 = 1.15 lines)
- margin.left/right/top/bottom: number (in twips) - paragraph indentation

EXAMPLES:

For PPTX with spacing:
{
  "type": "document",
  "user_message": "Here's your presentation!",
  "document": {
    "document_type": "pptx",
    ...
    "content": {
      "slides": [
        ...
        {
          "text": "First paragraph with normal spacing.\n\nSecond paragraph with a blank line in between.\n\nThird paragraph.",
          "options": {
            "fontSize": 20,
            "margin": 0.3,
            "lineSpacing": 1.2,
...
For DOCX with spacing:
{
  "type": "document",
  "user_message": "Here's your Word document!",
  "document": {
    "document_type": "docx",
    ...
    "content": {
      "paragraphs": [
        ...
        {
          "text": "This is a normal paragraph with standard line spacing.",
          "spacing": { "line": 360 }
        },
        {
          "text": "This paragraph has extra space before and after.",
...
async function buildPPTX(content) {
  ...
  const defaultOptions = {
    ...
    valign: "top",
    bullet: false,
    margin: 0.2,           // مسافة حول النص (بوصة)
    lineSpacing: 1.2,      // تباعد الأسطر (مضاعف)
    paraSpacing: 12        // تباعد الفقرات (نقاط)
  };
  ...
  content.slides.forEach((slide, index) => {
    ...
    // دمج الإعدادات
    const options = { ...defaultOptions, ...(slide.options || {}) };
    let textToAdd = slide.text;
    
    // معالجة النص مع الاحتفاظ بالأسطر الفارغة
    const lines = textToAdd.split('\n');
    
    // بناء قائمة من الفقرات مع الحفاظ على الأسطر الفارغة
    const paragraphs = [];
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];
      if (line.trim() === '' && i > 0 && i < lines.length - 1) {
        // سطر فارغ - نضيف فقرة فارغة لإنشاء مسافة
        paragraphs.push({ text: ' ', options: { breakLine: true, paraSpacing: options.paraSpacing } });
      } else if (line.trim() !== '') {
        paragraphs.push({ 
          text: line, 
          options: { 
    ...
    if (options.bullet) {
      ...
      const bulletPoints = paragraphs
        ...
        .map(p => ({
          ...
          options: { indentLevel: 0, bullet: true }
        }));
      ...
      // نص عادي مع فقرات
      if (paragraphs.length === 1 && paragraphs[0].text !== ' ') {
        // سطر واحد فقط
        s.addText(textToAdd, textOptions);
      } else {
        // عدة فقرات - نضيف كل فقرة على حدة مع مسافات بينها
        let currentY = options.y;
        const lineHeight = options.fontSize * options.lineSpacing / 72; // تحويل إلى بوصة
        const paraSpacingInches = options.paraSpacing / 72; // تحويل النقاط إلى بوصة
        ...
        for (let i = 0; i < paragraphs.length; i++) {
          ...
          if (paraText === ' ') {
            // فقرة فارغة - نضيف مسافة
            currentY += paraSpacingInches;
            continue;
          }
          
          // حساب ارتفاع النص
          const textHeight = estimateTextHeight(paraText, options.fontSize, options.w);
          const paraHeight = textHeight / 72; // تحويل إلى بوصة
          ...
          // تحديث الموقع للفقرة التالية
          currentY += paraHeight + paraSpacingInches;
        }
      }
    }
