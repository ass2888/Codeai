...
async function routeModel(userMessage, context) {
  const routerPrompt = `
You are a routing AI.
Your job is to select the best model.
...
Rules:
- If coding or bug fixing → coder model
- If explanation or reasoning → reasoning model
- If casual chat → fast model
`;

  const res = await callModel({
    model: "gemma-3-12b", // أو أي صغير سريع
    prompt: routerPrompt
  });
...
let finalKeyInfo = getSafeKeyForModel(finalModel);
...
console.log(`📊 Current Gemini usage: RPM=${usageStats[activeKeyInfo.id]);
