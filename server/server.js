...
async function buildPPTX(content) {
  ...
  for (const slide of content.slides) {
    ...
    // تنظيف النص: استبدال الأسطر المتعددة بفواصل HTML بسيطة
    let formattedText = textToAdd.replace(/\n/g, '<br/>');
    
    if (options.bullet) {
      const lines = textToAdd.split('\n').filter(line => line.trim());
      formattedText = lines.map(line => `• ${line.replace(/^[•\-*\d+\.\s]*/, '')}`).join('<br/>');
    }
    
    s.addText(formattedText, {
      x: options.x,
      y: options.y,
      w: options.w,
      h: options.h,
