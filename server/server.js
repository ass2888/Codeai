...
Return ONLY valid JSON in this exact format:
{
  "internal_analysis": "DETAILED TECHNICAL PLAN for the executor...",
  "user_explanation": "• problem: [what the user wants]\n• cause: [why they want it]\n• solution: [what we'll build]\n• result: [what they'll get]\n• features: [list of features]"
}

RULES FOR user_explanation:
- Use BULLET POINTS with • (not dashes or numbers)
- Each line starts with • keyword: description
- Keep it simple and motivational
- DO NOT use JSON in user_explanation - it's plain text
- DO NOT show code in user_explanation

Task intent: ${taskInfo.intent}
User message: "${message}"
${files && files.length ? `\nExisting files: ${JSON.stringify(files.map(f => f.name))}` : ''}
...
`;
  ...
  if (modelConfig.provider === "google") {
    ...
    .replace(/```json\s*/g, '')
    .replace(/```\s*$/g, '')
    .replace(/\n/g, '\\n')  // 🔥 الأهم: تحويل newlines إلى \n
    .replace(/\r/g, '\\r')
    .replace(/\t/g, '\\t')
    .replace(/\\"/g, '\\\\"') // escaping للاقتباسات
    .trim();
...
console.log("📦 Cleaned preview:", text);
  ...
  if (!jsonMatch) {
    ...
    return {
      ...
      user_explanation: {
        problem: taskInfo.fault?.summary || "Processing your request",
        cause: taskInfo.fault?.type || "Unknown",
        solution: "Proceeding with implementation",
        result: "Will apply changes directly"
      }
    };
  }
  ...
  try {
    ...
    return {
      ...
      user_explanation: {
        problem: parsed.user_explanation?.problem || taskInfo.fault?.summary || "",
        cause: parsed.user_explanation?.cause || taskInfo.fault?.type || "",
        solution: parsed.user_explanation?.solution || "Implementing solution",
        result: parsed.user_explanation?.result || "Changes applied"
      }
    };
  } catch (err) {
    ...
    return {
      internal_analysis: "Failed to parse reasoning response, proceeding directly.",
      user_explanation: {
        problem: taskInfo.fault?.summary || "Processing request",
        cause: taskInfo.fault?.type || "Direct execution",
        solution: "Continuing without detailed analysis",
        result: "Will execute based on user request"
      }
    };
  }
}
  ...
  try {
    ...
    if (reasoningData) {
      ...
      broadcast({
        ...
        text: userExplanation || "No analysis details provided."
      }, clientId);
    }
