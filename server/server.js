...
if (!finalUserPrompt || finalUserPrompt.trim().length === 0) {
    finalUserPrompt = message || "Hello"; // استخدم رسالة المستخدم الأصلية كـ fallback
}
  const messages = [
...
const wrappedOnComplete = (full) => {
    ...
    if (modelRequestedContinuation || appearsTruncated) {
        ...
        // ✅ بدء عملية الاستمرار
        // سيتم تنفيذ هذا بعد اكتمال الـ onComplete في القسم التالي من processModelCall
        // حيث يوجد شرط: if (needsContinuation && !callOptions.skipContinuation)
    }
