...
async function buildPPTX(content) {
  ...
  for (const slideData of content.slides) {
    ...
    const maxHeight = 6.5; // أقصى ارتفاع للشريحة بالبوصة
    const maxLines = Math.floor(maxHeight / lineHeightInches);
    
    // تقسيم الأسطر إلى شرائح متعددة
    for (let start = 0; start < lines.length; start += maxLines) {
      const slideLines = lines.slice(start, start + maxLines);
