...
`;
  ...
  const model = genAI.getGenerativeModel({
    ...
    temperature: 0,
    maxOutputTokens: 400,
    responseMimeType: "application/json" // ✨ يجبر النموذج على إخراج JSON فقط
  }
  });
...
// قبل إنشاء project_structure الافتراضي، أضف شرطاً يتحقق من غياب أي دلالة برمجية
const codingKeywords = /(\bcode\b|\bprogram\b|\bscript\b|\bhtml\b|\bcss\b|\bfunction\b|\bapp\b|\bgame\b|\bwebsite\b|\bweb\b|\bcomponent\b|\bdebug\b|\bfix\b|\bcreate\b|\bmake\b|\bdesign\b|\blanding page\b|\bui\b|\b interface\b|\bapi\b|\bresponsive\b)/i;
if (!codingKeywords.test(message)) {
  return { is_code_related: false, task_type: "general" };
}
