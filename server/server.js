...
const supabaseUrl = process.env.S_A_URL;
const supabaseAnonKey = process.env.S_A_ANON;

// استخدام anon key (بدون service_role) - مناسب للمشاريع بدون حسابات
export const supabaseAAA = createClient(supabaseUrl, supabaseAnonKey);
...
let currentQueueDate = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
let taskIntervalMs = 0; // سيتم حسابه من TPD
let currentTPD = 30;     // القيمة الافتراضية
let queueTimer = null;

// دالة لجلب الحد الأقصى للمهام اليومية من قاعدة البيانات
async function getDailyTasksLimit() {
  try {
    const { data, error } = await supabaseAAA
      .from('app_settings')
      .select('value')
      .eq('key', 'daily_tasks_limit')
      .single();
    if (error) throw error;
    return parseInt(data.value, 10) || 30;
  } catch (err) {
    console.error('Failed to get daily limit:', err.message);
    return 30;
  }
}

// دالة لحفظ الحد الأقصى للمهام اليومية
async function setDailyTasksLimit(limit) {
  try {
    const { error } = await supabaseAAA
      .from('app_settings')
      .upsert({ key: 'daily_tasks_limit', value: String(limit), updated_at: new Date().toISOString() })
      .eq('key', 'daily_tasks_limit');
    if (error) throw error;
    currentTPD = limit;
    // إعادة حساب الفاصل الزمني
    taskIntervalMs = (24 * 60 * 60 * 1000) / currentTPD;
    console.log(`✅ Daily tasks limit updated to ${limit} TPD (interval: ${(taskIntervalMs/1000/60).toFixed(1)} minutes)`);
    return true;
  } catch (err) {
    console.error('Failed to set daily limit:', err.message);
    return false;
  }
}
...
async function resetDailyCounterIfNeeded() {
  ...
  if (today !== currentQueueDate) {
    currentQueueDate = today;
    dailyTaskCount = 0;
    console.log(`🔄 Daily task counter reset. New date: ${today}`);
    // إعادة تشغيل الـ queue (لأنه قد يكون متوقفاً)
    if (!queueRunning) startTaskQueue();
  }
}



// تنفيذ مهمة واحدة بواسطة الوكيل المناسب



async function applyFileChanges(responseText, projectId) {
  // استخراج وإنشاء ملفات جديدة
  ...
  while ((match = fileRegex.exec(responseText)) !== null) {
    ...
    // حفظ أو تحديث الملف
    const { data: existing } = await supabaseAAA
      .from('project_files')
      .select('id')
    ...
    if (existing) {
      ...
      await supabaseAAA
        .from('project_files')
        .insert({
          id: `file_${Date.now()}_${Math.random().toString(36)}`,
  ...
  // استخراج وتطبيق REPLACE
  const replaceRegex = /<REPLACE\s+file="([^"]+)"\s*>([\s\S]*?)<\/REPLACE>/gi;
  while ((match = replaceRegex.exec(responseText)) !== null) {
    const fileName = match[1];
    const diffBlock = match[2];
    
    const { data: file } = await supabaseAAA
      .from('project_files')
      .select('content')
      .eq('project_id', projectId)
      .eq('name', fileName)
  ...
  // استخراج ADD_TO
  const addToRegex = /<ADD_TO\s+target="([^"]+)"\s+position="(start|end)"\s*>([\s\S]*?)<\/ADD_TO>/gi;
...
// الحصول على أول مهمة pending (ترتيب حسب order_index ثم created_at)
...
// دالة لحساب عدد المهام المعلقة
async function getPendingTasksCount() {
  const { count, error } = await supabaseAAA
    .from('project_tasks')
...
// الدالة الأساسية لمعالجة المهام واحدة تلو الأخرى
async function processNextTask() {
  await resetDailyCounterIfNeeded();
  
  const dailyLimit = await getDailyTasksLimit();
  if (dailyTaskCount >= dailyLimit) {
    console.log(`Daily limit reached. Next reset: tomorrow`);
    return;
  }
  ...
  dailyTaskCount++;
  await executeTaskWithRetry(task.id, 3, 5000);
  
  // حساب وقت التنفيذ التالي
  const intervalMs = (24 * 60 * 60 * 1000) / dailyLimit;
  const nextTime = new Date(Date.now() + intervalMs);
  
  // حفظ الوقت المستهدف في قاعدة البيانات
  await supabaseAAA
    .from('app_settings')
    .upsert({ 
      key: 'next_task_time', 
...
// بدء تشغيل الطابور (يُستدعى عند بدء السيرفر أو عند إضافة مهام جديدة)
...
// عند بدء السيرفر، استعادة الطابور من قاعدة البيانات
async function restoreQueueOnStartup() {
  const state = await getQueueState();
  const dailyLimit = await getDailyTasksLimit();
  const pendingTasks = await getPendingTasksCount();
  
  console.log(`🔄 Restoring queue: ${pendingTasks} pending tasks, ${state.dailyTaskCount}/${dailyLimit} executed today`);
  
  if (pendingTasks > 0 && state.dailyTaskCount < dailyLimit) {
    console.log('▶️ Restarting task queue');
    // الكرون سيتولى الباقي تلقائياً
  }
}

// استدعاء عند بدء السيرفر
restoreQueueOnStartup();

// تخزين حالة الطابور في قاعدة البيانات بدلاً من المتغيرات العامة
async function getQueueState() {
  const { data } = await supabaseAAA
    .from('app_settings')
    .select('value')
    .eq('key', 'queue_state')
    .single();
...
// تشغيل كرون كل دقيقة للتحقق من المهام
cron.schedule('* * * * *', async () => {
  const state = await getQueueState();
  const today = new Date().toISOString().split('T')[0];
  ...
  const dailyLimit = await getDailyTasksLimit();
  if (state.dailyTaskCount >= dailyLimit) return;
  
  // ✅ معالجة التأخير أولاً
  await catchUpMissedTasks();
  
  // جلب وقت التنفيذ التالي بعد إعادة الحساب
  const { data: nextTimeData } = await supabaseAAA
    .from('app_settings')
    .select('value')
    .eq('key', 'next_task_time')
    .single();
  
  const nextExecutionTime = nextTimeData?.value ? new Date(nextTimeData.value) : new Date();
  const now = new Date();
  
  // التحقق من أن الوقت الحالي هو الوقت المحدد أو بعده بقليل (تسامح 5 ثوانٍ)
  if (now >= nextExecutionTime && (now.getTime() - nextExecutionTime.getTime()) < 60000) {
    const task = await getNextPendingTask();
    if (task) {
...
async function restoreQueueOnStartup() {
  ...
  console.log(`Restoring queue: ${pendingTasks} pending tasks`);
  
  // استعادة وقت التنفيذ التالي من قاعدة البيانات
  const { data: nextTimeData } = await supabaseAAA
    .from('app_settings')
    .select('value')
    .eq('key', 'next_task_time')
  ...
  if (nextTimeData?.value) {
    const nextTime = new Date(nextTimeData.value);
    const now = new Date();
    const waitMs = Math.max(0, nextTime.getTime() - now.getTime());
    
    console.log(`Next task scheduled at: ${nextTime.toISOString()} (waiting ${Math.round(waitMs/1000/60)} minutes)`);
  }
...
async function catchUpMissedTasks() {
  const state = await getQueueState();
  const dailyLimit = await getDailyTasksLimit();
  const intervalMs = (24 * 60 * 60 * 1000) / dailyLimit; // 48 دقيقة مثلاً
  
  // جلب وقت التنفيذ التالي المخزن
  const { data: nextTimeData } = await supabaseAAA
    .from('app_settings')
    .select('value')
    .eq('key', 'next_task_time')
  ...
  if (!nextTimeData?.value) return;
  
  const scheduledTime = new Date(nextTimeData.value);
  const now = new Date();
  const delayMs = now.getTime() - scheduledTime.getTime();
  
  // إذا كان التأخير أقل من الفاصل الزمني، تنفيذ عادي
  if (delayMs < intervalMs) {
    return; // لا حاجة لإعادة جدولة
  }
  
  // حساب عدد المهام التي فاتت
  const missedTasksCount = Math.floor(delayMs / intervalMs);
  const pendingTasks = await getPendingTasksCount();
  const tasksToCatchUp = Math.min(missedTasksCount, pendingTasks);
  
  console.log(`⚠️ Server was down for ${Math.round(delayMs / 60000)} minutes`);
  console.log(`📊 Missed ${missedTasksCount} task slots, ${tasksToCatchUp} pending tasks need scheduling`);
  
  if (tasksToCatchUp === 0) return;
  
  // إعادة توزيع المهام على فترات متساوية من الآن فصاعداً
  // بدلاً من تنفيذها كلها الآن، نوزعها كل X دقيقة
  
  const newIntervalMs = Math.max(intervalMs, (24 * 60 * 60 * 1000) / dailyLimit);
  const firstTaskTime = new Date(now.getTime() + newIntervalMs);
  ...
  await supabaseAAA
    ...
    .upsert({ 
      key: 'next_task_time', 
      value: firstTaskTime.toISOString()
    });
  
  console.log(`✅ Rescheduled first task to: ${firstTaskTime.toISOString()}`);
  console.log(`📋 Remaining ${tasksToCatchUp - 1} tasks will follow every ${Math.round(newIntervalMs / 60000)} minutes`);
  
  // ملاحظة: المهام الإضافية سيتم جدولتها تلقائياً بعد كل تنفيذ
}

// نقطة نهاية لتحديث TPD


// بعد إنشاء مهمة جديدة (في /api/tasks/generate)، نستدعي startTaskQueue()
// لذا أضف في نهاية كتلة /api/tasks/generate (بعد حفظ المهام) السطر:
// startTaskQueue();



// ==========================================
// ONE-TIME MIGRATION: نقل files_snapshot من messages إلى checkpoints في conversations
// ==========================================


// دالة مساعدة لحساب التغييرات (نفس الموجودة في app4.js)
function getChangesSummary(prevSnapshot, currentSnapshot) {
    ...
    for (const [name, content] of currentMap) {
        if (!prevMap.has(name)) created.push(name);
        else if (prevMap.get(name) !== content) modified.push(name);
    }
    for (const [name] of prevMap) {
        if (!currentMap.has(name)) deleted.push(name);
    }
    return { created, modified, deleted };
}

// استدعاء الترحيل عند بدء السيرفر (مرة واحدة)
...
async function authenticateUser(req, res, next) {
    ...
    try {
        ...
        console.log('✅ [AUTH-A] Token verified.');
...
async function saveToSupabase(userId, convId, message, aiResponse, files = [], modelName = null, thoughtText = null, tasks = null, filesSnapshot = null) {
  try {
    ...
    // حفظ رسالة المستخدم ورد الذكاء الاصطناعي معاً
if (aiResponse && aiResponse.trim().length > 0) {
  await saveMessagesPair(userId, convId, message, aiResponse, modelName, thoughtText, tasks);
} else {
  // فقط رسالة مستخدم بدون رد (حالة نادرة)
...
async function saveMessagesPair(userId, convId, message, aiResponse, modelName = null, thoughtText = null, tasks = null) {
  try {
    if (!userId) return;
    
    const plainContent = extractPlainTextFromResponse(aiResponse);
    
    const convIdNumber = typeof convId === 'string' ? parseInt(convId, 10) : convId;

const { error } = await supabase.rpc('insert_message_pair', {
  p_conv_id: convIdNumber,        // ✅ يجب أن يكون رقماً
  p_user_message: message,
  p_ai_message: plainContent,
  p_model: modelName,
  p_thought: thoughtText,
  p_tasks: tasks,
});
    ...
    console.log("✅ Messages pair saved successfully");
    
  } catch (err) {
    console.error("❌ saveMessagesPair error:", err.message);
  }
}
...
const AGENT_PROMPTS = {
  // المدير العام - مسؤول عن التخطيط والتنسيق
  manager: `You are a Project Manager AI.
...
DECISION RULES:
- Complex/creative tasks → assign to analyst first
- Code implementation → assign to developer
- Testing/validation → assign to tester

OUTPUT FORMAT (ONLY JSON):
{
  "action": "assign_task",
  "task_id": "...",
  "assigned_to": "analyst|developer|tester",
...
Keep responses concise and actionable.`,
  ...
  // المحلل - مسؤول عن التحليل والمواصفات
  analyst: `You are a Business/Technical Analyst AI.

YOUR RESPONSIBILITIES:
1. Analyze user requirements deeply
2. Create detailed technical specifications
3. Break down complex features into atomic tasks
...
Be extremely detailed. Include edge cases.`,

  // المطور - مسؤول عن كتابة الكود
  developer: `You are a Senior Developer AI.

YOUR RESPONSIBILITIES:
1. Write clean, working code
2. Create or modify files using <FILE>, <REPLACE>, <ADD_TO>
...
NEVER output explanations. ONLY code in the specified format.`,

  // المختبر - مسؤول عن اختبار الجودة
  tester: `You are a QA Tester AI.

YOUR RESPONSIBILITIES:
1. Test code for bugs
...
// تعيين النماذج لكل وكيل (حسب طلبك)
const AGENT_MODELS = {
  manager: 'gemini-2.5-pro',      // مدير - نموذج قوي
  analyst: 'gemini-3.1-flash-lite-preview',  // محلل - lite
  developer: 'gemini-3-flash',    // منفذ - flash
  tester: 'gemini-3.1-flash-lite-preview'    // مختبر - lite
};

function countToolWords(responseText) {
    let toolCount = 0;
    let totalChars = 0;
    
    const fileRegex = /<FILE[^>]*>([\s\S]*?)<\/FILE>/gi;
    const replaceRegex = /<REPLACE[^>]*>([\s\S]*?)<\/REPLACE>/gi;
    const addToRegex = /<ADD_TO[^>]*>([\s\S]*?)<\/ADD_TO>/gi;
    ...
    return { toolCount, totalChars, estimatedTokens: Math.ceil(totalChars / 4) };
}

// تنفيذ مهمة باستخدام الوكيل المناسب مع System Prompt خاص
async function executeTaskWithAgent(taskId, agentRole, clientId = null) {
  ...
  broadcast({ type: 'task_status_update', taskId, status: 'in_progress', projectId: task.project_id });
  
  // جلب ملفات المشروع
  const { data: files } = await supabaseAAA
    .from('project_files')
    .select('name, content')
  ...
  // اختيار النموذج المناسب للوكيل
  const modelId = AGENT_MODELS[agentRole] || 'gemini-3-flash';
  const systemPrompt = AGENT_PROMPTS[agentRole] || AGENT_PROMPTS.developer;
  
  const keyInfo = getSafeKeyForModel(modelId);
  if (!keyInfo) throw new Error(`No key for model ${modelId}`);
  
  const userPrompt = `PROJECT FILES:
...
${agentRole === 'developer' ? 'Output ONLY code using <FILE> or <REPLACE> tags.' : ''}`;
  ...
  try {
    ...
    executionResult = result.response.text();
    // داخل دالة executeTaskWithAgent أو sendResponseUnified
// عند اكتشاف علامة REPLACE أو FILE

const toolRegex = /<(REPLACE|FILE|ADD_TO)\s+[^>]*>/g;
let match;
while ((match = toolRegex.exec(executionResult)) !== null) {
    const toolType = match[1];
    // استخراج اسم الملف
    const fileNameMatch = aiResponse.match(new RegExp(`${toolType}\\s+(?:file|name|target)="([^"]+)"`));
    ...
    broadcast({
        type: 'agent_action',
        taskId: task.id,
        action: toolType.toLowerCase(),
        file: fileName,
        message: `${toolType} ${fileName}`
    }, clientId);
}
    // تطبيق تغييرات الملفات (للمطور فقط)
    if (agentRole === 'developer') {
      await applyFileChanges(executionResult, task.project_id);
    }
    
    // تحديث المهمة إلى done
    await supabaseAAA
      .from('project_tasks')
      .update({ 
        status: 'done', 
        completed_at: new Date().toISOString(),
        result: { output: executionResult.substring(0, 1000) },
        last_executed_at: new Date().toISOString()
      })
      .eq('id', taskId);
    
    broadcast({ type: 'task_status_update', taskId, status: 'done', projectId: task.project_id, result: executionResult });
    ...
    await supabaseAAA
      ...
      .eq('id', taskId);
    broadcast({ type: 'task_status_update', taskId, status: 'failed', projectId: task.project_id, error: err.message });
  }
  
  // تسجيل النشاط
  await supabaseAAA.from('agent_logs').insert({
    id: `log_${Date.now()}`,
    ...
    output: executionResult?.substring(0, 500),
    success: !executionError,
    error: executionError
  });
  
  return { success: !executionError, result: executionResult, error: executionError };
}

// استبدل executeTaskById القديمة بهذه النسخة الجديدة
async function executeTaskById(taskId) {
  const task = await getTaskById(taskId);
  const agentRole = task.assigned_role || 'developer';
  return executeTaskWithAgent(taskId, agentRole);
}
...
for (const [file, content] of Object.entries(generated)) {
  ...
  // داخل executeCodeR، بعد إنشاء الملفات وقبل saveToSupabase
if (userId && convId) {
  const newCheckpoint = {
    ...
    source: 'ai',
    model: 'code-R',
    description: `Code-R generated ${Object.keys(generated).length} files`,
    filesSnapshot: Object.entries(generated).map(([name, content]) => ({ name, content, type: getMimeType(name) })),
    changes: { built: Object.keys(generated), modified: [], deleted: [] }
  };
  // إضافة إلى checkpoints المحلية (للسيرفر)
  // يجب استرجاع checkpoints الحالية للمحادثة وإضافتها ثم حفظها
  const { data: conv } = await supabase.from('conversations').select('checkpoints').eq('id', convId).single();
  const checkpoints = conv?.checkpoints || [];
  checkpoints.push(newCheckpoint);
  await supabase.from('conversations').update({ checkpoints }).eq('id', convId);
}
  ...
  await saveToSupabase(userId, convId, message, fullResponse, filesArray, 'Code‑R', thoughtText, extractedTasks);
...
16. **SIZE REQUIREMENTS**:
   - CSS files: 400-1000+ lines
   - JavaScript files: 500-3000+ lines  
   - HTML files: 200-1000 lines (structure only, use external files)
   - Your response must be SUBSTANTIAL - never write tiny, minimal files
17. NEVER use <FILE> to rewrite an existing file. For modifications, ALWAYS use <REPLACE> or <ADD_TO> tags instead. Only use <FILE> for creating completely new files.
18. **CONTINUATION MARKER:** If you are forced to stop because you reached the maximum output limit AND you still have more code or files to generate, you MUST end your response with the EXACT marker: <|CONTINUE|>. This tells the system to continue generating with a secondary model. DO NOT use this marker if you have completed the full response.
--------------------------------------------------
...
async function summarizeConversationWithGemma(convId, userMessage, aiResponse) {
  ...
  // ✅ نظام تعليمات صارم جدًا – إجبار النموذج على إخراج عنوان فقط
  const systemInstruction = `You are a title generator. Output ONLY the title, nothing else. No JSON, no markdown, no extra words. Max 40 characters.`;

  const prompt = `Generate a short, descriptive title for this coding conversation in the same language as the user's message (English or Arabic). 
MAX 40 characters. No quotes, no punctuation at the end.
User: "${userMessage.substring(0, 150)}"
AI: "${aiResponse.substring(0, 200)}"
Title:`;

  try {
    // ✅ استخدام نفس الدالة المحمية مع إعدادات صارمة
    const rawTitle = await callGemmaWithProtection({
      genAI: new GoogleGenerativeAI(gemmaKeyInfo.token),
      modelConfig: {
        modelName: MODEL_CONFIGS['gemma'].modelName,
        temperature: 0.1,       // منخفضة جدًا للإخراج الحرفي
        maxOutputTokens: 2,    // صغير جدًا – لا يسمح بالكلام الزائد
        topP: 0.9,
        topK: 40
      },
      systemPrompt: systemInstruction,
    ...
    console.log(`📝 Raw Gemma response: "${rawTitle}"`);
    
    // تنظيف صارم: إزالة أي شيء غير مرغوب
    let cleaned = rawTitle
      .trim()
      .replace(/^["'`]|["'`]$/g, '')               // إزالة علامات الاقتباس
      .replace(/\n/g, ' ')                         // استبدال الأسطر بمسافة
      .replace(/\s+/g, ' ')                        // مسافات متعددة إلى واحدة
      .replace(/[^\w\s\u0600-\u06FF]/g, '')       // إزالة الرموز (احتفظ بالحروف العربية والإنجليزية والمسافات)
      .substring(0, 40);                          // قطع للطول المطلوب

    // إذا أصبح فارغًا أو قصيرًا جدًا، استخدم الـ fallback
    if (!cleaned || cleaned.length < 3) {
      console.log("⚠️ Cleaned title too short, using fallback");
      return generateFallbackSummary(userMessage);
    }

    // جعل الحرف الأول كبيرًا للعناوين الإنجليزية
    if (/^[a-z]/.test(cleaned)) {
      cleaned = cleaned.charAt(0).toUpperCase() + cleaned.slice(1);
    }

    console.log(`✅ Final title: "${cleaned}"`);

    // تحديث الإحصائيات (اختياري)
    const tokens = estimateTokens(prompt + rawTitle);
...
app.post('/api/settings/daily-tasks-limit', async (req, res) => {
  const { limit } = req.body;
  if (!limit || limit < 1 || limit > 200) {
    return res.status(400).json({ error: 'Limit must be between 1 and 200' });
  }
  const success = await setDailyTasksLimit(limit);
  if (success) {
    // إعادة تشغيل الطابور ليأخذ الإعداد الجديد
    if (queueRunning) {
      queueRunning = false;
      if (queueTimer) clearTimeout(queueTimer);
      startTaskQueue();
...
// نقطة نهاية لجلب حالة الطابور والإعدادات
app.get('/api/tasks/queue-status', async (req, res) => {
  const limit = await getDailyTasksLimit();
  const { count: pendingCount, error: pendingError } = await supabaseAAA
    .from('project_tasks')
    .select('*', { count: 'exact', head: true })
    .eq('status', 'pending');
  const { count: inProgressCount } = await supabaseAAA
    .from('project_tasks')
    .select('*', { count: 'exact', head: true })
    .eq('status', 'in_progress');
  res.json({
    dailyLimit: limit,
    executedToday: dailyTaskCount,
    pendingTasks: pendingError ? 0 : pendingCount,
    inProgressTasks: inProgressCount || 0,
    nextIntervalMs: (24 * 60 * 60 * 1000) / limit
  });
});


// === SHARE API: GET /api/share/:code ===
// === SHARE API: GET /api/share/:code ===
// ==========================================
// SHARE API: GET /api/share/:code
// يعيد رسالة الذكاء الاصطناعي + رسالة المستخدم + الملفات الخاصة بالمحادثة
// ==========================================
app.get('/api/share/:code', async (req, res) => {
  const shareCode = req.params.code;
  try {
    // 1. جلب بيانات المشاركة
    ...
    // 2. التحقق من صلاحية الرابط (expires_at)
    if (share.expires_at && new Date(share.expires_at) < new Date()) {
      return res.status(410).json({ error: 'Share link has expired' });
    }

    // 3. جلب محتوى رسالة الذكاء الاصطناعي
    const { data: aiMsg, error: aiError } = await supabase
      .from('messages')
      .select('content, model, files_snapshot, conv_id, created_at')
    ...
    // 4. جلب محتوى رسالة المستخدم (اختياري)
    let userMessageContent = '';
    if (share.user_message_id) {
      const { data: userMsg, error: userError } = await supabase
        .from('messages')
        .select('content')
        .eq('id', share.user_message_id)
        .maybeSingle();
    ...
    // 5. جلب جميع الملفات المرتبطة بنفس المحادثة (من جدول files)
    let conversationFiles = [];
    if (aiMsg.conv_id) {
      const { data: files, error: filesError } = await supabase
        .from('files')
        .select('id, name, content, url, preview_url, preview_urls, type, created_at')
        .eq('conv_id', aiMsg.conv_id)
        .order('created_at', { ascending: true });
      
      if (!filesError && files && files.length) {
        conversationFiles = files.map(f => ({
          id: f.id,
          name: f.name,
          content: f.content || null,
          url: f.url || null,
          previewUrl: f.preview_url || null,
          previewUrls: f.preview_urls || null,
          type: f.type || null,
          isBinary: !!f.url
        }));
      }
    ...
    // 6. دمج الـ files_snapshot (لقطة من الـ AI) مع conversationFiles (ملفات المحادثة)
    // نعطي الأولوية للـ files_snapshot إذا وجدت، ثم نضيف الملفات الإضافية من المحادثة
    let allFiles = [];
    const snapshotFiles = (aiMsg.files_snapshot || []).map(f => ({
      name: f.name,
      content: f.content,
      type: f.type,
      isSnapshot: true
    ...
    // إضافة الملفات من جدول files (مع تجنب التكرار حسب الاسم)
    const snapshotNames = new Set(snapshotFiles.map(f => f.name));
    const additionalFiles = conversationFiles.filter(f => !snapshotNames.has(f.name));

    allFiles = [...snapshotFiles, ...additionalFiles];

    // 7. إرجاع البيانات للواجهة الأمامية
    res.json({
      message_content: aiMsg.content,
      user_message: userMessageContent,
      model_name: aiMsg.model || 'AI',
      files_snapshot: allFiles,   // الآن يحتوي على جميع ملفات المحادثة
      created_at: share.created_at,
      conv_id: aiMsg.conv_id       // اختياري: يمكن استخدامه لاحقًا
    });
  } catch (err) {
...
app.post('/api/project/create', async (req, res) => {
  ...
  // إنشاء محادثة للمدير تلقائياً
  await supabaseAAA.from('project_conversations').insert({
    id: `conv_manager_${projectId}`,
    project_id: projectId,
    role: 'manager',
    messages: []
  });
  
  res.json({ projectId, project: data });
});

// ------------------------------
// 2. تحويل النص إلى مهام (باستخدام المحلل Analyst)
// ------------------------------
app.post('/api/tasks/generate', async (req, res) => {
  const { projectId, prompt } = req.body;
  
  if (!projectId || !prompt) {
    return res.status(400).json({ error: 'projectId and prompt required' });
  }
  
  console.log(`🤖 [Analyst] Analyzing: ${prompt.substring(0, 100)}...`);
  
  try {
    // استخدام نموذج المحلل (Trinity Large للتحليل العميق)
    const analystModel = 'gemini-3.1-flash-lite-preview';
    const keyInfo = getSafeKeyForModel(analystModel);
    
    if (!keyInfo) throw new Error('No key for analyst model');
    
    const systemPrompt = `You are a Project Analyst AI. Your task is to break down user requests into detailed, executable tasks with proper role assignments.
  ...
  "tasks": [
    {
      ...
      "description": "VERY DETAILED execution steps. Include: what to do, which files to create/modify, expected output, and technical specifications. (3-5 sentences)",
      "assigned_role": "manager|analyst|developer|tester",
      "order_index": 0,
      "dependencies": []
    }
  ],
  "manager_note": "Optional: A note to the manager about this plan"
}

ROLES & RESPONSIBILITIES:
- manager: Planning, prioritization, coordination, approving changes
- analyst: Breaking down requirements, research, creating specifications
- developer: Writing code, creating files, implementing features
- tester: Testing, validation, quality assurance, bug reporting

RULES:
- First task (order_index = 0) executes FIRST, last task executes LAST
- Each description MUST be detailed enough for an AI agent to execute autonomously
- Include dependencies when a task requires previous tasks to be done
- Assign the appropriate role based on task nature
- Generate 3-8 tasks

Now analyze this user request: "${prompt}"`;

    const genAI = new GoogleGenerativeAI(keyInfo.token);
    const model = genAI.getGenerativeModel({
      model: keyInfo.modelConfig.modelName, // أو النموذج المتاح لديك
      generationConfig: { maxOutputTokens: 4096, temperature: 0.3 }
    });
    ...
    for (const task of tasksData.tasks) {
      ...
      const { data: savedTask, error: insertError } = await supabaseAAA
        .from('project_tasks')
        .insert({
          id: taskId,
          project_id: projectId,
          title: task.title,
          description: task.description,
          assigned_role: task.assigned_role || 'developer',
    ...
    // إضافة ملاحظة المدير إلى محادثة المدير
    if (tasksData.manager_note) {
      const { data: conv } = await supabaseAAA
      ...
      messages.push({
        role: 'system',
        content: `📋 تم إنشاء ${savedTasks.length} مهام جديدة:\n${tasksData.manager_note}`,
        timestamp: Date.now()
      });
    ...
    await supabaseAAA.from('agent_logs').insert({
      ...
      input: prompt,
      output: JSON.stringify(savedTasks.map(t => t.title)),
      success: true
    });
    // بعد الحلقة التي تحفظ المهام
startTaskQueue(); // بدء الطابور فوراً إذا كان هناك مهام
    res.json({ 
      success: true, 
      tasks: savedTasks,
      managerNote: tasksData.manager_note || null
    });
...
app.post('/api/tasks/update-status', async (req, res) => {
  ...
  const { data, error: updateError } = await supabaseAAA
    .from('project_tasks')
    .update(updateData)
    .eq('id', taskId)
    .select()
...
// ------------------------------
// 5. محادثة مع المدير (طلب تعديل)
// ------------------------------
app.post('/api/project/:projectId/talk-to-manager', async (req, res) => {
  const { projectId } = req.params;
  const { message } = req.body;
  
  // جلب المحادثة الحالية للمدير
  let { data: conv } = await supabaseAAA
    .from('project_conversations')
    .select('*')
    .eq('project_id', projectId)
    .eq('role', 'manager')
  ...
  if (!conv) {
    ...
    const newId = `conv_manager_${projectId}_${Date.now()}`;
    const { data: newConv } = await supabaseAAA
      .from('project_conversations')
      .insert({
        id: newId,
        project_id: projectId,
  ...
  // استخدام الـ Manager AI للرد
  const managerModel = 'gemini-2.5-pro';
  const keyInfo = getSafeKeyForModel(managerModel);
  ...
  if (keyInfo) {
    try {
      ...
      // جلب المهام الحالية للسياق
      const { data: tasks } = await supabaseAAA
        .from('project_tasks')
        .select('title, status, order_index')
        .eq('project_id', projectId)
        .order('order_index', { ascending: true });
      
      const systemPrompt = `You are a Project Manager AI. You help manage tasks, prioritize work, and respond to user requests about the project.

Current project tasks:
${tasks?.map(t => `- [${t.status}] ${t.title}`).join('\n') || 'No tasks yet'}

Respond helpfully to the user's message. If they ask to modify tasks, explain what changes are possible. Be concise and professional.`;
      ...
      console.error('Manager AI error:', err);
      aiResponse = "تم استلام طلبك. سأقوم بمراجعته والعودة إليك قريباً.";
    }
  } else {
    aiResponse = "تم استلام رسالتك. سيراجعها المدير.";
  }
...
// ------------------------------
// 6. الحصول على محادثة المدير
// ------------------------------
app.get('/api/project/:projectId/manager-conversation', async (req, res) => {
  const { projectId } = req.params;
...
// ------------------------------
// 7. تنفيذ المهمة التالية (باستخدام الوكيل المناسب)
// ------------------------------
app.post('/api/tasks/execute-next', async (req, res) => {
  const { projectId } = req.body;
  
  // جلب أول مهمة pending (حسب order_index)
  const { data: pendingTasks, error } = await supabaseAAA
    .from('project_tasks')
    .select('*')
    .eq('project_id', projectId)
    .eq('status', 'pending')
  ...
  if (error || !pendingTasks || pendingTasks.length === 0) {
    return res.json({ message: 'No pending tasks', task: null });
  }
  ...
  // اختيار النموذج المناسب حسب الدور
  let modelId = 'gemini-3-flash'; // default
  switch (task.assigned_role) {
    case 'manager': modelId = 'gemini-2.5-pro'; break;
    case 'analyst': modelId = 'trinity-large'; break;
    case 'developer': modelId = 'codeai-code-r'; break;
    case 'tester': modelId = 'gemini-3-flash'; break;
  }
  ...
  if (keyInfo) {
    try {
      const genAI = new GoogleGenerativeAI(keyInfo.token);
      const model = genAI.getGenerativeModel({
        model: MODEL_CONFIGS[modelId]?.modelName || 'gemini-3-flash-preview',
        generationConfig: { maxOutputTokens: 4096, temperature: 0.3 }
      });
      
      const execPrompt = `You are a ${task.assigned_role} AI. Execute this task exactly as described.

TASK TITLE: ${task.title}
TASK DESCRIPTION: ${task.description}

Provide a complete, working solution. Output ONLY the result (code, files, or analysis).`;
  ...
  res.json({ 
    task: { ...task, status: executionError ? 'failed' : 'done' },
...
// 10. تحديث ملف
// ------------------------------
app.put('/api/project/:projectId/files/:fileId', async (req, res) => {
...
// 11. حذف ملف
// ------------------------------
app.delete('/api/project/:projectId/files/:fileId', async (req, res) => {
  const { projectId, fileId } = req.params;
  const { error } = await supabaseAAA
    .from('project_files')
    .delete()
...
if (!conversationMemory[convId]) {
  console.log(`🧠 [MEMORY-DEBUG] Loading memory from DB for conv ${convId}`);
  const { data: convData } = await supabase
    .from('conversations')
    .select('summary')
    .eq('id', convId)
    .single();
  const loadedSummary = convData?.summary || '';
  console.log(`🧠 [MEMORY-DEBUG] Loaded summary: "${loadedSummary.substring(0, 100)}..." (length=${loadedSummary.length})`);
...
if (!conversationMemory[convId]) {
  ...
  const loadedSummary = convData?.summary || '';
  console.log(`🧠 [MEMORY-DEBUG] Initializing memory for conv ${convId}. Loaded summary from DB: "${loadedSummary.substring(0, 50)}..."`);
...
// استخدم هذا:
const { count: actualTotalMessages, error: countError } = await supabase
  .from('messages')
  .select('id', { count: 'exact', head: true })
  .eq('conv_id', convId);

if (!countError) {
  currentTotalMessages = actualTotalMessages || 0;
} else {
  currentTotalMessages = history.length; // fallback
}
mem.messageCount = currentTotalMessages;

console.log(`🧠 [MEMORY-DEBUG] conv=${convId}, total msgs in history=${currentTotalMessages}, lastSummarizedCount=${mem.lastSummarizedCount}, messageCount=${mem.messageCount}`);

// احسب عدد الرسائل الجديدة التي لم تُلخص بعد
const unsummarizedCount = currentTotalMessages - mem.lastSummarizedCount;
console.log(`🧠 [MEMORY-DEBUG] unsummarizedCount=${unsummarizedCount} (threshold=${CHUNK_SIZE})`);
console.log(`🧠 [MEMORY-DEBUG] DB count = ${currentTotalMessages}, history.length = ${history.length}`);
console.log(`🧠 [MEMORY-DEBUG] unsummarizedCount = ${unsummarizedCount} (need ${CHUNK_SIZE} to trigger)`);
...
if (unsummarizedCount >= CHUNK_SIZE) {
  ...
  console.log(`🧠 [MEMORY-DEBUG] Triggering summarization. Chunk size=${chunkToSummarize.length}, startIdx=${startIdx}, total msgs=${currentTotalMessages}`);
  console.log(`🧠 [MEMORY-DEBUG] Chunk messages: ${chunkToSummarize.map(m => `${m.role}:${(m.content||'').substring(0,20)}`).join(' | ')}`);
  console.log(`🧠 [MEMORY-DEBUG] Current summary before update: "${mem.summary.substring(0, 100)}..."`);
  
  // تشغيل التلخيص والحفظ في الخلفية
  generateMemorySummary(chunkToSummarize, mem.summary)
    .then(async (newSummary) => {
      console.log(`🧠 [MEMORY-DEBUG] generateMemorySummary returned newSummary length=${newSummary?.length || 0}. Content: "${newSummary?.substring(0, 150)}..."`);
      if (newSummary && newSummary !== mem.summary) {
        mem.summary = newSummary;
        mem.lastSummarizedCount = currentTotalMessages; // حدّث العدد بعد الانتهاء
        console.log(`🧠 [MEMORY-DEBUG] Memory updated. New summary: "${newSummary.substring(0, 200)}"`);
        console.log(`🧠 [MEMORY-DEBUG] lastSummarizedCount updated to ${mem.lastSummarizedCount}`);
  ...
  .update({ 
    summary: newSummary,
    last_summarized_count: currentTotalMessages
  })
  .eq('id', convId);
        if (error) {
          console.error(`🧠 [MEMORY-DEBUG] Failed to persist summary to DB: ${error.message}`);
        } else {
          console.log(`🧠 [MEMORY-DEBUG] Summary saved to DB for conv ${convId}`);
        }
      } else {
        console.warn(`🧠 [MEMORY-DEBUG] New summary is empty or identical to old. No update.`);
      }
    })
    .catch(err => {
      console.error(`🧠 [MEMORY-DEBUG] Background memory summary failed:`, err.message);
    });
} else {
  console.log(`🧠 [MEMORY-DEBUG] Not enough new messages to summarize. Skipping.`);
}
...
app.get('/api/conversation', async (req, res) => {
  ...
  try {
    // 1. التحقق من صلاحية المستخدم وجلب المحادثة (مع checkpoints)
    ...
    // ✅ إضافة checkpoints إلى الاستجابة
    res.json({
      conversation: {
        ...conv,
...
app.post('/api/conversation', async (req, res) => {
  ...
  try {
    const { id, title, messages, files, checkpoints } = conversation;
    
    // تأكد من وجود المستخدم
    await ensureUser(userId);
    
    // حفظ المحادثة (بما في ذلك حقل checkpoints)
    const { error: convError } = await supabase
      ...
      .upsert({
        ...
        user_id: userId,
        title: title,
        checkpoints: checkpoints || [],  // ✅ إضافة checkpoints
        updated_at: new Date().toISOString()
      });
    
    if (convError) throw convError;
    
    // حفظ الرسائل (نفس الكود السابق)
...
// POST /api/feedback/general - حفظ تعليق عام
app.post('/api/feedback/general', authenticateUser, async (req, res) => {
  const userId = req.userId;
...
// 2. خدمة الملفات الثابتة من مسار /share أيضاً
app.use('/share', express.static(path.join(__dirname, '..', 'client')));
...
app.post('/api/share', async (req, res) => {
  console.log('📤 [SHARE] Request received. Body:', req.body);
  const userId = req.userId;
  const { messageId, userMsgId, modelName } = req.body;

  if (!userId || !messageId || !userMsgId) {
    console.log('❌ [SHARE] Missing fields:', { userId, messageId, userMsgId });
    return res.status(400).json({ error: 'Missing required fields: userId, messageId, userMsgId' });
  }
  ...
  try {
    ...
    console.log('✅ [SHARE] Share created:', shareCode);
    res.json({ shareCode });
  } catch (err) {
    console.error('❌ [SHARE] Database error:', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
app.get('/share/:code', (req, res) => {
  // إرسال ملف index.html الرئيسي، وسيقوم تطبيق الواجهة الأمامية بقراءة
  // shareCode من الرابط وتحميل البيانات عبر /api/share/:code
