...
JSON format:
{
  ...
  "search": {
    "needs": true/false,
    "query": "SEARCH_QUERY"  // 🚫 MUST be a non‑empty string if needs is true, otherwise null
  }
}
...
- If needs is true, you MUST generate a concise, accurate search query based on the user message. Do NOT leave it empty.
- If needs is false, set query to null.
...
if (parsed.search && parsed.search.needs && (!parsed.search.query || parsed.search.query.trim() === '')) {
  // إذا كان يحتاج بحث ولكن الاستعلام فارغ، استخدم رسالة المستخدم كاحتياط
  parsed.search.query = message;
  console.log("⚠️ Search query was empty, falling back to full message");
}
...
if (routeResult.search && routeResult.search.needs) {
  broadcast({ type: "search_status", status: "searching", query: routeResult.search.query }, clientId);
  try {
    searchContext = await performSearch(routeResult.search.query);
    broadcast({ type: "search_status", status: "completed" }, clientId);
  } catch (err) {
    console.error("Search failed:", err);
    broadcast({ type: "search_status", status: "failed" }, clientId);
  }
  // تخزين السياق في executionContext (اختياري للاستخدام لاحقًا)
