...
const requestedModel = settings?.selectedModel || 'gemini-3-flash';
    ...
    console.log(`🎯 Requested model: ${modelConfig.displayName} (${requestedModel})`);
...
console.log(`📝 User message: ${message.substring(0, 100)}...`);
  ...
  try {
    ...
    const model = genAI.getGenerativeModel({ 
        model: modelConfig.modelName, // ← استخدم modelName من الـ config
        generationConfig: { 
...
console.log("==================== FULL PROMPT SENT TO GEMINI ====================");
    ...
    let fullResponse = ""; // لتجميع النص الكامل

for await (const chunk of result.stream) {
  const chunkText = chunk.text();
  if (chunkText) {
    usageStats[activeKeyInfo.id].gemini.tpm += estimateTokens(chunkText);
    broadcast({ type: "assistant_message", text: chunkText });
    fullResponse += chunkText; // تجميع النص
  }
}

// بعد اكتمال الـ stream
conversationMemory[convId].history.push(fullResponse); // ✅ حفظ النص الكامل كسطر واحد
