...
async function ensureArabicFont() {
  ...
  const fontPath = path.join(fontDir, 'Amiri-Regular.ttf');
...
const storagePath = `${userId}/${convId}/${path.basename(filePath)}`;
...
if (document_type === "pdf") {
  ...
  try {
    ...
    for (const previewFile of previewFiles) {
      ...
      const previewStoragePath = `${userId}/${convId}/${safeName}_preview/${previewFile}`;
