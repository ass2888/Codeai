...
async function processNextTask() {
  ...
  if (dailyTaskCount >= dailyLimit) {
    console.log(`⏸️ Daily task limit reached (${dailyTaskCount}/${dailyLimit}). Waiting for next day...`);
    queueRunning = false;
  ...
  if (!task) {
    console.log('📭 No pending tasks. Queue idle.');
    queueRunning = false;
    if (queueTimer) clearTimeout(queueTimer);
    queueTimer = null;
    return;
  ...
  console.log(`⚙️ Executing task: ${task.title} (${task.id})`);
  dailyTaskCount++;
  await executeTaskById(task.id);
  
  // حساب التأخير حتى المهمة التالية بناءً على TPD
  const intervalMs = (24 * 60 * 60 * 1000) / dailyLimit;
  console.log(`⏱️ Next task will run in approx ${(intervalMs/1000/60).toFixed(1)} minutes (${dailyTaskCount}/${dailyLimit} today)`);
...
cron.schedule('* * * * *', async () => {
  ...
  // تنفيذ مهمة واحدة فقط كل دقيقة (بدلاً من الانتظار الطويل)
  const task = await getNextPendingTask();
  if (task) {
...
async function executeTaskWithAgent(taskId, agentRole) {
