...
fullResponse = await executeWithFallback({
    ...
    null,  // tasks - سيتم حفظها من خلال updateConversation بعد finishMessageProcessing
    null   // filesSnapshot - سيتم حفظها من خلال updateConversation بعد finishMessageProcessing
));
    }
  });
}
const extractedTasks = extractTasksFromResponse(fullResponse);
  ...
  await saveToSupabase(
    ...
    modelConfig.displayName,
    executionContext.thoughtText,
    extractedTasks,      // ✅ تمرير المهام المستخرجة
    filesSnapshot        // ✅ تمرير لقطة الملفات
  );
} catch (error) {
