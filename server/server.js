...
{
  "type": "document",
  "user_message": "Your friendly response to the user here (like 'I've created your PDF!' or 'Here's your presentation')",
  "document": {
...
RULES:
- The "user_message" field MUST contain a natural, friendly response in the user's language
...
{
  "type": "document",
  "user_message": "لقد قمت بإنشاء ملف PDF لك! يمكنك تحميله من الزر أدناه.",
  "document": {
    "document_type": "pdf",
...
async function sendResponseUnified({
  ...
  // ✅ POST-PROCESSING (قبل الإنهاء)
let handledAsDocument = false;
...
try {
        ...
        if (parsed.type === 'document' && parsed.document) {
            // إرسال رسالة المستخدم أولاً
            if (parsed.user_message) {
                onChunk(parsed.user_message);
            }
            ...
            // إرسال رسالة تأكيد إضافية (اختياري)
            onChunk("\n\n📄 Document ready for download!");
        } 
        // إذا كان JSON عادي (للحالات الأخرى)
            ...
            onChunk("✅ Document generated successfully!");
        }
        else {
            // ليس مستنداً، نرسل النص كاملاً
            onChunk(text);
        }
    } catch (e) {
        // ليس JSON صالح، نرسل النص كاملاً
...
`;
  ...
  const modelId = "gemini-3-flash"; // أو أي fast model
...
function buildPDF(content) {
  return new Promise(resolve => {
    ...
    doc.fontSize(14).text(content.text || "", { align: "left" });
    doc.end();
  });
}
...
async function handleDocumentGeneration({
  ...
  if (document_type === "pdf") {
    fileBuffer = await buildPDF(content);
    filePath = `/generated/${file_name || "document"}.pdf`;
  }

  if (document_type === "pptx") {
    fileBuffer = await buildPPTX(content);
    filePath = `/generated/${file_name || "presentation"}.pptx`;
  }

  if (document_type === "docx") {
    fileBuffer = await buildDOCX(content);
    filePath = `/generated/${file_name || "document"}.docx`;
  }

  fs.writeFileSync(path.join(PUBLIC_DIR, filePath), fileBuffer);
...
const systemPrompt = built?.systemPrompt ?? built;
const fullPrompt   = built?.userPrompt   ?? built;
...
try {
  ...
  onComplete: (full) => {
    broadcast({
      type: "session_info",
      modelName: executionContext.usedModels.join(" + "),
      convId
    }, clientId);
