...
async function buildPPTX(content) {
  ...
  let textToAdd = (slideData.text || "").trim();

  if (!textToAdd) textToAdd = " ";

  const lines = textToAdd.split('\n');
  const approxLinesPerSlide = 12; // رقم ثابت آمن

  for (let start = 0; start < lines.length; start += approxLinesPerSlide) {
    const slideLines = lines.slice(start, start + approxLinesPerSlide);
    const s = pptx.addSlide();

    s.addText(slideLines.join('\n'), {
      x: options.x,
      y: options.y,
      w: options.w,
      h: 6.5, // مهم للتنظيم (لكن ليس سبب المشكلة)
