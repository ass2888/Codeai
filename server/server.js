...
if (aiResponse && aiResponse.trim().length > 0) {
  await saveMessagesPair(userId, convId, message, aiResponse, modelName, thoughtText, tasks, filesSnapshot);
} else {
...
async function saveMessagesPair(userId, convId, message, aiResponse, modelName = null, thoughtText = null, tasks = null, filesSnapshot = null) {
  try {
    if (!userId) return;
...
const { error } = await supabase.rpc('insert_message_pair', {
  ...
  p_tasks: tasks,
  p_files_snapshot: filesSnapshot
});
...
for (const msg of messages) {
  await supabase.from('messages').insert({
    ...
    files_snapshot: msg.files_snapshot || null,
