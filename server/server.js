...
`;
  ...
  try {
    // المحاولة الأولى: تحليل مباشر إذا كان الرد JSONاً نقياً
    parsed = JSON.parse(text);
  } catch {
    // المحاولة الثانية: إزالة كل ما هو خارج أول '{' وآخر '}'
    ...
    if (firstCurly !== -1 && lastCurly !== -1 && lastCurly > firstCurly) {
      ...
      try {
        ...
        console.log("✅ JSON extracted from curly braces");
      } catch (innerError) {
        // المحاولة الثالثة: إزالة أي نصوص قبل الـ JSON باستخدام تعبير منتظم يبحث عن كلمة مفتاحية
        const jsonRegex = /({[\s\S]*})/;
  ...
  if (!parsed) {
    ...
    return fallbackTaskClassifier(message, files, historyContext);
  }
