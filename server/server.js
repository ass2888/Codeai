...
async function ensureArabicFont() {
  ...
  const oldFont = path.join(fontDir, 'Amiri-Regular.ttf');
if (fs.existsSync(oldFont)) fs.unlinkSync(oldFont);
  if (!fs.existsSync(fontPath)) {
    ...
    await new Promise((resolve, reject) => {
      https.default.get(
        ...
        res => { res.pipe(file); file.on('finish', () => { file.close(); resolve(); }); }
      ).on('error', reject);
    });
    console.log("✅ Arabic font downloaded");
  }
...
const storagePath = `${userId}/${convId}/${encodeURIComponent(path.basename(filePath))}`;
...
if (document_type === "pdf") {
  ...
  try {
    ...
    for (const previewFile of previewFiles) {
      ...
      const previewStoragePath = `${userId}/${convId}/${encodeURIComponent(safeName)}_preview/${previewFile}`;
