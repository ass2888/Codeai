...

{
  ...
  "document": {
    ...
    "content": {
      ...
      "slides": [
        {
          "text": "Slide content with optional formatting",
          "options": {
            "fontSize": 24,
            "bold": true,
            "italic": false,
...
async function buildDOCX(content) {
  const paragraphs = content.paragraphs.map(p => {
    ...
    // تحديد المحاذاة: إذا كان النص عربي، استخدم RIGHT
  ...
  const doc = new Document({
    sections: [{
      properties: { 
        bidi: true,  // تفعيل BiDi للنصوص العربية
        page: {
          margin: {
            top: 720,   // 1 inch
            bottom: 720,
...
async function buildPPTX(content) {
  ...
  content.slides.forEach((slide, index) => {
    ...
    // دمج الإعدادات الافتراضية مع إعدادات الشريحة (إن وجدت)
    const options = { ...defaultOptions, ...(slide.options || {}) };
    
    // معالجة النص: تقسيم الأسطر للقوائم النقطية
    let textToAdd = slide.text;
    let isBulletList = false;
    
    // التحقق مما إذا كان النص يحتوي على قائمة نقطية
    if (options.bullet && textToAdd.includes('\n')) {
      isBulletList = true;
    }
    ...
    // إضافة النص إلى الشريحة
    if (isBulletList) {
      // للقوائم النقطية: تقسيم النص إلى أسطر منفصلة
      const lines = textToAdd.split('\n').filter(line => line.trim());
      const bulletPoints = lines.map(line => ({
        text: line.replace(/^[•\-*]\s*/, ''), // إزالة الرموز النقطية الموجودة
    ...
    // يمكن إضافة تنسيقات إضافية للشريحة نفسها (خلفية، انتقالات) مستقبلاً
