...
async function routeTaskMandatory(message, history = []) {
...
`;
  ...
  try {
    ...
    // fallback ذكي: تحقق من وجود كلمات مفتاحية في السياق
    const fullContext = (historyContext + " " + message).toLowerCase();
    if (fullContext.includes('docx') || fullContext.includes('word') || (fullContext.includes('عدل') && fullContext.includes('ملف'))) {
...
async function buildDOCX(content) {
  const paragraphs = content.paragraphs.map(p => {
    ...
    return new Paragraph({
      ...
      bidirectional: direction === "rtl" // ✅ تفعيل BiDi
    });
  });
  ...
  content.slides.forEach((slide, index) => {
    ...
    if (options.bullet) {
      ...
      const bulletPoints = lines.map(line => ({
        text: line.replace(/^[•\-*\d+\.\s]*/, ''), // إزالة الرموز النقطية
      ...
      // نص عادي - نضيف كل فقرة على حدة مع مسافة بينها
      let currentY = options.y;
      const lineHeightInches = options.fontSize * options.lineSpacing / 72; // تحويل إلى بوصة
      const paraSpacingInches = 0.2; // مسافة ثابتة بين الفقرات (حوالي 14 نقطة)
      
      for (let i = 0; i < paragraphs.length; i++) {
        const para = paragraphs[i];
        if (!para.trim()) continue;
        
        // حساب ارتفاع النص التقريبي
        const charsPerLine = Math.floor(options.w * 72 / (options.fontSize * 0.6));
        const lines = Math.ceil(para.length / charsPerLine);
        const paraHeight = lines * lineHeightInches;
        ...
        // إذا تجاوزت الشريحة، نضيف شريحة جديدة
        if (currentY + lineHeightInches > options.y + options.h && i < paragraphs.length - 1) {
          // نضيف شريحة جديدة ونعيد تعيين currentY
          const newSlide = pptx.addSlide();
          currentY = options.y;
          // نستمر في إضافة الفقرات المتبقية على الشريحة الجديدة
        }
      }
    }
...
const routeResult = await routeTaskMandatory(message, history);
