...
async function buildPPTX(content) {
  ...
  for (const slideData of content.slides) {
    ...
    // حساب الحد الأقصى للأسطر في الشريحة
    const pointsPerInch = 72;
    const lineHeightPoints = options.fontSize * options.lineSpacing;
    const lineHeightInches = lineHeightPoints / pointsPerInch;
    const maxLines = Math.floor(options.h / lineHeightInches);
    
    const allLines = textToAdd.split('\n');
    
    // ========== حالة القوائم النقطية ==========
    if (options.bullet) {
      // تصفية الأسطر الفارغة للقوائم
      const bulletLines = allLines.filter(line => line.trim());
      
      // تقسيم القوائم إلى شرائح متعددة
      for (let i = 0; i < bulletLines.length; i += maxLines) {
        const chunk = bulletLines.slice(i, i + maxLines);
        
        // إزالة الرموز النقطية من النص
        const bulletPoints = chunk.map(line => ({
    ...
    // ========== حالة النص العادي ==========
    // تقسيم النص العادي إلى شرائح متعددة
    for (let i = 0; i < allLines.length; i += maxLines) {
      const slideLines = allLines.slice(i, i + maxLines);
      const s = pptx.addSlide();
      
      s.addText(slideLines, {
        x: options.x,
        y: options.y,
        w: options.w,
        h: options.h,
