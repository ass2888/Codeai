...
12. You MUST output COMPLETE, FULL code without truncation
13. NEVER cut off or shorten your response
14. Include ALL functions, styles, and interactive elements
15. Make the project COMPLETE and FULLY FUNCTIONAL
16. Your response can be VERY LONG (up to 1 million tokens)
17. DO NOT stop early, write everything completely
18. ADD 2-7 additional features the user might like to the project in building
--------------------------------------------------
...
try {
  if (!isAutoMode(requestedModel)) {
    ...
    await sendResponseUnified({
      ...
      onComplete: (full) => {
        ...
        const extractedTasksManual = extractTasksFromResponse(full);
        const filesSnapshotManual = files.map(file => ({
          name: file.name,
    ...
    // ✅ Auto mode: لا تحفظ داخل onComplete
    fullResponse = await executeWithFallback({
      ...
      executionContext,
      onChunk: (text) => { 
        broadcast({ type: "assistant_message", text }, Array.isArray(clientId) ? clientId[0] : clientId); 
      },
      onComplete: (full) => {
        broadcast({ type: "session_info", modelName: executionContext.usedModels.join(" + "), convId }, clientId);
        broadcast({ type: "stream_complete" }, clientId);
        // ✅ لا تحفظ هنا - فقط سجل fullResponse
        fullResponse = full;
      }
    });
  }
  
  // ✅ حفظ مرة واحدة فقط بعد الحصول على fullResponse (لكلا الوضعين)
  ...
  try {
    ...
    if (simpleModel) {
      ...
      const emergencyModel = genAI.getGenerativeModel({
        model: "gemini-2.5-flash",  // ✅ استخدم نموذجاً مختلفاً
        generationConfig: { 
      ...
      emergencyResponse = result.response.text();
      
      if (emergencyResponse) {
        broadcast({ type: "assistant_message", text: emergencyResponse }, clientId);
        broadcast({ 
          type: 'session_info', 
          modelName: "Gemini 2.5 Flash (Emergency)",
          convId: convId
        }, clientId);
        broadcast({ type: "stream_complete" }, clientId);
      }
    ...
    // ✅ إذا فشل كل شيء، أرسل رسالة خطأ ودية للمستخدم
    const errorMessage = "⚠️ عذراً، خدمة الذكاء الاصطناعي مشغولة حالياً. يرجى المحاولة مرة أخرى بعد قليل.";
    broadcast({ type: "assistant_message", text: errorMessage }, clientId);
    broadcast({ type: "stream_complete" }, clientId);
    emergencyResponse = errorMessage;
  }
  
  // ✅ تأكد من حفظ الرد (حتى لو كان رسالة خطأ)
  const extractedTasks = extractTasksFromResponse(emergencyResponse);
  const filesSnapshot = files.map(file => ({
    name: file.name,
    content: file.content,
    type: file.type || getMimeType(file.name)
  ...
  await saveToSupabase(
    ...
    convId, 
    message, 
    emergencyResponse || "⚠️ عذراً، لم نتمكن من معالجة طلبك حالياً. يرجى المحاولة مرة أخرى.",
    [],
    "Emergency Model",
    executionContext.thoughtText,
    extractedTasks,
...
app.post('/api/welcome-messages', async (req, res) => {
    ...
    try {
        // الحصول على مفتاح لـ Gemma 1B
        const gemmaKeyInfo = getSafeKey('gemma');
        if (!gemmaKeyInfo) {
            // Fallback messages if no key available
            const fallbackMessages = language === 'ar' 
                ? [
        ...
        const prompt = `You are a friendly coding assistant. Generate 5 short, warm, engaging welcome messages for the user. Each message should be a single sentence, max 12 words, friendly tone. Language: ${language === 'ar' ? 'Arabic' : 'English'}. Return ONLY valid JSON array, no extra text. Example: ["Message 1", "Message 2", "Message 3", "Message 4", "Message 5"]`;
        ...
        if (jsonMatch) {
            try {
                ...
                messages = language === 'ar' 
                    ? ["مرحباً! كيف أخدمك اليوم؟", "جاهز للبرمجة؟", "لنبدأ!", "ما هو سؤالك؟", "أنا هنا لمساعدتك"]
                    : ["Hello! How can I serve you today?", "Ready to code?", "Let's get started!", "What's your question?", "I'm here to help"];
            }
        } else {
            messages = language === 'ar' 
                ? ["مرحباً! كيف أخدمك اليوم؟", "جاهز للبرمجة؟", "لنبدأ!", "ما هو سؤالك؟", "أنا هنا لمساعدتك"]
        ...
        // تحديث الاستخدام
        const tokens = estimateTokens(prompt + JSON.stringify(messages));
        updateUsage(gemmaKeyInfo.id, 'gemma', tokens);
        
        res.json({ messages });
        
    } catch (error) {
        console.error("❌ Welcome messages error:", error);
        // Fallback messages
        const fallbackMessages = language === 'ar' 
