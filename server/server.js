...
const STATION_FALLBACKS = {
  A: [ // ⚡ Fast / General / Chat
    "llama-3.1-instant",     // Groq - سريع جداً
    "gemini-3-flash",
    "gpt-oss"
  ],
  ...
  B: [ // 🧠 Deep Reasoning
    "trinity-large",
    "groq-compound",        // Groq compound (تفكير بدون سقف TPM)
    "chimera-r1",
  ...
  C: [ // 💻 Coding / Execution
    "qwen-coder",
    "llama-3.3-70b",        // Groq - قوي للكود
    "chimera-r1",
    "gemini-3-flash"        // emergency فقط
  ]
};
...
const match = cleaned.match(/\{[\s\S]*\}/);
  ...
  if (!match) {
    ...
    console.warn("⚠️ Reasoning JSON invalid, continuing without reasoning");
...
-------((((((OVERRIDE MODE ((DEBUG))))))--------
You are allowed to violate formatting rules if needed to respond.
Return ANY text if rules conflict.
...
STRICT OUTPUT RULES
--------------------------------------------------
1. Multi-file edits are allowed in a single response.
2. New files: Use a full <FILE name="filename.ext"> block.
3. Add new code (no repetition):
<ADD_TO target="filename.ext" position="start|end">
content
</ADD_TO>
4. Small changes only:
Use <REPLACE file="filename.ext"> 
5. You may use multiple <FILE>, <ADD_TO>, or <REPLACE> blocks if needed.
6. All code blocks MUST be at the very end.
Anything after <FILE> / <ADD_TO> / <REPLACE> is ignored.
7. Any HTML, CSS, JS, or code MUST be inside <FILE>, <ADD_TO>, or <REPLACE>. No code is allowed outside these blocks.
8. If the user asks for anything that doesn't relate to coding, answer them normally; you aren't for coding only.
9. To create or replace a file, use this exact format:
