...
// تهيئة جميع المفاتيح
// غيّر السطر ليصبح:
['G1', 'G2', 'G3', 'D1', 'O1', 'O2'].forEach(keyId => {
    usageStats[keyId] = {
...
async function callOpenRouterAPI(keyInfo, messages) {
    const response = await fetch(keyInfo.modelConfig.apiUrl, {
        ...
        headers: {
            ...
            'HTTP-Referer': 'https://codeai.app', // مطلوب من OpenRouter لظهور التطبيق في الترتيب
            'X-Title': 'CodeAI'
        },
        body: JSON.stringify({
            model: keyInfo.modelConfig.modelName, // "qwen/qwen3-coder:free"
            messages: messages,
    ...
    if (!response.ok) {
        ...
        // إذا كان الخطأ 429 (rate limit)، نستخدم fallback
        if (response.status === 429) {
            console.log(`⚠️ OpenRouter rate limit (429) for ${keyInfo.modelConfig.modelName}, using fallback...`);
            throw new Error('RATE_LIMITED'); // رمز خاص للتعرف على هذا الخطأ
        }
...
gemini-2.5-pro
gemini-2.5-flash
arcee-ai/trinity-large-preview:gemini-3-flash-preview
tngtech/deepseek-r1t2-chimera:gemini-3-flash-preview
nousresearch/hermes-3-llama-3.1-405b:gemini-3-flash-preview
openai/gpt-oss-20b:gemini-3-flash-preview
...
`;
  ...
  const genAI = new GoogleGenerativeAI(
    getSafeKeyForModel("gemini-3-flash-preview").token
  );
...
- Strategy.
- Risks.
- This section is PRIVATE and will NEVER be shown to the user.

2) USER_EXPLANATION
- A simplified explanation for the user.
- DO NOT reveal chain-of-thought.
- DO NOT explain step-by-step thinking.
- Focus on:
  - What was wrong
  - Why it mattered
  - What approach will be used
- Clear, short, and friendly.
...
`;
  ...
  let currentModelKey = taskInfo.reasoning_model;
  let attempts = 0;
  const maxAttempts = 3;
  let text = "";
  let keyInfo = null;
...
while (!finalKeyInfo && fallbackAttempts < maxFallbackAttempts) {
  if (fallbackAttempts === 0) {
    // المحاولة الأولى: جرب النموذج المطلوب
    finalKeyInfo = getSafeKeyForModel(finalModel);
  } else if (fallbackAttempts === 1) {
    // المحاولة الثانية: جرب Gemini Flash
    console.log(`⚠️ Attempt #${fallbackAttempts}: Trying Gemini Flash fallback...`);
    ...
    // المحاولة الثالثة: جرب أي مفتاح متاح
    console.log(`⚠️ Attempt ${fallbackAttempts}: Trying any available key...`);
    finalKeyInfo = getSafeKey('gemini') || getSafeKey('gemma');
    if (finalKeyInfo) {
      provider = 'google';
      finalKeyInfo.modelConfig = {
        modelName: 'gemini-3-flash-preview',
        ...
        temperature: 0.7,
        supportsStreaming: true,
        displayName: 'Gemini 3 Flash (Emergency)'
      };
    }
  }
...
if (!finalKeyInfo) {
  // كل المحاولات فشلت - إرسال رسالة ودية دون إيقاف العملية
  broadcast({ 
    type: 'assistant_message', 
    text: 'عذراً، جميع النماذج مشغولة حاليًا. يرجى المحاولة مرة أخرى بعد قليل.' 
  }, clientId);
  ...
  if (!text && attempts >= maxAttempts) {
    console.error("❌ All reasoning attempts failed");
    return null;
  }

  const match = text.match(/\{[\s\S]*\}/);
  if (!match) {
    console.error("❌ No JSON found in reasoning response");
    return null;
  }
...
const isAutoPro = requestedModel === "codeai-pro";
...
if (requestedModel === "auto" || requestedModel === "codeai-pro") {
    ...
    provider = MODEL_CONFIGS[finalModel].provider;
...
You operate inside the Codeai PRO system.
...
- You assist the user inside Codeai PRO
...
try {
  await sendResponseUnified({
    ...
    maxRetries: 1 // محاولة إضافية واحدة
  });
} catch (error) {
