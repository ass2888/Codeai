...
try {
  ...
  // تحقق أن الرد خاص بإنشاء ملف
  if (parsed && parsed.document_type) {
    await handleDocumentGeneration({
      json: parsed,
      clientId: keyInfo.clientId, // أو مرره من الأعلى
      convId: keyInfo.convId
    });

    handledAsDocument = true;
  }
} catch (e) {
  // ليس JSON → تجاهل
}
...
if (context && modelName) {
  if (!context.usedModels) {
      context.usedModels = [];
    }
    if (!context.usedModels.includes(modelName)) {
      context.usedModels.push(modelName);
    }
  }
  broadcast({
