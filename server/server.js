...
if (provider === 'google') {
  if (supportsStreaming) {
  ...
  } else if (provider === 'deepseek' || provider === 'openrouter') { // ⬅ دمج OpenRouter
    ...
    // استخدام الدالة المناسبة حسب المزود
...
function sendStageUpdate(clientId, userText, modelId, actionText) {
  const modelName = MODEL_CONFIGS[modelId]?.displayName || "AI";

  broadcast({
    type: "assistant_status",
    stage: {
      user: userText,
      model: `${modelName} ${actionText}`
    }
  }, clientId);
}
...
`;
  ...
  const genAI = new GoogleGenerativeAI(
    getSafeKeyForModel(taskInfo.reasoning_model).token
  );

  const model = genAI.getGenerativeModel({
    model: MODEL_CONFIGS[taskInfo.reasoning_model].modelName,
    generationConfig: {
      temperature: 0.2,
      maxOutputTokens: 600
    }
...
if (requestedModel === "auto" || requestedModel === "codeai-pro") {
    ...
    finalModel = routeResult.execution_model; // مثل qwen-coder
...
await sendResponseUnified({
    ...
    onComplete: (fullResponse) => {
        if (conversationMemory[convId]) {
          ...
          broadcast({ 
              type: 'session_info', 
              modelName: combinedModelName, // سيظهر هذا الاسم في الواجهة
              convId: convId
          }, clientId);
